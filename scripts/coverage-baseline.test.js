...
test('reviewed coverage baselines match the clean measurement envelopes', () => {
    ...
    assert.deepEqual(baselines.profiles.server.measured, { coveredLines: 47263, totalLines: 57163 });
...
test('server relationship-reuse scope accepts only its clean observed envelope', () => {
    ...
    for (const [coveredLines, ok, reason] of [
        ...
        [47261, true, 'within-tolerance'],
        [47263, true, 'exact'],
        [47264, false, 'stale-baseline'],
    ]) {
    ...
    for (const totalLines of [57162, 57164]) {
        const result = evaluateCoverage({ coveredLines: 47263, totalLines }, profile);
