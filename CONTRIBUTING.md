...
- `scripts/static-css.js` — compacts whitespace in four inventoried CSS templates for production. Keep CSS readable and place `/* @canopy-static-css */` immediately before an untagged template without substitutions. Within the inventory, misplaced annotations, count drift and parser warnings/errors fail the build. TypeScript's maps compose through esbuild with full original source contents. Dev/watch keeps the original templates; runtime style ownership and output inventory stay unchanged. New opt-ins or CSS syntax minification need compatibility and source-map review; existing byte caps still apply.
