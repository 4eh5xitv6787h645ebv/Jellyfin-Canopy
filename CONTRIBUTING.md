...
- `scripts/static-css.js` — compacts whitespace in three inventoried CSS templates for production. Keep CSS readable and place `/* @canopy-static-css */` immediately before an untagged template without substitutions. Within the inventory, misplaced annotations, count drift and parser warnings/errors fail the build. TypeScript's maps compose through esbuild with full original source contents. Dev/watch keeps the original templates; runtime style ownership and output inventory stay unchanged. New opt-ins or CSS syntax minification need compatibility and source-map review; existing byte caps still apply.
...
Coverage thresholds are **ratchets**. For each fixed scope, `scripts/coverage-baselines.json` records a reviewed clean-run envelope and uses the observed high-water—not a modal or repeated lower result—as its baseline; that high-water never decreases. A narrow downward instrumentation tolerance defines the floor. Update the envelope as coverage advances. A fixed-scope floor may move downward only to the exact minimum proven by linked clean-run evidence on identical source, tests, and total-line scope; unexplained loss, an unevidenced or unreviewed tolerance increase, and unreviewed scope drift remain blocking. The coverage commands already execute the complete unit suite, so do not run the corresponding plain command immediately before them.
