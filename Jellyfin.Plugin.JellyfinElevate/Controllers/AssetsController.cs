...
namespace Jellyfin.Plugin.JellyfinElevate.Controllers
{
    ...
    /// at /JellyfinElevate/assets/{key} so browsers never talk to a CDN. See
    ...
    public class AssetsController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetAsset(string key)
        {
            ...
            // Embedded assets ship inside the plugin DLL and involve no third-party
            // fetch, so the AssetCacheEnabled kill-switch (which exists to stop CDN
            // mirroring) does not apply to them — they have no CDN twin to fall back to.
        ...
        private ActionResult ServeEmbedded(ResolvedAsset asset)
        {
            ...
            var etag = $"\"emb-{JellyfinElevate.Instance?.Version?.ToString() ?? "unknown"}\"";
            if (NotModified(etag))
            {
                stream.Dispose();
                return StatusCode(StatusCodes.Status304NotModified);
