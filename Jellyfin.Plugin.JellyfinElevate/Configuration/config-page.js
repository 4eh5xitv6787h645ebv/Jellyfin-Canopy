        (() => {
            ...
            const page = document.querySelector('#JellyfinElevatePage');
            const form = document.querySelector('#JellyfinElevateForm');
            ...
            // never open this tab).
            const DOCS_URL = 'https://4eh5xitv6787h645ebv.github.io/Jellyfin-Elevate/';
            ...
            tabs.forEach(tab => {
                tab.addEventListener('click', () => {
                    ...
                    try {
                        sessionStorage.setItem('jellyfinElevateActiveTab', tabId);
                    } catch (e) {
            ...
            try {
                let savedTab = sessionStorage.getItem('jellyfinElevateActiveTab');
                if (savedTab && LEGACY_TAB_MAP[savedTab]) {
                    savedTab = LEGACY_TAB_MAP[savedTab];
                    sessionStorage.setItem('jellyfinElevateActiveTab', savedTab);
                }
                if (savedTab && document.getElementById(savedTab)) {
                    ...
                    sessionStorage.removeItem('jellyfinElevateActiveTab');
                }
            } catch (e) {
            ...
            function exitSearchMode() {
                ...
                try {
                    savedTab = sessionStorage.getItem('jellyfinElevateActiveTab') || 'overview';
        ...
        async function testJellyseerrConnection() {
            ...
            for (const url of urls) {
                try {
                    const validationUrl = ApiClient.getUrl(`/JellyfinElevate/jellyseerr/validate`, {
                        url: url
                    });
        ...
        async function testTmdbConnection(event) {
            ...
            try {
                const validationUrl = ApiClient.getUrl(`/JellyfinElevate/tmdb/validate`, { apiKey: apiKey });
        ...
        var CUSTOM_TAB_MANAGED_ENTRIES = [
            ...
            { masterKey: 'HiddenContentEnabled',  parentKey: 'HiddenContentUseCustomTabs', autoKey: 'HiddenContentAutoCreateCustomTab', ownedKey: 'HiddenContentCustomTabJeOwned', title: 'Hidden Content', html: '<div class="jellyfinelevate hidden-content"></div>' },
            { masterKey: 'DownloadsPageEnabled',  parentKey: 'DownloadsUseCustomTabs',     autoKey: 'DownloadsAutoCreateCustomTab',     ownedKey: 'DownloadsCustomTabJeOwned',     title: 'Requests',       html: '<div class="jellyfinelevate requests"></div>' },
            { masterKey: 'CalendarPageEnabled',   parentKey: 'CalendarUseCustomTabs',      autoKey: 'CalendarAutoCreateCustomTab',      ownedKey: 'CalendarCustomTabJeOwned',      title: 'Calendar',       html: '<div class="jellyfinelevate calendar"></div>' }
        ];
        ...
        function checkCustomTabsConfigCompat() {
            ApiClient.ajax({
                ...
                if (!ok) {
                    ...
                    setProbeWarning('customtabs', "Custom Tabs config has an unrecognized shape. Auto-create toggles disabled until Jellyfin Elevate supports the new schema.");
                } else {
                    setProbeWarning('customtabs', null);
                }
            ...
            return ApiClient.ajax({
                ...
                if (!isCustomTabsConfigShapeOk(cfg)) {
                    return {
                        ok: false,
                        status: 'failed',
                        detail: 'Custom Tabs configuration shape no longer matches what Jellyfin Elevate knows how to write — auto-manage skipped to avoid corrupting it.',
                        ownedUpdates: []
                    };
                }
                var changed = false;
        ...
        function loadAutoMovieRadarrServers(savedConfig) {
            ...
            ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl('/JellyfinElevate/jellyseerr/radarr'),
                dataType: 'json'
        ...
        function loadAutoMovieServerDetails(serverId, savedConfig) {
            ...
            ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl('/JellyfinElevate/jellyseerr/radarr/' + serverId),
                dataType: 'json'
        ...
        function renderQualityCatOrderAdmin(config) {
            ...
            try {
                ...
                console.error('Jellyfin Elevate: renderQualityCatOrderAdmin failed; will skip *Order save', err);
            }
        }
        ...
        async function buildConfigFromForm() {
            ...
            (function () {
                ...
                if (invalid.length > 0) {
                    console.warn('Jellyfin Elevate: dropping invalid Seerr URL(s) on save (must start with http:// or https://):', invalid);
            ...
            try {
                ...
                return Object.assign({}, syncResult, {
                    ...
                    detail: (syncResult.detail ? syncResult.detail + ' — ' : '') +
                            "Custom Tabs has the new entry, but Jellyfin Elevate could not save its ownership record. " +
        ...
        async function saveConfig(e) {
            ...
            try {
                ...
                try {
                    if (config.MaintenanceModeEnabled) {
                        ...
                        await ApiClient.ajax({
                            type: 'POST',
                            url: ApiClient.getUrl('/JellyfinElevate/MaintenanceMode/Enable'),
                            contentType: 'application/json',
                            data: JSON.stringify({
                        ...
                        try {
                            await ApiClient.ajax({
                                type: 'POST',
                                url: ApiClient.getUrl('/JellyfinElevate/MaintenanceMode/Broadcast'),
                                contentType: 'application/json',
                                data: JSON.stringify({
                        ...
                        await ApiClient.ajax({
                            type: 'POST',
                            url: ApiClient.getUrl('/JellyfinElevate/MaintenanceMode/Disable')
                        });
                    }
                } catch (mmErr) {
                ...
                if (syncResult && syncResult.ok === false) {
                    try {
                        Dashboard.alert({
                            ...
                            message: 'Your Jellyfin Elevate settings were saved, but the Custom Tabs entry could not be updated.\n\n' +
                ...
                try {
                    Dashboard.alert({
                        title: 'Save failed',
                        message: 'Could not save Jellyfin Elevate settings. Check the browser console and server logs, then try again.'
                    });
                } catch (alertErr) { console.warn('[JE] Dashboard.alert threw:', alertErr); }
            } finally {
        ...
        async function resetAllUserSettings() {
            if (confirm("Are you sure?\n\nThis will save the current configuration and overwrite every per-user default for ALL users on this server.")) {
                ...
                try {
                    ...
                    await ApiClient.ajax({
                        type: 'POST',
                        url: ApiClient.getUrl('/JellyfinElevate/reset-all-users-settings'),
                        dataType: 'json'
                    });
        ...
        function loadMaintenanceUsers() {
            ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl('/JellyfinElevate/MaintenanceMode/Users'),
                dataType: 'json'
            }).then(function(users) {
        ...
        async function uploadBrandingImage(file, config, statusDiv) {
            ...
            try {
                ...
                const response = await fetch(ApiClient.getUrl('/JellyfinElevate/UploadBrandingImage'), {
                    method: 'POST',
                    body: formData,
                    headers: {
        ...
        async function refreshBrandingPreview(config) {
            ...
            const token = ApiClient.accessToken ? ApiClient.accessToken() : '';
            try {
                const response = await fetch(ApiClient.getUrl('/JellyfinElevate/BrandingImage', { fileName: config.fileName, t: Date.now() }), {
                    // Jellyfin 12 authenticates from the Authorization header; the
                    // legacy X-MediaBrowser-Token is kept for 10.11 back-compat.
        ...
        async function deleteBrandingImage(config, statusDiv) {
            ...
            try {
                const response = await fetch(ApiClient.getUrl('/JellyfinElevate/DeleteBrandingImage'), {
                    method: 'POST',
                    body: formData,
                    // Jellyfin 12 authenticates from the Authorization header; the
                    // legacy X-MediaBrowser-Token is kept for 10.11 back-compat.
        ...
        (function wireRequestsBannerReactive() {
            var formEl = document.getElementById('JellyfinElevateForm');
            if (!formEl) {
                console.error('[JE] #JellyfinElevateForm missing — reactive dep updates disabled');
                return;
            }
        ...
        async function testInstanceConnection(card) {
            ...
            try {
                ...
                var validationUrl = ApiClient.getUrl('/JellyfinElevate/arr/validate/' + endpoint, { url: urlVal });
        ...
        async function triggerSeerrScanNow() {
            ...
            for (const url of urls) {
                try {
                    const triggerUrl = ApiClient.getUrl('/JellyfinElevate/jellyseerr/trigger-recently-added-scan', { url: url });
        ...
        function syncBlockedUsersToHiddenInput() {
            if (!_blockedUsersLoaded) {
                console.warn('Jellyfin Elevate: skipping blocklist sync — user list failed to load. Existing config preserved.');
                return;
            }
        ...
        document.getElementById('btnImportJellyseerrUsers').addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await ApiClient.fetch({
                    url: ApiClient.getUrl('JellyfinElevate/jellyseerr/import-users'),
                    type: 'POST',
                    dataType: 'json'
                });
        ...
        // GETs /JellyfinElevate/jellyseerr/permission-audit which returns
            ...
            btn.addEventListener('click', async function() {
                ...
                try {
                    var data = await ApiClient.ajax({
                        type: 'GET',
                        url: ApiClient.getUrl('/JellyfinElevate/jellyseerr/permission-audit'),
                        dataType: 'json'
                    });
        ...
        if (testCustomPluginLinksBtn) {
            testCustomPluginLinksBtn.addEventListener('click', async () => {
                ...
                if (window.JellyfinElevate && window.JellyfinElevate.customPlugins) {
                    // Temporarily store test data
                    window.testCustomPluginLinks = validLinks;
                    window.JellyfinElevate.customPlugins.refresh();
                }
            });
        }
