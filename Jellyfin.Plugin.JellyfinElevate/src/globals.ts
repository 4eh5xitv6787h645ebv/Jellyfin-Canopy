// src/globals.ts
//
// The one place src/ modules obtain the shared window.JellyfinElevate
...
if (!window.JellyfinElevate) {
    throw new Error(
        '🪼 Jellyfin Elevate: window.JellyfinElevate is missing — the client bundle was ' +
        'loaded before js/plugin.js created the namespace. Load order is plugin.js first.'
    );
}

/** The shared plugin namespace (window.JellyfinElevate). */
export const JE: JEGlobal = window.JellyfinElevate;
