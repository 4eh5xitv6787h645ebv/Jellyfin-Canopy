...
JE.initializeReviewsScript = function () {
    ...
    if (!tmdbReviewsEnabled && !userReviewsEnabled) {
        console.log('🪼 Jellyfin Elevate: Reviews feature disabled.');
        return;
    }

    const logPrefix = '🪼 Jellyfin Elevate: Reviews:';
    ...
    function fetchReviews(tmdbId: string, mediaType: string): Promise<any[] | null> {
        ...
        const url = `${ApiClient.getUrl(`/JellyfinElevate/tmdb/${apiMediaType}/${tmdbId}/reviews`)}?language=en-US&page=1`;
     ...
     * Backed by DELETE /JellyfinElevate/reviews/admin/{userIdN}/{mediaType}/{tmdbId},
     ...
     */
    function jeConfirm(text: string, title?: string): Promise<boolean> {
     ...
     */
    function jeAlert(text: string, title?: string): void {
        const dashboard = (window as { Dashboard?: DashboardLike }).Dashboard;
    ...
    function createUserReviewElement(
        ...
        if (isOwn) {
            ...
            reviewCard.querySelector('.je-review-delete-btn')!.addEventListener('click', () => { void onDeleteCallback(review); });
        } else if (showModerationDelete) {
            reviewCard.querySelector('.je-review-admin-delete-btn')!.addEventListener('click', () => { void onDeleteCallback(review); });
        }
    ...
    function createReviewForm(
        ...
        const starBtns = form.querySelectorAll<HTMLButtonElement>('.je-star-btn');
        const clearBtn = form.querySelector<HTMLButtonElement>('.je-star-clear-btn')!;
        const starLabel = form.querySelector<HTMLElement>('.je-star-label')!;
        const textarea = form.querySelector<HTMLTextAreaElement>('.je-review-textarea')!;
        const charCount = form.querySelector<HTMLElement>('.je-review-char-count')!;
        const submitBtn = form.querySelector<HTMLButtonElement>('.je-review-submit-btn')!;
        const cancelBtn = form.querySelector<HTMLButtonElement>('.je-review-cancel-btn')!;
        const errorEl = form.querySelector<HTMLElement>('.je-review-form-error')!;
    ...
    function addReviewsToPage(reviews: any[] | null, userReviews: any[], contextPage: Element, tmdbId: string, tmdbMediaType: string, currentUser: any): void {
        ...
        // Always build the section, even with zero reviews, so users can add their own.
        {
            ...
            function openForm(existingReview: any): void {
                ...
                reviewsSection.setAttribute('open', '');
                form.querySelector<HTMLTextAreaElement>('.je-review-textarea')!.focus();
            }
            ...
            reviewsSection.addEventListener('toggle', function () {
                try {
                    if (!window.JellyfinElevate) return;
                    const JE = window.JellyfinElevate as ReviewsJE;
