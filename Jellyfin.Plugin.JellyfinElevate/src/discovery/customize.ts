...
function labelFor(spec: DiscoveryRowSpec): string {
    return spec.title || (spec.titleKey ? JE.t!(spec.titleKey) : spec.id);
}
...
export function openCustomize(mt: DiscoveryMediaType, genres: Map<number, string>, onSave: (ids: string[] | null) => void): void {
    ...
    overlay.className = 'je-discovery-customize-overlay';
    ...
    title.id = 'je-discovery-customize-title';
    title.textContent = JE.t!('discovery_customize_title');
    title.style.cssText = 'margin:0 0 4px 0;font-size:18px;font-weight:600;';
    const hint = document.createElement('div');
    hint.textContent = JE.t!('discovery_customize_hint');
    ...
    function render(): void {
        ...
        items.forEach((item, i) => {
            ...
            const up = mkArrow('▲', JE.t!('discovery_customize_move_up'), i === 0, () => { swap(i, i - 1); });
            const down = mkArrow('▼', JE.t!('discovery_customize_move_down'), i === items.length - 1, () => { swap(i, i + 1); });
    ...
    const reset = mkTextButton(JE.t!('discovery_customize_reset'), 'rgba(255,255,255,0.1)');
    const right = document.createElement('div');
    right.style.cssText = 'display:flex;gap:10px;';
    const cancel = mkTextButton(JE.t!('discovery_customize_cancel'), 'rgba(255,255,255,0.1)');
    const save = mkTextButton(JE.t!('discovery_customize_save'), 'rgba(0,164,220,0.75)');
    ...
    a11y = installModalA11y(dialog, { labelledBy: 'je-discovery-customize-title', initialFocus: save, onEscape: close });
}
