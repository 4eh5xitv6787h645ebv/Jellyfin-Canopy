...
export function injectCss(id: string, css: string): void {
    ...
    console.log(`🪼 Jellyfin Elevate: Added CSS: ${id}`);
}
...
export function removeCss(id: string): boolean {
    ...
    if (existing) {
        existing.remove();
        console.log(`🪼 Jellyfin Elevate: Removed CSS: ${id}`);
        return true;
    }
    return false;
}
...
export function toast(html: string, duration?: number): void {
    ...
    t.className = 'jellyfin-elevate-toast';
...
console.log('🪼 Jellyfin Elevate: UI kit core initialized');
