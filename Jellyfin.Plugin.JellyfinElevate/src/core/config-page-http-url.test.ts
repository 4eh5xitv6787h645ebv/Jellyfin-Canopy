...
// Configuration/config-page.js::jeIsHttpUrl, the config-page save gate that
...
// jeIsHttpUrl function (it references only the global URL and its argument), and
...
// Capture `function jeIsHttpUrl(value) { ... }` up to its 8-space-indented
...
const JE_IS_HTTP_URL_RE = /function jeIsHttpUrl\(value\)\s*\{[\s\S]*?\n {8}\}/;
...
function loadJeIsHttpUrl(): (value: unknown) => boolean {
    ...
    const match = source!.match(JE_IS_HTTP_URL_RE);
    expect(match, 'jeIsHttpUrl not found in config-page.js').toBeTruthy();
    ...
    // (never user/network input), narrowed to the jeIsHttpUrl function by the
...
describe('config-page.js jeIsHttpUrl (drift guard)', () => {
    const jeIsHttpUrl = loadJeIsHttpUrl();

    it.each(LINK_BASE_CASES.map((c) => [c.input, c.accept, c.note] as const))(
        '%s -> %s (%s)',
        (input, accept) => {
            expect(jeIsHttpUrl(input)).toBe(accept);
        },
    );
});
