 ...
 * @param userKey - Key on JE.currentSettings (e.g. 'genreTagsPosition').
 * @param pluginKey - Key on JE.pluginConfig (e.g. 'GenreTagsPosition').
...
export function resolvePosition(userKey: string, pluginKey: string, fallback: string): TagPosition {
    const pos = (JE.currentSettings?.[userKey] as string | undefined) ||
        (JE.pluginConfig?.[pluginKey] as string | undefined) ||
        fallback;
 ...
 * @param camel - e.g. 'jeGenreTagged'
 * @returns e.g. 'je-genre-tagged'
 */
...
JE.core.tagRenderer = tagRenderer;

console.log('🪼 Jellyfin Elevate: Tag renderer core initialized');
