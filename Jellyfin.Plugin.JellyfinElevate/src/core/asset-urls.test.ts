...
describe('assetUrl', () => {
    it('serves from the local asset route by default (config not loaded yet)', () => {
        ...
        expect(assetUrl('icons/seerr.svg'))
            .toBe('http://jellyfin.test/JellyfinElevate/assets/icons/seerr.svg');
    });
    ...
    it('serves from the local asset route when AssetCacheEnabled is true', () => {
        ...
        expect(assetUrl('fonts/material-symbols-rounded.woff2'))
            .toBe('http://jellyfin.test/JellyfinElevate/assets/fonts/material-symbols-rounded.woff2');
    });
    ...
    it('embedded assets stay on the local route even when the cache is disabled', () => {
        ...
        expect(assetUrl('jellyseerr/poster-fallback.svg'))
            .toBe('http://jellyfin.test/JellyfinElevate/assets/jellyseerr/poster-fallback.svg');
        expect(assetUrl('ratings/ratings.css'))
            .toBe('http://jellyfin.test/JellyfinElevate/assets/ratings/ratings.css');
    });
});

describe('family helpers', () => {
    it('flagSvgUrl builds local flag URLs and lowercases the code', () => {
        expect(flagSvgUrl('US')).toBe('http://jellyfin.test/JellyfinElevate/assets/flags/4x3/us.svg');
    });

    it('flagPngUrl builds local flag URLs and lowercases the code', () => {
        expect(flagPngUrl('De')).toBe('http://jellyfin.test/JellyfinElevate/assets/flags/w20/de.png');
    });

    it('themeCssUrl builds local theme URLs', () => {
        expect(themeCssUrl('ocean.css')).toBe('http://jellyfin.test/JellyfinElevate/assets/themes/ocean.css');
    });
