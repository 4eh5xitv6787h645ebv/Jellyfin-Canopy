...
//
// When an admin updates the JellyfinElevate DLL while browser sessions stay
...
// js/plugin.js from /JellyfinElevate/version at boot) against the server's
...
const logPrefix = '🪼 Jellyfin Elevate: Self-Update:';
...
export function notifyIfNewer(serverVersion: string | null | undefined): void {
    ...
    notified = true;
    const message = 'Jellyfin Elevate updated — refresh to load the new version';
    try {
...
async function checkNow(): Promise<void> {
    ...
    try {
        const res = await fetch(ApiClient.getUrl(`/JellyfinElevate/version?_je=${Date.now()}`), {
