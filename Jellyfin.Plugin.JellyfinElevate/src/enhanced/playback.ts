...
function getCurrentVideoItemId(): string | null {
    try {
        ...
        console.warn('🪼 Jellyfin Elevate: frame-step item id parse failed', err);
        return null;
    }
}
...
function getActiveMediaSourceId(video: HTMLVideoElement | null): string | null {
    try {
        ...
        console.warn('🪼 Jellyfin Elevate: frame-step MediaSourceId parse failed', err);
        return null;
    }
}
...
async function fetchFpsForItem(itemId: string, activeMediaSourceId: string | null): Promise<number | null> {
    ...
    try {
        ...
        console.warn('🪼 Jellyfin Elevate: frame-step fps lookup failed', err);
    }
    return null;
}
    ...
    const promise = (async () => {
        ...
        if (!isReal && itemId && !_fallbackFpsWarned.has(itemId)) {
            try {
                ...
                console.warn('🪼 Jellyfin Elevate: frame-step fallback toast failed', err);
            }
        }
        return fps;
    })();
...
JE.frameStep = async (direction: 'forward' | 'back') => {
  try {
    ...
    if (!video.paused) {
        try {
            ...
            if (r && typeof r.catch === 'function') {
                r.catch((err: unknown) => console.warn('🪼 Jellyfin Elevate: video.pause() rejected', err));
            }
        } catch (err) {
            console.warn('🪼 Jellyfin Elevate: video.pause() threw', err);
        }
    }
    ...
    console.warn('🪼 Jellyfin Elevate: frameStep failed', err);
  }
};
