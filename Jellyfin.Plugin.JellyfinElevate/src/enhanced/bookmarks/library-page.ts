...
export function isPluginPagesActive(): boolean {
  ...
  return !!sb?.querySelector('a[is="emby-linkbutton"][data-itemid="Jellyfin.Plugin.JellyfinElevate.BookmarksPage"]');
}
...
export function injectNavigation(): void {
  ...
  const pluginPageItem = sb?.querySelector<HTMLElement>(
    'a[is="emby-linkbutton"][data-itemid="Jellyfin.Plugin.JellyfinElevate.BookmarksPage"]'
  );
  if (pluginPageItem) {
  ...
  const jellyfinElevateSection = document.querySelector('.jellyfinElevateSection');
  if (jellyfinElevateSection) {
    ...
    const hiddenNav = jellyfinElevateSection.querySelector('.je-nav-hidden-content-item');
    const calendarNav = jellyfinElevateSection.querySelector('.je-nav-calendar-item');
    const insertAfter = hiddenNav || calendarNav;
    if (insertAfter && insertAfter.nextSibling) {
      jellyfinElevateSection.insertBefore(navItem, insertAfter.nextSibling);
    } else {
      jellyfinElevateSection.appendChild(navItem);
    }
...
export function setupNavigationWatcher(): void {
  ...
  onSidebarRebuild('bookmarks-nav', () => {
    if (isPluginPagesActive()) return;
    if (!document.querySelector('.je-nav-bookmarks-item') && document.querySelector('.jellyfinElevateSection')) {
      injectNavigation();
    }
  });
}
