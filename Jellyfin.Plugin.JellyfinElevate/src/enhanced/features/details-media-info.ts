...
export function displayWatchProgress(itemId: string, container: HTMLElement): void {
    ...
    const persistWatchProgressMode = (mode: string): void => {
        ...
        window.JellyfinElevate.currentSettings = window.JellyfinElevate.currentSettings || {};
        window.JellyfinElevate.currentSettings.watchProgressMode = mode;
        if (typeof window.JellyfinElevate.saveUserSettings === 'function') {
            void window.JellyfinElevate.saveUserSettings('settings.json', window.JellyfinElevate.currentSettings);
        }
    };
    ...
    const getTimeString = (ticks: number): string => {
        ...
        let result = '';
        const format = (window.JellyfinElevate?.currentSettings?.watchProgressTimeFormat || 'hours');
        if (format === 'hours') {
    ...
    const getWatchProgressValue = (watchProgress: WatchProgressEntry): HTMLDivElement => {
        ...
        const defaultMode = (window.JellyfinElevate?.currentSettings?.watchProgressMode || 'percentage');
    ...
    const performFetch = async (attempt = 1): Promise<void> => {
        ...
        try {
            const itemResult: any = await ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl(`/JellyfinElevate/watch-progress/${ApiClient.getCurrentUserId()}/${itemId}`),
                dataType: 'json'
            });
            ...
            console.error('🪼 Jellyfin Elevate: Error fetching watch progress for ID %s:', itemId, error);
...
export function displayItemSize(itemId: string, container: HTMLElement): void {
    ...
    const performFetch = async (attempt = 1): Promise<void> => {
        ...
        try {
            const itemResult: any = await ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl(`/JellyfinElevate/file-size/${ApiClient.getCurrentUserId()}/${itemId}`),
                dataType: 'json'
            });
            ...
            console.error('🪼 Jellyfin Elevate: Error fetching item size for ID %s:', itemId, error);
...
export function displayAudioLanguages(itemId: string, container: HTMLElement): void {
    ...
    const performFetch = async (attempt = 1): Promise<void> => {
        ...
        try {
            ...
            console.error('🪼 Jellyfin Elevate: Error fetching audio languages for %s:', itemId, error);
