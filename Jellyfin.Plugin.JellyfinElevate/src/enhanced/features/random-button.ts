...
async function getRandomItem(): Promise<any> {
    ...
    if (!userId) {
        console.error("🪼 Jellyfin Elevate: User not logged in.");
        return null;
    }
    ...
    try {
        ...
        console.error('🪼 Jellyfin Elevate: Error fetching random item:', error);
...
function navigateToItem(item: any): void {
    if (item && item.Id) {
        ...
        console.error('🪼 Jellyfin Elevate: Invalid item object or ID:', item);
