...
// /JellyfinElevate/locales endpoint is authoritative — no GitHub call at all.
...
describe('language control locale source', () => {
    ...
    it('never fetches api.github.com when populating the language dropdown', async () => {
        ...
        const ajax = vi.fn((opts: { url: string }) => {
            if (opts.url.includes('/JellyfinElevate/locales')) return Promise.resolve(['en', 'fr', 'de']);
        ...
        expect(ajax.mock.calls.some((c) => c[0].url.includes('/JellyfinElevate/locales'))).toBe(true);
    });
});
