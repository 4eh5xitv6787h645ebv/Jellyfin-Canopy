// INT-1 regression test: a global JE shortcut must NOT fire while any JE modal
...
// away behind the dialog. The je-modal-open body class (set by modal-a11y) now
...
describe('keyListener modal guard (INT-1)', () => {
    ...
    beforeEach(async () => {
        vi.resetModules();
        const JE = window.JellyfinElevate as unknown as Record<string, any>;
        JE.state = { activeShortcuts: { GoToHome: 'H' } };
        JE.pluginConfig = {};
        JE.t = (k: string) => k;
        ...
        JE.isVideoPage = () => false;
        await import('./events');
        keyListener = (window.JellyfinElevate as unknown as { keyListener: EventListener }).keyListener;
    ...
    it('DOES fire the shortcut when no modal is open (control)', () => {
        // No je-modal-open class → the shortcut is wired and only the gate blocks it.
        pressH();
