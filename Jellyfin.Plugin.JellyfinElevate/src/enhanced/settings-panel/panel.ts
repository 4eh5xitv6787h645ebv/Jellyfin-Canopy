...
JE.showEnhancedPanel = async () => {
    ...
    if (currentUserId) {
        try {
            ...
            const settingsResponse = await ApiClient.ajax({
                type: 'GET',
                url: ApiClient.getUrl(`/JellyfinElevate/user-settings/${currentUserId}/settings.json?_=${Date.now()}`),
                dataType: 'json'
            });
            ...
            if (settingsResponse) {
                ...
                JE.userConfig.settings = (window.JellyfinElevate as any).toCamelCase(settingsResponse);
            ...
            console.warn("🪼 Jellyfin Elevate: Could not refresh settings for panel display:", e);
        }
    }
    ...
    const panelId = 'jellyfin-elevate-panel';
    ...
    if (!JE.state!.activeShortcuts || Object.keys(JE.state!.activeShortcuts).length === 0) {
        console.warn('🪼 Jellyfin Elevate: activeShortcuts not initialized, initializing now...');
        if (typeof JE.initializeShortcuts === 'function') {
            JE.initializeShortcuts();
        }
    }
