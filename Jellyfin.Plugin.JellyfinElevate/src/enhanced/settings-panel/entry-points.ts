...
/**
 * Adds the "Jellyfin Elevate" menu button to the sidebar.
 */
JE.addPluginMenuButton = () => {
    const addMenuButton = (sidebar: HTMLElement) => {
        let jellyfinElevateSection = sidebar.querySelector<HTMLElement>('.jellyfinElevateSection');

        if (!jellyfinElevateSection) {
            jellyfinElevateSection = document.createElement('div');
            jellyfinElevateSection.className = 'jellyfinElevateSection';
            jellyfinElevateSection.innerHTML = '<h3 class="sidebarHeader">Jellyfin Elevate</h3>';

            // Insert just above Media section
            const mediaSection = sidebar.querySelector('.libraryMenuOptions');
            if (mediaSection) {
                sidebar.insertBefore(jellyfinElevateSection, mediaSection);
            } else {
                sidebar.appendChild(jellyfinElevateSection);
            }
        }

        if (!jellyfinElevateSection.querySelector('#jellyfinElevateSettingsLink')) {
            const jellyfinElevateLink = document.createElement('a');
            jellyfinElevateLink.setAttribute('is', 'emby-linkbutton');
            jellyfinElevateLink.className = 'lnkMediaFolder navMenuOption emby-button';
            jellyfinElevateLink.href = '#';
            jellyfinElevateLink.id = 'jellyfinElevateSettingsLink';
            jellyfinElevateLink.innerHTML = `
            ...
            jellyfinElevateLink.addEventListener('click', (e) => {
                e.preventDefault();
                void JE.showEnhancedPanel!();
            });

            jellyfinElevateSection.appendChild(jellyfinElevateLink);
        }
    };
    ...
    onBodyMutation('ui-menu-button', () => {
        ...
        // layout. Every other module that looks for `.jellyfinElevateSection`
        ...
        if (sidebar && !sidebar.querySelector('#jellyfinElevateSettingsLink')) {
...
/**
 * Injects the "Jellyfin Elevate" settings button into the video player OSD.
 */
JE.addOsdSettingsButton = () => {
    ...
    enhancedSettingsBtn.title = 'Jellyfin Elevate';
...
function addPrefsLinkIfOnPage(): boolean {
    ...
    if (document.getElementById('jellyfinElevateUserPrefsLink')) return true;

    // Create the link element matching Jellyfin's structure
    const enhancedLink = document.createElement('a');
    enhancedLink.id = 'jellyfinElevateUserPrefsLink';
    ...
    enhancedLink.innerHTML = `
            <div class="listItem">
                ...
                <div class="listItemBody">
                    <div class="listItemBodyText">Advanced Settings (Jellyfin Elevate)</div>
                </div>
            </div>
        `;
...
/**
 * Injects the "Jellyfin Elevate" link into the user preferences menu (mypreferencesmenu.html).
