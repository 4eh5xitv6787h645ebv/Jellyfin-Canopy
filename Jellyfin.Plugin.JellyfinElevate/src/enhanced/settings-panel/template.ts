...
function tWithFallback(key: string, fallback?: string): string {
    ...
    try {
        ...
        console.warn(`🪼 Jellyfin Elevate: JE.t('${key}') threw, using fallback:`, err);
        result = null;
    }
    if (!result || result === key) {
        if (!_tFallbackWarned.has(key)) {
            _tFallbackWarned.add(key);
            console.warn(`🪼 Jellyfin Elevate: missing translation key '${key}', using inline fallback`);
        }
        return fallback || key;
    }
    return result;
...
export function buildPanelHtml(ctx: PanelContext): string {
    ...
    return `
            <style>
                #jellyfin-elevate-panel .tabs { display: flex; border-bottom: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.2); }
                #jellyfin-elevate-panel .tab-button { font-family: inherit; flex: 1; padding: 14px; text-align: center; cursor: pointer; background: transparent; border: none; color: rgba(255,255,255,0.6); font-size: 15px; font-weight: 600; transition: all 0.2s; border-bottom: 2px solid transparent; background: ${panelBgColor}; }
                #jellyfin-elevate-panel .tab-button:hover { background: ${panelBgColor}; color: #fff; }
                #jellyfin-elevate-panel .tab-button.active { color: #fff; border-bottom-color: ${primaryAccentColor}; background: ${headerFooterBg}; }
                #jellyfin-elevate-panel .tab-content { display: none; }
                #jellyfin-elevate-panel .tab-content.active { display: block; }
            ...
            <div style="padding: 18px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); background: ${headerFooterBg};">
                <div style="font-size: 24px; font-weight: 700; margin-bottom: 8px; text-align: center; background: ${primaryAccentColor}; -webkit-background-clip: text; -webkit-text-fill-color: transparent;">${JE.icon!(JE.IconName!.JELLYFISH)} Jellyfin Elevate</div>
            ...
            <div class="panel-main-content" style="padding: 0 20px; flex: 1; overflow-y: auto; position: relative; background: ${panelBgColor};">
                ...
                <div id="settings-content" class="tab-content" style="padding-top: 20px; padding-bottom: 20px; width: 50vw;">
                    ...
                    ${JE.pluginConfig?.SpoilerBlurEnabled === true && JE.spoilerGuard ? (() => {
                        const sbPrefs = JE.spoilerGuard.getUserPrefs ? JE.spoilerGuard.getUserPrefs() : {};
                        ...
                        // literals at every call site (class (a)); JE.t is the trusted
                        ...
                        return `
                        <details style="margin-bottom: 16px; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; background: ${detailsBackground};">
                            <summary style="padding: 16px; font-weight: 600; color: ${primaryAccentColor}; cursor: pointer; user-select: none; font-family: inherit;">${JE.icon!(JE.IconName!.MASK)} ${JE.t!('panel_settings_spoiler_guard')}</summary>
                            <div style="padding: 0 16px 16px 16px;">
