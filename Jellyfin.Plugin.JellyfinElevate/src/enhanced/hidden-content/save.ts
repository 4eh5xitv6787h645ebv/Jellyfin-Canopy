    ...
    saveTimeout = window.setTimeout(() => {
        void (async () => {
            ...
            try {
                ...
                console.warn('🪼 Jellyfin Elevate: debouncedSave failed; scheduling background retry', e);
...
export async function fetchHiddenContentUsers(): Promise<HiddenContentUser[] | null> {
    try {
        const res: any = await ApiClient.ajax({
            type: 'GET',
            url: ApiClient.getUrl('/JellyfinElevate/admin/hidden-content-users'),
            dataType: 'json'
        });
        return (res && Array.isArray(res.users)) ? res.users : [];
    } catch (e: any) {
        if (e && e.status === 403) console.warn('🪼 Jellyfin Elevate: Hidden Content admin user-list denied (not an admin).');
        return null;
    }
}
...
export async function fetchUserHiddenItemsForAdmin(targetUserId: string): Promise<HiddenItem[] | null> {
    ...
    try {
        const res: any = await ApiClient.ajax({
            type: 'GET',
            url: ApiClient.getUrl(`/JellyfinElevate/admin/hidden-content/${targetUserId}`),
            dataType: 'json'
        });
        ...
        if (e && e.status === 403) console.warn('🪼 Jellyfin Elevate: Hidden Content admin read denied (not an admin).');
        return null;
    }
}
...
export async function adminUnhideForUser(targetUserId: string, keys: string[]): Promise<boolean> {
    ...
    try {
        await ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl(`/JellyfinElevate/admin/hidden-content/${targetUserId}/unhide`),
            data: JSON.stringify(keys),
            contentType: 'application/json'
        });
        return true;
    } catch (e: any) {
        if (e && e.status === 403) console.warn('🪼 Jellyfin Elevate: Hidden Content admin unhide denied (not an admin).');
        return false;
    }
}
...
export async function adminHideForUser(targetUserId: string, items: HiddenItem[]): Promise<number | boolean> {
    ...
    try {
        const res: any = await ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl(`/JellyfinElevate/admin/hidden-content/${targetUserId}/hide`),
            data: JSON.stringify(items),
            contentType: 'application/json'
        });
        return (res && typeof res.added === 'number') ? res.added : true;
    } catch (e: any) {
        if (e && e.status === 403) console.warn('🪼 Jellyfin Elevate: Hidden Content admin hide denied (not an admin / disabled).');
        return false;
    }
}
...
async function directSaveHiddenContent(): Promise<string> {
    ...
    await ApiClient.ajax({
        type: 'POST',
        url: ApiClient.getUrl(`/JellyfinElevate/user-settings/${userId}/hidden-content.json`),
        data: snapshot,
        contentType: 'application/json'
    });
    return snapshot;
...
function scheduleFlushRetry(attempt: number): void {
    if (attempt >= FLUSH_RETRY_DELAYS_MS.length) {
        console.error('🪼 Jellyfin Elevate: hidden-content save retries exhausted; local change may be lost on reload');
    ...
    pendingRetryHandle = window.setTimeout(() => {
        void (async () => {
            ...
            if (typeof ApiClient === 'undefined' || typeof ApiClient.getCurrentUserId !== 'function' || !ApiClient.getCurrentUserId()) {
                console.error('🪼 Jellyfin Elevate: abandoning hidden-content retry; ApiClient unavailable');
                pendingRetryHandle = null;
                return;
            }
            try {
                ...
                console.warn(`🪼 Jellyfin Elevate: hidden-content save retry ${attempt + 1} failed`, err);
...
export async function flushPendingSave(): Promise<void> {
    ...
    try {
        ...
        console.warn('🪼 Jellyfin Elevate: flushPendingSave failed; scheduling background retry', e);
        if (pendingRetryHandle == null) scheduleFlushRetry(0);
        throw e;
    }
}
