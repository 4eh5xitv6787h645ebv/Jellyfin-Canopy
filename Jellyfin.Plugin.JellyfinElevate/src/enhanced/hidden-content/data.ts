...
export function emitChange(): void {
    try {
        ...
        console.warn('🪼 Jellyfin Elevate: Failed to emit hidden-content-changed event', e);
    }
}
...
export async function refresh(): Promise<boolean> {
    try {
        ...
        const fresh = await ApiClient.ajax({
            type: 'GET',
            url: ApiClient.getUrl(`/JellyfinElevate/user-settings/${userId}/hidden-content.json?_=${Date.now()}`),
            dataType: 'json'
        });
        ...
        console.warn('🪼 Jellyfin Elevate: Failed to refresh hidden-content', e);
        return false;
    }
}
