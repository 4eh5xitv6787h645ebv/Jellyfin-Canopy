...
//
// Centralized helper utilities for Jellyfin Elevate.
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (attempt === maxAttempts) {
                console.error(`🪼 Jellyfin Elevate: Failed after ${maxAttempts} attempts:`, error);
                throw error;
            }

            const delay = baseDelay * Math.pow(2, attempt - 1);
            console.warn(`🪼 Jellyfin Elevate: Attempt ${attempt}/${maxAttempts} failed, retrying in ${delay}ms...`, error);
    ...
    return new Promise((resolve) => {
        ...
        const checkCondition = (): void => {
            ...
            if (Date.now() - startTime >= timeout) {
                console.warn('🪼 Jellyfin Elevate: Timeout waiting for condition');
                resolve(false);
                return;
            }
...
console.log('🪼 Jellyfin Elevate: Helpers initialized successfully');
