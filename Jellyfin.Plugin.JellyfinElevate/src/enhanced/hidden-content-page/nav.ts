...
export function injectNavigation(): void {
    ...
    const pluginPageItem = sidebar?.querySelector<HTMLElement>(
        'a[is="emby-linkbutton"][data-itemid="Jellyfin.Plugin.JellyfinElevate.HiddenContentPage"]'
    );
    ...
    if (jellyfinElevateSection) {
        ...
        const calendarNavItem = jellyfinElevateSection.querySelector('.je-nav-calendar-item');
        if (calendarNavItem && calendarNavItem.nextSibling) {
            jellyfinElevateSection.insertBefore(navItem, calendarNavItem.nextSibling);
        ...
        console.log(`${logPrefix} jellyfinElevateSection not found, will wait for it`);
    }
}
