...
const logPrefix = '🪼 Jellyfin Elevate: Hidden Content Page:';
...
async function resolveIsAdmin(): Promise<boolean> {
    ...
    const currentUser = JE.currentUser;
...
export async function maybeInitAdminFilter(): Promise<void> {
    ...
    try {
        ...
        const list = await (JE as any).hiddenContent.fetchHiddenContentUsers();
...
export async function onAdminUserChange(value: string): Promise<void> {
    ...
    const items = await (JE as any).hiddenContent.fetchUserHiddenItemsForAdmin(value);
...
export function handleUnhide(key: string): void {
    ...
    (JE as any).hiddenContent.unhideItem(key);
}
...
export function handleUnhideMany(keys: string[]): void {
    ...
    keys.forEach((k) => (JE as any).hiddenContent.unhideItem(k));
}
...
async function adminUnhide(keys: string[]): Promise<void> {
    ...
    if (!uid) return;
    const ok = await (JE as any).hiddenContent.adminUnhideForUser(uid, keys);
    if (!ok) return;
...
async function adminAddItem(targetUserId: string, result: any): Promise<boolean> {
    ...
    const added = await (JE as any).hiddenContent.adminHideForUser(targetUserId, [item]);
...
export function openAdminAddModal(): void {
    ...
    const doSearch = async (q: string): Promise<void> => {
        ...
        const libP = JE.core.api!.fetch((ApiClient as { getUrl(path: string, params?: unknown): string }).getUrl('/Items', {
        ...
        const jellyseerrAPI = (JE as any).jellyseerrAPI;
