...
// Creates <div class="jellyfinelevate hidden-content"></div>, either inside a
...
(function () {
    ...
    if (!window.JellyfinElevate?.pluginConfig?.HiddenContentEnabled) {
        return;
    }

    const useCustomTabs = !!window.JellyfinElevate?.pluginConfig?.HiddenContentUseCustomTabs;
    const useNativeTab = !!window.JellyfinElevate?.pluginConfig?.HiddenContentUseNativeTab;
    ...
    if (useNativeTab) {
        window.JellyfinElevate.nativeTabs!.register('hidden-content', 'Hidden Content', function (panel) {
            const marker = document.createElement('div');
            marker.className = 'jellyfinelevate hidden-content';
    ...
    style.textContent = [
        '.jellyfinelevate.hidden-content {',
        '  padding: 12px 3vw;',
        '}',
        '.backgroundContainer.withBackdrop:has(~ .mainAnimatedPages #indexPage .tabContent.is-active .jellyfinelevate.hidden-content) {',
    ...
    function waitForHiddenContent(callback: (JE: any) => void): void {
        ...
        const check = setInterval(function () {
            ...
            const JE = (window as any).JE || window.JellyfinElevate;
    ...
    function findActiveContainer(): HTMLElement | null {
        const all = document.querySelectorAll<HTMLElement>('.jellyfinelevate.hidden-content');
     ...
     * @param container - The active .jellyfinelevate.hidden-content element.
     * @param JE - The JellyfinElevate global object.
     */
