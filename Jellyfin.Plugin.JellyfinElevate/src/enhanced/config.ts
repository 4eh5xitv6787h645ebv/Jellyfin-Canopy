...
JE.saveUserSettings = async (fileName: string, settings: unknown): Promise<void> => {
    if (typeof ApiClient === 'undefined' || !ApiClient.getCurrentUserId) {
        console.error("🪼 Jellyfin Elevate: ApiClient not available");
        return;
    }
    try {
        const userId = ApiClient.getCurrentUserId();
        if (!userId) {
            console.error("🪼 Jellyfin Elevate: User ID not available");
            return;
        }
        ...
        if (!fileName || typeof settings === 'undefined') {
            console.error('🪼 Jellyfin Elevate: saveUserSettings called without fileName/settings', { fileName });
            return;
        }

        // Convert data back to PascalCase for server C# deserialization
        let dataToSave: unknown = settings;
        if (typeof window.JellyfinElevate?.toPascalCase === 'function') {
            if (fileName === 'bookmark.json') {
                ...
                dataToSave = window.JellyfinElevate.toPascalCase(settings, { preserveKey: (k: string) => /^bm_/i.test(k) });
            } else if (fileName === 'settings.json') {
                dataToSave = window.JellyfinElevate.toPascalCase(settings);
            }
        }
        ...
        await ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl(`/JellyfinElevate/user-settings/${userId}/${fileName}`),
            data: serialized,
        ...
        console.error(`🪼 Jellyfin Elevate: Failed to save ${fileName}:`, e);
    }
};
