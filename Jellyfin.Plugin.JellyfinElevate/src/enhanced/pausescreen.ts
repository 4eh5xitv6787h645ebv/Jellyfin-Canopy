...
JE.initializePauseScreen = function() {
  ...
  if (!JE.currentSettings?.pauseScreenEnabled) {
      console.log('🪼 Jellyfin Elevate: Custom Pause Screen is disabled.');
      return;
  }
  ...
  if (prevInstance) {
      ...
      catch (e) { console.warn('🪼 Jellyfin Elevate: pause-screen destroy failed', e); }
    ...
    class JellyfinPauseScreen {
      ...
      constructor() {
        ...
        // Pause screen delay state
        this.pauseScreenDelayMs = ((JE.currentSettings?.pauseScreenDelaySeconds as number) ?? 5) * 1000;
      ...
      init() {
        ...
        if (!credentials) {
          console.error("🪼 Jellyfin Elevate: Jellyfin credentials not found");
          return;
        }
      ...
      createOverlay() {
        ...
        const tryResume = (event: Event) => {
          if (event.target === this.overlay || event.target === this.overlayContent) {
              ...
              JE.state!.pauseScreenClickTimer = window.setTimeout(() => {
                  this.hideOverlay();
          ...
          if (this.overlay.getAttribute('aria-hidden') === 'false') {
            if (e.code === 'Space' || e.code === 'Enter') {
              ...
              if (this.currentVideo && this.currentVideo.paused) {
                  this.currentVideo.play().catch(err => console.warn("🪼 Jellyfin Elevate: Play() blocked:", err));
              }
            }
            // Keep Tab inside
            if (e.code === 'Tab') {
      ...
      attachVideoListeners(video: HTMLVideoElement): () => void {
        const handlePause = () => {
          if (video === this.currentVideo && !video.ended) {
            ...
            if (typeof (JE as any)?.resumeRatingsPolling === 'function') {
              (JE as any).resumeRatingsPolling();
            }
        ...
        const handlePlay = () => {
          if (video === this.currentVideo) {
            ...
            if (typeof (JE as any)?.pauseRatingsPolling === 'function') {
              (JE as any).pauseRatingsPolling();
            }
            this.hideOverlay();
          }
        };
      ...
      async fetchItemInfo(itemId: string) {
          ...
          try {
              ...
              console.error("🪼 Jellyfin Elevate: Error fetching item info:", err);
    ...
    JE._pauseScreenInstance = new JellyfinPauseScreen();
      console.log('🪼 Jellyfin Elevate: Custom Pause Screen initialized.');
  };
