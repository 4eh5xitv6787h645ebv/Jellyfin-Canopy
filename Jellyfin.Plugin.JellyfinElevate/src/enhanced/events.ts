...
function panelKeyListener(e: KeyboardEvent): void {
    ...
    if (document.getElementById('jellyfin-elevate-panel') || ['INPUT', 'TEXTAREA'].includes(document.activeElement!.tagName)) {
        return;
    }
    ...
    switch (combo) {
        case activeShortcuts.BookmarkCurrentTime:
            ...
            if ((JE as any).bookmarks?.showModal) {
                ...
                console.warn('🪼 Jellyfin Elevate: New bookmark system not loaded, using fallback');
            }
            break;
...
function observeActionSheets(): void {
    ...
    const runInjection = () => {
        ...
        if (typeof (JE as any).addRemoveButton === 'function') {
            ...
            console.warn('🪼 Jellyfin Elevate: addRemoveButton not available');
        }
...
function addContextMenuListener(): void {
    ...
    const captureRemoveContext = (itemElement: Element | null) => {
        ...
        if (surface) {
            console.log(`🪼 Jellyfin Elevate: ${surface} item detected for Remove action sheet:`, itemId);
        }
    };
...
/**
 * Initializes all event listeners for the core Jellyfin Elevate script.
 */
JE.initializeElevateScript = function() {
    ...
    const localClearedTimestamp = parseInt(localStorage.getItem('jellyfinElevateLastCleared') || '0', 10);
    if (serverClearTimestamp > localClearedTimestamp) {
        localStorage.removeItem('jellyfinElevateSettings');
        localStorage.setItem('jellyfinElevateLastCleared', serverClearTimestamp.toString());
    }
    ...
    if (JE.currentSettings!.longPress2xEnabled) {
        const videoPageCheck = (handler: (e: Event) => void) => (e: Event) => {
            if ((JE as any).isVideoPage()) {
                ...
                if (e.target && (e.target as Element).closest && (e.target as Element).closest('.osdControls, .pause-screen-active, .jellyfin-elevate-panel')) return;
                handler(e);
            }
        };
    ...
    document.addEventListener('visibilitychange', () => {
        ...
        if (document.hidden) {
            ...
            if (JE.currentSettings!.autoPipEnabled && !document.pictureInPictureElement) {
                video.requestPictureInPicture().catch(err => console.error("🪼 Jellyfin Elevate: Auto PiP Error:", err));
            }
        } else {
            ...
            if (JE.currentSettings!.autoPipEnabled && document.pictureInPictureElement) {
                document.exitPictureInPicture().catch(err => console.error("🪼 Jellyfin Elevate: Auto PiP Error:", err));
            }
        }
    });
};
