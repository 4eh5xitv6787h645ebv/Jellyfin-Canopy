...
function createCustomPluginLink(plugin: CustomPlugin): void {
    ...
    const existingLink = pluginsSection.querySelector(`[data-jellyfin-elevate-plugin-id="${plugin.id}"]`);
    ...
    link.setAttribute('data-jellyfin-elevate-plugin-id', plugin.id);
...
async function processPluginIcons(): Promise<void> {
    ...
    try {
        ...
        const existingTestLinks = pluginsSection.querySelectorAll('[data-jellyfin-elevate-plugin-id^="test-"]');
        ...
        const iconConfigs: IconConfig[] = [
            {
                selector: 'a[href*="Jellyfin%20Elevate"]',
                type: 'image',
                src: assetUrl('icons/jellyfish-favicon.ico'),
                alt: 'Jellyfin Elevate'
            },
            {
