...
const api = (): StreamsApi => window.JellyfinElevate as unknown as StreamsApi;

function stubCore(plugin: any): void {
    const headerContainer = document.createElement('div');
    document.body.appendChild(headerContainer);
    const JE = window.JellyfinElevate as unknown as Record<string, unknown>;
    JE.pluginConfig = { ActiveStreamsEnabled: true, ActiveStreamsAllUsers: false };
...
describe('updateCounter request ordering (_refreshSeq guard)', () => {
    ...
    it('drops a stale response that lands after a newer one', async () => {
        ...
        await flush();
        const refreshBtn = document.querySelector<HTMLButtonElement>('.je-as-refresh-btn')!;
