...
/**
 * The Seerr API surface (JE.jellyseerrAPI). Parameter/return shapes are the
...
const logPrefix = '🪼 Jellyfin Elevate: Seerr API:';
...
async function get(path: string, options: any = {}): Promise<any> {
    const url = ApiClient.getUrl(`/JellyfinElevate/jellyseerr${path}`);
...
api.fetchUserQuota = async function(options = {}) {
    if (window.JellyfinElevate?.pluginConfig?.JellyseerrShowQuotaInfo === false) {
        return null;
    }
    try {
