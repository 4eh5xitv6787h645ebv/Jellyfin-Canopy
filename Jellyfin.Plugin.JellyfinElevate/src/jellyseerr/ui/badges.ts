...
const logPrefix = '🪼 Jellyfin Elevate: Seerr UI:';
...
async function fetchProviderIcons(container: any, tmdbId: any, mediaType: any) {
    ...
    const DEFAULT_REGION = (JE.pluginConfig.DEFAULT_REGION as string) || 'US';
    const DEFAULT_PROVIDERS = JE.pluginConfig.DEFAULT_PROVIDERS ? (JE.pluginConfig.DEFAULT_PROVIDERS as string).replace(/'/g, '').replace(/\n/g, ',').split(',').map((s: any) => s.trim()).filter((s: any) => s) : [];
    const IGNORE_PROVIDERS = JE.pluginConfig.IGNORE_PROVIDERS ? (JE.pluginConfig.IGNORE_PROVIDERS as string).replace(/'/g, '').replace(/\n/g, ',').split(',').map((s: any) => s.trim()).filter((s: any) => s) : [];

    try {
        // Routed through the core API client (auth headers, retry, dedup);
        // non-OK responses throw and land in the catch below.
        const data: any = await JE.core.api!.plugin(`/tmdb/${mediaType}/${tmdbId}/watch/providers`);
