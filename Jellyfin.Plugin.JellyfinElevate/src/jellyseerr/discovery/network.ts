...
async function getStudioInfo(studioId: string, signal?: AbortSignal): Promise<any> {
    ...
    try {
        ...
        const response = await fetchWithManagedRequest(`/JellyfinElevate/studio/${studioId}`, { signal });
...
async function searchTmdbCompany(networkName: string, signal?: AbortSignal): Promise<any> {
    ...
    try {
        ...
        const response = await fetchWithManagedRequest(
            `/JellyfinElevate/tmdb/search/company?query=${encodeURIComponent(networkName)}`,
            { signal }
        );
...
const discovery = JE.discoveryBase!.createDiscovery({
    ...
    buildDiscoverPath: (kind: string, id: number) => kind === 'tv'
        ? `/JellyfinElevate/jellyseerr/discover/tv/network/${id}`
        : `/JellyfinElevate/jellyseerr/discover/movies/studio/${id}`
});
