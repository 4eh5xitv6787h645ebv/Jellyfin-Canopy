...
async function fetchTmdbGenres(signal?: AbortSignal): Promise<any> {
    ...
    try {
        ...
        const [tvResponse, movieResponse] = await Promise.all([
            fetchWithManagedRequest('/JellyfinElevate/tmdb/genres/tv', fetchOptions).catch(() => []),
            fetchWithManagedRequest('/JellyfinElevate/tmdb/genres/movie', fetchOptions).catch(() => [])
        ]);
...
async function getGenreInfo(genreId: string, signal?: AbortSignal): Promise<any> {
    ...
    try {
        ...
        const response = await fetchWithManagedRequest(`/JellyfinElevate/genre/${genreId}`, { signal });
...
const discovery = JE.discoveryBase!.createDiscovery({
    ...
    buildDiscoverPath: (kind: string, id: number) => kind === 'tv'
        ? `/JellyfinElevate/jellyseerr/discover/tv/genre/${id}`
        : `/JellyfinElevate/jellyseerr/discover/movies/genre/${id}`
});
