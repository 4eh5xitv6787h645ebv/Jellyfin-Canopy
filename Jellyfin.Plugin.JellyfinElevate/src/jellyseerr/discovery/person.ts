...
const logPrefix = '🪼 Jellyfin Elevate: Person Discovery:';
...
async function getPersonInfo(personId: string, signal?: AbortSignal): Promise<any> {
    ...
    try {
        ...
        const response = await fetchWithManagedRequest(`/JellyfinElevate/person/${personId}`, { signal });
...
async function searchTmdbPerson(personName: string, signal?: AbortSignal): Promise<any> {
    ...
    try {
        ...
        const response = await fetchWithManagedRequest(
            `/JellyfinElevate/tmdb/search/person?query=${encodeURIComponent(personName)}`,
            { signal }
        );
...
async function fetchPersonCredits(personId: number, signal?: AbortSignal): Promise<any> {
    try {
        ...
        const response = await fetchWithManagedRequest(
            `/JellyfinElevate/jellyseerr/person/${personId}/combined_credits`,
            { signal }
        );
