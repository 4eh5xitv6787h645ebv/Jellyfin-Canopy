...
JE.initializeJellyseerrScript = function() {
    ...
    if (!JE.pluginConfig.JellyseerrEnabled) {
        console.log('🪼 Jellyfin Elevate: Seerr Search: Integration is disabled in plugin settings.');
        return;
    }
    if (JE.pluginConfig.JellyseerrShowSearchResults === false) {
        console.log('🪼 Jellyfin Elevate: Seerr Search: Search results are disabled in plugin settings.');
        return;
    }

    const logPrefix = '🪼 Jellyfin Elevate: Seerr:';
        ...
        const checkForUser = async () => {
            if (ApiClient.getCurrentUserId() && ApiClient.accessToken()) {
                ...
                if (isJellyseerrActive && JE.pluginConfig?.JellyseerrShowGenreDiscovery !== false) {
                    Promise.all([
                        JE.discoveryFilter?.fetchWithManagedRequest?.('/JellyfinElevate/tmdb/genres/tv', 'genre', {})?.catch(() => {}),
                        JE.discoveryFilter?.fetchWithManagedRequest?.('/JellyfinElevate/tmdb/genres/movie', 'genre', {})?.catch(() => {})
                    ]).catch(() => {});
                }
