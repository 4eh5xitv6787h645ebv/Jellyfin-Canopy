...
/** Seerr issue-reporter surface (JE.jellyseerrIssueReporter). */
...
const logPrefix = '🪼 Jellyfin Elevate: Issue Reporter:';
...
issueReporter.checkReportingAvailability = async function (item) {
    try {
        ...
        const statusUrl = ApiClient.getUrl('/JellyfinElevate/jellyseerr/status');
        ...
        if (!tmdbId && (type === 'Season' || type === 'Episode')) {
            try {
                ...
                if (parentId && userId) {
                    const parentItem: any = JE.helpers?.getItemCached
