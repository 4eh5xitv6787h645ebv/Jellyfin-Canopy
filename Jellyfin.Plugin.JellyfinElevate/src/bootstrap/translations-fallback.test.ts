...
// strings. It must resolve against this repo (Jellyfin-Elevate) on main.
...
describe('translation GitHub fallback base (XCUT-7)', () => {
    ...
    it('points at this repo on the main branch, not upstream', () => {
        expect(base).toContain('4eh5xitv6787h645ebv/Jellyfin-Elevate');
        expect(base).toContain('/Jellyfin-Elevate/main/');
        expect(base).not.toContain('n00bcodr');
    });

    it('still resolves the plugin locales path', () => {
        expect(base).toContain('Jellyfin.Plugin.JellyfinElevate/js/locales');
    });
});
