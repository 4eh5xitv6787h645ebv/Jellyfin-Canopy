...
// It is NOT part of je.bundle.js — it must be individually fetchable so the
// loader can pull translations before the main bundle exists.
//
// Attaches JE.loadTranslations to the shared namespace. Behaviour is identical
...
(function (JE: JEGlobal) {
    ...
    const GITHUB_RAW_BASE = 'https://raw.githubusercontent.com/4eh5xitv6787h645ebv/Jellyfin-Elevate/main/Jellyfin.Plugin.JellyfinElevate/js/locales';
    ...
    async function getPluginVersion(): Promise<string> {
        ...
        try {
            const versionResponse = await fetch(ApiClient.getUrl('/JellyfinElevate/version'));
            ...
            console.warn('🪼 Jellyfin Elevate: Failed to fetch plugin version', e);
        }
            ...
            for (let i = localStorage.length - 1; i >= 0; i--) {
                ...
                if (key && (key.startsWith('JE_translation_') || key.startsWith('JE_translation_ts_'))) {
                    if (!key.includes(`_${pluginVersion}`)) {
                        ...
                        console.log(`🪼 Jellyfin Elevate: Removed old translation cache: ${key}`);
                    }
                }
            }
        } catch (e) {
            console.warn('🪼 Jellyfin Elevate: Failed to clean up old translation caches', e);
        }
    }
    ...
    async function tryLoadSingleLanguage(code: string, pluginVersion: string): Promise<LangResult> {
        ...
        if (cachedTranslations && cachedTimestamp) {
            ...
            if (age < CACHE_DURATION) {
                console.log(`🪼 Jellyfin Elevate: Using cached translations for ${code} (age: ${Math.round(age / 1000 / 60)} minutes, version: ${pluginVersion})`);
                try {
                    return { translations: JSON.parse(cachedTranslations) as Record<string, string>, usedLang: code };
                } catch (e) {
                    console.warn('🪼 Jellyfin Elevate: Failed to parse cached translations, will fetch fresh', e);
                }
            }
        }

        console.log(`🪼 Jellyfin Elevate: Loading bundled translations for ${code}...`);
        try {
            const bundledResponse = await fetch(ApiClient.getUrl(`/JellyfinElevate/locales/${code}.json`));
            if (bundledResponse.ok) {
                const translations = await bundledResponse.json() as Record<string, string>;
                try {
                    localStorage.setItem(cacheKey, JSON.stringify(translations));
                    localStorage.setItem(timestampKey, Date.now().toString());
                    console.log(`🪼 Jellyfin Elevate: Successfully loaded and cached bundled translations for ${code} (version: ${pluginVersion})`);
                } catch (e) { /* ignore */ }
                return { translations, usedLang: code };
            }
        } catch (bundledError) {
            console.warn('🪼 Jellyfin Elevate: Bundled translations failed, falling back to GitHub:', (bundledError as Error).message);
        }
        ...
        try {
            ...
            console.log(`🪼 Jellyfin Elevate: Fetching translations for ${code} from GitHub...`);
            ...
            if (githubResponse.ok) {
                ...
                try {
                    localStorage.setItem(cacheKey, JSON.stringify(translations));
                    localStorage.setItem(timestampKey, Date.now().toString());
                    console.log(`🪼 Jellyfin Elevate: Successfully fetched and cached translations for ${code} from GitHub (version: ${pluginVersion})`);
                } catch (storageError) {
                    console.warn('🪼 Jellyfin Elevate: Failed to cache translations (localStorage full?)', storageError);
                }
                return { translations, usedLang: code };
            }

            if (githubResponse.status === 404 && code !== 'en') {
                console.warn(`🪼 Jellyfin Elevate: Language ${code} not found on GitHub, falling back to English`);
            ...
            if (githubResponse.status === 403) {
                console.warn('🪼 Jellyfin Elevate: GitHub rate limit detected, using bundled fallback');
            } else if (githubResponse.status >= 500) {
                console.warn(`🪼 Jellyfin Elevate: GitHub server error (${githubResponse.status}), using bundled fallback`);
            }

            throw new Error(`GitHub fetch failed with status ${githubResponse.status}`);
        } catch (githubError) {
            console.warn('🪼 Jellyfin Elevate: GitHub fetch failed, falling back to bundled translations:', (githubError as Error).message);
        }

        console.log(`🪼 Jellyfin Elevate: Loading bundled translations for ${code}...`);
        let response = await fetch(ApiClient.getUrl(`/JellyfinElevate/locales/${code}.json`));
        ...
        console.warn(`🪼 Jellyfin Elevate: Bundled ${code} not found, falling back to bundled English`);
        response = await fetch(ApiClient.getUrl('/JellyfinElevate/locales/en.json'));
        if (response.ok) {
    ...
    JE.loadTranslations = async function (): Promise<Record<string, string>> {
        try {
            ...
            for (const code of langCodes) {
                try {
                    ...
                    console.warn(`🪼 Jellyfin Elevate: Failed to load translations for ${code}`, e);
                }
            }

            console.error('🪼 Jellyfin Elevate: Failed to load translations from any source');
            return {};
        } catch (error) {
            console.error('🪼 Jellyfin Elevate: Failed to load translations:', error);
            return {};
        }
    };
