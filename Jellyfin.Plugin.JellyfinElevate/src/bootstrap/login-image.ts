...
(function () {
    ...
    const isUserLoggedIn = (): boolean => {
        try {
            ...
            console.error('🪼 Jellyfin Elevate: Login Image - Error checking login status.', error);
            return false;
        }
    };
    ...
    const cleanImageUrl = (rawUrl: string, quality: number): string => {
        try {
            ...
            console.error('🪼 Jellyfin Elevate: Login Image - Invalid URL', e);
            return rawUrl;
        }
    };
    ...
    const displayProgressiveImage = (
        ...
        // Handle errors
        imgElement.onerror = () => {
            console.warn('🪼 Jellyfin Elevate: Login Image - Failed to load image');
            resetFormState();
        };
    ...
    const applyProgressiveLoadingToCards = (): void => {
        ...
        cardImageContainers.forEach((container, index) => {
            ...
            highQualityImg.onerror = () => {
                console.warn(`🪼 Jellyfin Elevate: Login Image - Card ${index + 1} high quality failed, keeping low quality`);
    ...
    const setupObservers = (): void => {
        ...
        if (!userNameInput || !manualLoginForm) {
            console.error('🪼 Jellyfin Elevate: Login Image - Required elements not found.');
            return;
        }
    ...
    const initializeLoginImage = (): void => {
        ...
        if (userNameInput && manualLoginForm) {
            ...
            console.log('🪼 Jellyfin Elevate: Login Image initialized');
            setupObservers();
    ...
    window.JellyfinElevate.initializeLoginImage = initializeLoginImage;
