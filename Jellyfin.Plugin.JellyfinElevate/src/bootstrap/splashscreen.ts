...
(function () {
    ...
    function installPreemptiveStyles(): void {
        try {
            ...
            console.warn('🪼 Jellyfin Elevate: Failed to install preemptive styles', error);
        }
    }
    ...
    function installPermanentBlock(): void {
        ...
        try {
            ...
            console.log('🪼 Jellyfin Elevate: Permanent splash block installed');
        } catch (error) {
            console.warn('🪼 Jellyfin Elevate: Failed to install permanent block', error);
        }
    }
    ...
    function removeMediaBarSplash(): void {
        ...
        mediaBarElements.forEach(element => {
            if (element && element.parentNode) {
                console.log('🪼 Jellyfin Elevate: Removing media-bar splash element');
                element.remove();
            }
        });
    }
                ...
                mutation.addedNodes.forEach((node) => {
                    if (node instanceof HTMLElement) {
                        if (node.id === 'page-loader' ||
                            ...
                            console.log('🪼 Jellyfin Elevate: Blocking media-bar splash attempt');
                            node.remove();
                        }
                    }
                });
    ...
    function hideSplashScreen(reason?: string): void {
        ...
        const completeRemoval = () => {
            ...
            setTimeout(() => {
                ...
                if (reason) {
                    console.log(`🪼 Jellyfin Elevate: Splash screen hidden → ${reason}`);
                }
    ...
    function createSplashScreen(): void {
        ...
        const pluginConfig = window.JellyfinElevate?.pluginConfig || {};
    ...
    function initializeSplashScreen(): void {
        ...
        if (!pluginConfig.EnableCustomSplashScreen) {
            cleanup();
            console.log('🪼 Jellyfin Elevate: Custom splash screen disabled');
            return;
        }
        ...
        console.log('🪼 Jellyfin Elevate: Splash screen initialized');
    }
    ...
    window.JellyfinElevate.initializeSplashScreen = initializeSplashScreen;
    window.JellyfinElevate.hideSplashScreen = publicHideSplashScreen;

    console.log('🪼 Jellyfin Elevate: Splash screen module loaded.');
})();
