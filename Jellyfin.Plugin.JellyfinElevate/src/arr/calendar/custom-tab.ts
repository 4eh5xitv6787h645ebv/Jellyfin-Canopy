...
//
// Calendar Custom Tab
// Creates <div class="jellyfinelevate calendar"></div>, either inside a tab
...
(function () {
    ...
    if (useNativeTab) {
        JE.nativeTabs?.register('calendar', 'Calendar', function (panel) {
            ...
            marker.className = 'jellyfinelevate calendar';
    ...
    style.textContent = [
        '.jellyfinelevate.calendar {',
        '  padding: 12px 3vw;',
        '}',
        '.backgroundContainer.withBackdrop:has(~ .mainAnimatedPages #indexPage .tabContent.is-active .jellyfinelevate.calendar) {',
    ...
    function findActiveContainer(): HTMLElement | null {
        const all = document.querySelectorAll<HTMLElement>('.jellyfinelevate.calendar');
     ...
     * @param container - The active .jellyfinelevate.calendar element.
