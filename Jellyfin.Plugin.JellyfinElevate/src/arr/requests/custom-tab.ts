...
//
// Requests Custom Tab
// Creates <div class="jellyfinelevate requests"></div>, either inside a tab
...
(function () {
    ...
    console.log('🪼 Jellyfin Elevate: [requests-custom-tab] DownloadsUseCustomTabs=' + useCustomTabs + ', DownloadsUseNativeTab=' + useNativeTab);
    ...
    if (useNativeTab) {
        JE.nativeTabs?.register('requests', 'Requests', function (panel) {
            ...
            marker.className = 'jellyfinelevate requests';
    ...
    style.textContent = [
        '.jellyfinelevate.requests {',
        '  padding: 12px 3vw;',
        '}',
        '.backgroundContainer.withBackdrop:has(~ .mainAnimatedPages #indexPage .tabContent.is-active .jellyfinelevate.requests) {',
    ...
    function findActiveContainer(): HTMLElement | null {
        const all = document.querySelectorAll<HTMLElement>('.jellyfinelevate.requests');
     ...
     * @param container - The active .jellyfinelevate.requests element.
