...
JE.initializeArrLinksScript = async function () {
    const logPrefix = '🪼 Jellyfin Elevate: Arr Links:';
    ...
    // JE.core.api is assigned by src/core/api-client.ts, which the bundle
    ...
    if (JE?.pluginConfig?.SonarrInstancesCorrupt && typeof JE.toast === 'function') {
        JE.toast('⚠ Sonarr instance configuration is corrupt. Open the Jellyfin Elevate config page to reset it.');
    ...
    if (JE?.pluginConfig?.RadarrInstancesCorrupt && typeof JE.toast === 'function') {
        JE.toast('⚠ Radarr instance configuration is corrupt. Open the Jellyfin Elevate config page to reset it.');
    ...
    try {
        ...
        const radarrInstances: ResolvedInstance[] = (JE.pluginConfig.RadarrInstances || [])
            .filter(i => i && i.Enabled !== false)
            .map(i => ({
                name: i.Name || 'Radarr',
                url: resolveArrLinkBase(i.Url, i.ExternalUrl, i.UrlMappings),
