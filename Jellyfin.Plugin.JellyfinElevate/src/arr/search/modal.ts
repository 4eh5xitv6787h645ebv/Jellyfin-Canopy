...
export function createArrModal(opts: { title: string; subtitle?: string; icon?: string }): ArrModalHandle {
    ...
    overlay.className = 'je-arr-modal-overlay';

    const dialog = document.createElement('div');
    dialog.className = 'je-arr-modal';

    const header = document.createElement('div');
    header.className = 'je-arr-modal-header';

    const titleWrap = document.createElement('div');
    titleWrap.className = 'je-arr-modal-titles';

    const titleId = `je-arr-modal-title-${Math.round(performance.now())}`;
    const titleEl = document.createElement('h2');
    titleEl.className = 'je-arr-modal-title';
    titleEl.id = titleId;
    if (opts.icon) {
        const icon = document.createElement('span');
        icon.className = `material-icons je-arr-modal-title-icon ${opts.icon}`;
    ...
    subtitleEl.className = 'je-arr-modal-subtitle';
    ...
    closeBtn.className = 'je-arr-modal-close';
    ...
    body.className = 'je-arr-modal-body';

    const footer = document.createElement('div');
    footer.className = 'je-arr-modal-footer';
    ...
    const close = (): void => {
        ...
        overlay.remove();
        for (const cb of closeCbs) { try { cb(); } catch (e) { console.warn('🪼 Jellyfin Elevate: arr modal close cb failed', e); } }
    };
