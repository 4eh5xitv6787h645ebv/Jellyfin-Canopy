...
namespace Jellyfin.Plugin.JellyfinElevate
{
    public class JellyfinElevate : BasePlugin<PluginConfiguration>, IHasWebPages
    {
        private readonly IApplicationPaths _applicationPaths;
        private readonly ILogger<JellyfinElevate> _logger;
        private const string PluginName = "Jellyfin Elevate";

        public JellyfinElevate(IApplicationPaths applicationPaths, IServerConfigurationManager serverConfigurationManager, IXmlSerializer xmlSerializer, ILogger<JellyfinElevate> logger, Logging.JellyfinElevateFileLoggerProvider fileLogProvider) : base(applicationPaths, xmlSerializer)
        {
            Instance = this;
            ...
            // empty UA as bot.
            Helpers.Jellyseerr.SeerrHttpHelper.UserAgent = $"JellyfinElevate/{Version}";
            CleanupOldScript();
        ...
        public static JellyfinElevate? Instance { get; private set; }
        ...
        private static string GetPluginDataSubdirectory(string name)
        {
            ...
            var pluginFolderName = Path.GetFileNameWithoutExtension(configPath) ?? "Jellyfin.Plugin.JellyfinElevate";
        ...
        internal string ScriptCacheKey
        {
            get
            {
                ...
                try
                {
                    var location = typeof(JellyfinElevate).Assembly.Location;
        ...
        internal string BuildScriptTag()
        {
            ...
            return $"<script plugin=\"{Name}\" version=\"{cacheKey}\" dev=\"{(devMode ? "true" : "false")}\" src=\"../JellyfinElevate/script?v={cacheKey}\" defer></script>";
        }
        ...
        private void CleanupOldScript()
        {
            try
            {
                ...
                {
                    _logger.LogInformation("Found old Jellyfin Elevate script tag in index.html. Removing it now.");
        ...
        private void CheckPluginPages(IApplicationPaths applicationPaths, IServerConfigurationManager serverConfigurationManager, int pluginPageConfigVersion)
        {
            ...
            var namespaceName = typeof(JellyfinElevate).Namespace;
            ...
            if (!calendarExists && pluginConfig.CalendarPageEnabled && pluginConfig.CalendarUsePluginPages)
            {
                pages.Add(new JsonObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinElevate/calendarPage" },
            ...
            if (!downloadsExists && pluginConfig.DownloadsPageEnabled && pluginConfig.DownloadsUsePluginPages)
            {
                pages.Add(new JsonObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinElevate/downloadsPage" },
            ...
            if (!bookmarksExists && pluginConfig.BookmarksEnabled && pluginConfig.BookmarksUsePluginPages)
            {
                pages.Add(new JsonObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinElevate/bookmarksPage" },
            ...
            if (!hiddenContentExists && pluginConfig.HiddenContentEnabled && pluginConfig.HiddenContentUsePluginPages)
            {
                pages.Add(new JsonObject
                {
                    ...
                    { "Url", $"{(supportsSubUrls ? "" : rootUrl)}/JellyfinElevate/hiddenContentPage" },
        ...
        public IEnumerable<PluginPageInfo> GetPages()
        {
            return new[]
            {
                ...
                {
                    Name = this.Name,
                    DisplayName = "Jellyfin Elevate",
                    EnableInMainMenu = true,
                    EmbeddedResourcePath = "Jellyfin.Plugin.JellyfinElevate.Configuration.configPage.html",
