...
namespace Jellyfin.Plugin.JellyfinElevate.Services
{
    ...
    public sealed class SpoilerBlurImageFilter : IAsyncActionFilter, IDisposable
    {
        ...
        // Image controller actions we care about. Jellyfin decorates the same
        // C# methods with both [HttpGet] and [HttpHead(Name="HeadItemImage")];
        ...
        // dispose Singletons at host shutdown.
            ...
            Configuration.PluginConfiguration pluginConfig)
        {
            ...
            {
                _logger.LogError($"Spoiler Guard post-processing failed for item {itemId} ({imageType}, {spoilerMode}): {ex.Message}");
