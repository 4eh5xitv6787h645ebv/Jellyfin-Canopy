...
{
    /// <summary>
    /// Injects the Jellyfin Elevate client &lt;script&gt; tag into jellyfin-web's
    ...
    public class ScriptInjectionStartupFilter : IStartupFilter
    {
        ...
        private async Task InvokeAsync(HttpContext context, Func<Task> nextMw)
        {
            ...
            try
            {
                ...
                // case where it isn't.
                var plugin = JellyfinElevate.Instance!;
                // Idempotency guard keyed on the controller endpoint, so we never
                // double-inject alongside a legacy on-disk tag.
                var alreadyInjected = html.IndexOf("/JellyfinElevate/script", StringComparison.OrdinalIgnoreCase) >= 0;
                ...
                if (!alreadyInjected && bodyClose >= 0)
                {
                    ...
                    {
                        _logger.LogInformation("Jellyfin Elevate: injected the client script via request-time middleware (IStartupFilter).");
                    }
                }
            }
            catch (Exception ex)
