...
namespace Jellyfin.Plugin.JellyfinElevate.Services
{
    ...
    /// stored under <see cref="JellyfinElevate.BrandingDirectory"/> using fixed,
                ...
                if (!string.IsNullOrWhiteSpace(brandingDir))
                {
                        ...
                        && File.Exists(filePath))
                    {
                        ...
                        if (fileInfo.Length > 0)
                        {
                            ...
                            {
                                _logger.LogInformation("Jellyfin Elevate: serving custom branding via request-time middleware (IStartupFilter).");
                            }
