...
const themer: ThemerApi = {
    ...
    detectActiveTheme() {
        ...
        for (const [themeKey, theme] of Object.entries(this.supportedThemes)) {
            ...
            if (identifierValue && identifierValue !== '' && identifierValue !== 'none') {
                console.log(`🪼 Jellyfin Canopy: Detected ${theme.name} theme`);
        ...
        // Default fallback
        console.log('🪼 Jellyfin Canopy: Using default theme (no specific theme detected)');
    ...
    registerTheme(themeKey: string, themeConfig: ThemeConfig) {
        ...
        console.log(`🪼 Jellyfin Canopy: Registered theme - ${themeConfig.name} (identifier: ${themeConfig.uniqueIdentifier})`);
    },
