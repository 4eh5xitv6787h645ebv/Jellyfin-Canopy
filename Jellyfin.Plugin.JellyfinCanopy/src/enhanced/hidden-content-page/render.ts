...
function createPageHeader(totalCount: number): HTMLElement {
    ...
    header.className = "jc-hidden-content-header";
    const title = document.createElement("h1");
    title.className = "jc-hidden-content-title";
    title.textContent = JC.t!('hidden_content_manage_title');
    const countSpan = document.createElement("span");
    countSpan.className = "jc-hidden-content-count";
...
function createToolbar(): ToolbarRefs {
    ...
    toolbar.className = "jc-hidden-content-toolbar";

    const searchInput = document.createElement("input");
    searchInput.type = "text";
    searchInput.className = "jc-hidden-content-page-search";
    ...
    scopedToggle.className = 'jc-hidden-scoped-filter' + (state.scopedOnly ? ' active' : '');
    scopedToggle.textContent = JC.t!('hidden_content_scope_filter_button');
    ...
    if (adminAllowed && state.adminIsAdmin === true && Array.isArray(state.adminUsers) && state.adminUsers.length > 0) {
        ...
        adminUserSelect.className = "jc-hidden-admin-user-filter";
        adminUserSelect.setAttribute('aria-label', JC.t!('hidden_content_admin_select_user'));

        const tv: any = (JC.themer && JC.themer.getThemeVariables) ? JC.themer.getThemeVariables() : {};
        ...
        ownOption.value = '';
        ownOption.textContent = JC.t!('hidden_content_admin_view_own');
        ...
        if (state.selectedAdminUserId) {
            ...
            adminEditToggle.className = 'jc-hidden-admin-edit-toggle' + (state.adminEditMode ? ' active' : '');
            adminEditToggle.textContent = state.adminEditMode
                ? JC.t!('hidden_content_admin_done')
                : JC.t!('hidden_content_admin_edit');
            ...
            if (state.adminEditMode) {
                ...
                adminAddBtn.className = 'jc-hidden-admin-add-btn';
                adminAddBtn.textContent = JC.t!('hidden_content_admin_add');
    ...
    unhideAllBtn.className = "jc-hidden-content-page-unhide-all";
...
function renderMoviesSection(movies: any[], scopedMovies: any[], container: HTMLElement): void {
    ...
    grid.className = 'jc-hidden-content-page-grid';
    for (const item of allMovies) {
        const card = (JC as any).hiddenContent.createItemCard(item);
        if (item.hideScope && item.hideScope !== 'global') {
            const infoDiv = card.querySelector('.jc-hidden-item-meta');
            if (infoDiv) {
                const badge = document.createElement('span');
                badge.className = 'jc-hidden-scoped-badge';
...
function renderSeriesSection(seriesRelated: any[], container: HTMLElement): void {
    ...
    for (const item of seriesRelated) {
        ...
        if (item.type === 'Series') {
            ...
            groupName = item.name || JC.t!('hidden_content_unknown_show');
            groupId = item.itemId || '';
        } else {
            key = item.seriesId || item.seriesName || item.name || 'unknown';
            groupName = item.seriesName || item.name || JC.t!('hidden_content_unknown_show');
...
export function renderPage(): void {
    ...
    if (filtered.length === 0) {
        ...
        emptyDiv.className = "jc-hidden-content-page-empty";
    ...
    toolbar.scopedToggle.addEventListener('click', () => {
        ...
        if (state.scopedOnly) {
            const expandBtns = container.querySelectorAll<HTMLElement>('.jc-hidden-group-expand');
            expandBtns.forEach((btn) => {
                if (!btn.classList.contains('expanded')) btn.click();
            });
            const expandAllBtn = container.querySelector('.jc-hidden-expand-all-btn');
            if (expandAllBtn) {
                expandAllBtn.textContent = JC.t!('hidden_content_collapse_all');
            }
        }
    });
        ...
        toolbar.unhideAllBtn.addEventListener('click', () => {
            showUnhideConfirmation(JC.t!('hidden_content_clear_confirm') || 'Unhide all items?', () => {
                if (viewingOther) {
                    ...
                    (JC as any).hiddenContent.unhideAll();
                }
            });
        });
    }
 ...
 * `.jc-hidden-item-unhide`; series group cards use `.jc-hidden-group-unhide`,
 * `.jc-hidden-group-item-unhide`, and `.jc-hidden-group-unhide-all`. Clicking any of these would
 * call JC.hiddenContent.unhideItem(), which writes to the CURRENT (admin) user's store — so they
...
function stripUnhideControls(container: HTMLElement): void {
    container.querySelectorAll(
        '.jc-hidden-item-unhide, .jc-hidden-group-unhide, .jc-hidden-group-item-unhide, .jc-hidden-group-unhide-all'
