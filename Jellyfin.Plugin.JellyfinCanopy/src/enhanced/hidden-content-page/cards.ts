...
function formatEpisodeLabel(item: any): string {
    ...
    if (item.seasonNumber != null && item.episodeNumber != null) {
        ...
        parts.push(JC.t!('hidden_content_season_label', { number: item.seasonNumber }));
    }
    if (item.name) parts.push(item.name);
    return parts.join(' – ') || item.name || JC.t!('hidden_content_unknown_show');
}
...
function createGroupPoster(group: any, tmdbId: string): HTMLElement {
    ...
    img.className = 'jc-hidden-group-poster';
...
function createGroupInfo(group: any, mainItem: any, totalItems: number, hasEpisodes: boolean, tmdbId: string): { info: HTMLElement; nameEl: HTMLElement } {
    ...
    info.className = 'jc-hidden-group-info';
    ...
    meta.className = 'jc-hidden-group-meta';
    if (totalItems === 1 && !hasEpisodes) {
        ...
        meta.textContent = JC.t!('hidden_content_1_hidden_item');
    } else {
        meta.textContent = JC.t!('hidden_content_n_hidden_items', { count: totalItems });
    }
...
function createExpandableItemsList(group: any, displayItems: any[], totalItems: number): DocumentFragment {
    ...
    expandBtn.className = 'jc-hidden-group-expand';
    const expandLabel = document.createElement('span');
    expandLabel.textContent = totalItems === 1
        ? JC.t!('hidden_content_1_hidden_item')
        : JC.t!('hidden_content_n_hidden_items', { count: totalItems });
    ...
    itemsList.className = 'jc-hidden-group-items';

    for (const item of displayItems) {
        const row = document.createElement('div');
        row.className = 'jc-hidden-group-item';

        const infoCol = document.createElement('div');
        infoCol.className = 'jc-hidden-group-item-info';
        ...
        const unhideBtn = document.createElement('button');
        unhideBtn.className = 'jc-hidden-group-item-unhide';
...
export function createGroupCard(group: any): HTMLElement {
    ...
    for (const item of seriesItems) {
        displayItems.push({ ...item, _label: JC.t!('hidden_content_entire_show') });
    }
...
export function createSection(titleKey: string, content: HTMLElement, options: { expandable?: boolean } = {}): HTMLElement {
    ...
    if (options.expandable) {
        ...
        header.className = 'jc-hidden-section-header';

        const titleEl = document.createElement('div');
        titleEl.className = 'jc-hidden-section-header-title';
        titleEl.textContent = JC.t!(titleKey);
        header.appendChild(titleEl);

        const toggleBtn = document.createElement('button');
        toggleBtn.className = 'jc-hidden-expand-all-btn';
        toggleBtn.textContent = JC.t!('hidden_content_expand_all');
        ...
        toggleBtn.addEventListener('click', () => {
            ...
            toggleBtn.textContent = allExpanded
                ...
                : JC.t!('hidden_content_expand_all');
            const cards = content.querySelectorAll('.jc-hidden-group-card');
            cards.forEach((card) => {
                const items = card.querySelector('.jc-hidden-group-items');
                const btn = card.querySelector('.jc-hidden-group-expand');
                const unhideAll = card.querySelector('.jc-hidden-group-unhide-all');
        ...
        const titleEl = document.createElement('div');
        titleEl.className = 'jc-hidden-group-section-title';
        titleEl.textContent = JC.t!(titleKey);
