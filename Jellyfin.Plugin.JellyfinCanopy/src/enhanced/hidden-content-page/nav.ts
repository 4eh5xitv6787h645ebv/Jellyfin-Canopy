...
export function createPageContainer(): HTMLElement {
    let page = document.getElementById("jc-hidden-content-page");
    if (!page) {
        page = document.createElement("div");
        page.id = "jc-hidden-content-page";
        ...
        contentPrimary.className = "content-primary jc-hidden-content-page";
...
export function showPage(): void {
    ...
    const activePage = document.querySelector(".mainAnimatedPage:not(.hide):not(#jc-hidden-content-page)");
    if (activePage) {
...
export function hidePage(): void {
    ...
    const page = document.getElementById("jc-hidden-content-page");
    if (page) {
    ...
    if (state.previousPage && !document.querySelector(".mainAnimatedPage:not(.hide):not(#jc-hidden-content-page)")) {
...
export function handleViewShow(e: Event): void {
    ...
    if (state.pageVisible && targetPage && targetPage.id !== "jc-hidden-content-page") {
        hidePage();
    }
}
...
export function handleNavClick(e: MouseEvent): void {
    ...
    if (btn && !btn.classList.contains("jc-nav-hidden-content-item")) {
        hidePage();
    }
}
...
export function injectNavigation(): void {
    ...
    if (document.querySelector(".jc-nav-hidden-content-item")) return;
    ...
    if (jellyfinCanopySection) {
        ...
        navItem.className =
            "navMenuOption lnkMediaFolder emby-button jc-nav-hidden-content-item";
        navItem.href = "#";
        ...
        textSpan.textContent = JC.t!("hidden_content_manage_title");
        ...
        const calendarNavItem = jellyfinCanopySection.querySelector('.jc-nav-calendar-item');
        if (calendarNavItem && calendarNavItem.nextSibling) {
            jellyfinCanopySection.insertBefore(navItem, calendarNavItem.nextSibling);
...
export function setupNavigationWatcher(): void {
    ...
    onSidebarRebuild('hidden-content-nav', () => {
        ...
        if (!document.querySelector('.jc-nav-hidden-content-item')) {
            const jellyfinCanopySection = document.querySelector('.jellyfinCanopySection');
            if (jellyfinCanopySection) {
