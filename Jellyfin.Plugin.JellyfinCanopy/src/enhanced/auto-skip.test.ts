...
describe('auto-skip engine', () => {
    ...
    it('never skips Recap/Preview/Commercial (no JC toggle — left to native)', async () => {
        const h = makeHarness([
...
describe('manual skipActiveSegment', () => {
    it('seeks past the segment containing the current position and reports it', async () => {
        const h = makeHarness([intro5to30]);
        h.engine.attach(h.video);
        await flush();
        h.video.currentTime = 10;

        const skipped = h.engine.skipActiveSegment();
        expect(skipped?.Id).toBe('seg-intro');
        expect(h.video.currentTime).toBe(30);
        // Manual skip must not double-fire the auto engine's toast.
        expect(h.onSkipped).not.toHaveBeenCalled();
    });

    it('skips even when the per-type toggle is OFF (deliberate keypress wins)', async () => {
        const h = makeHarness([intro5to30], { intro: false });
        h.engine.attach(h.video);
        await flush();
        h.video.currentTime = 10;

        expect(h.engine.skipActiveSegment()?.Type).toBe('Intro');
        expect(h.video.currentTime).toBe(30);
    });

    it('skips types the auto toggles never cover (Recap) — manual = any segment', async () => {
        const recap: MediaSegment = { Id: 'seg-recap', Type: 'Recap', StartTicks: sec(5), EndTicks: sec(30) };
        const h = makeHarness([recap]);
        h.engine.attach(h.video);
        await flush();
        h.video.currentTime = 10;

        expect(h.engine.skipActiveSegment()?.Type).toBe('Recap');
        expect(h.video.currentTime).toBe(30);
    });

    it('returns null outside every segment (caller falls back to the native button)', async () => {
        const h = makeHarness([intro5to30]);
        h.engine.attach(h.video);
        await flush();
    ...
    it('returns null before the segment fetch lands (no segments yet)', () => {
        const h = makeHarness([intro5to30]);
        h.engine.attach(h.video);
        // no flush: fetch still pending
        h.video.currentTime = 10;
        expect(h.engine.skipActiveSegment()).toBeNull();
        expect(h.video.currentTime).toBe(10);
    });

    it('the auto engine does not re-act on the manually skipped segment when the user seeks back into it', async () => {
        const h = makeHarness([intro5to30]);
        h.engine.attach(h.video);
        await flush();
        h.video.seekTo(1); // establish lastTime before the segment
        h.video.currentTime = 10;
        expect(h.engine.skipActiveSegment()?.Id).toBe('seg-intro');

        // Seek back INTO the segment: backward-entry + manual latch → inert.
        h.video.seekTo(12);
        expect(h.video.currentTime).toBe(12);
        expect(h.onSkipped).not.toHaveBeenCalled();
    });

    it('honours the transcode offset (element clock ≠ absolute clock)', async () => {
        const h = makeHarness([intro5to30]);
        h.engine.attach(h.video);
        // Progressive transcode restarted at 8s absolute: element 2s = absolute 10s.
        h.video.currentSrc = `/Videos/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/stream.mp4?StartTimeTicks=${sec(8)}`;
        h.video.tick();
        await flush();
        h.video.currentTime = 2;

        expect(h.engine.skipActiveSegment()?.Id).toBe('seg-intro');
        // Absolute EndTicks 30s − 8s offset = element 22s.
        expect(h.video.currentTime).toBe(22);
    });

    it('clamps the manual seek to a finite duration', async () => {
        const outroToEnd: MediaSegment = { Id: 'seg-outro', Type: 'Outro', StartTicks: sec(500), EndTicks: sec(700) };
        const h = makeHarness([outroToEnd]);
        h.engine.attach(h.video);
        await flush();
        h.video.currentTime = 550; // duration is 600

        expect(h.engine.skipActiveSegment()?.Id).toBe('seg-outro');
        expect(h.video.currentTime).toBe(600);
    });

    it('returns null when already at/past the reachable target (nothing to jump)', async () => {
        const outroToEnd: MediaSegment = { Id: 'seg-outro', Type: 'Outro', StartTicks: sec(500), EndTicks: sec(700) };
        const h = makeHarness([outroToEnd]);
        h.engine.attach(h.video);
        await flush();
        h.video.currentTime = 600; // clamped target == currentTime
