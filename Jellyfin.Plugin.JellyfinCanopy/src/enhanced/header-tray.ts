...
// enhanced/helpers.ts (which assigns JC.helpers on import) so a consumer can use
...
/**
 * Insert a JC button into the header-right tray at a deterministic position.
 *
 ...
 * leading slot — a nondeterministic order. This keeps JC tray buttons as a
...
export function insertHeaderTrayButton(container: HTMLElement, el: HTMLElement, order: number): void {
    el.dataset.jcTrayOrder = String(order);
    let ref: Element | null = null;
    for (const child of Array.from(container.children)) {
        if (child === el) continue;
        const childOrder = (child as HTMLElement).dataset?.jcTrayOrder;
        // Stop before the first native (non-JC) element, or the first JC tray
