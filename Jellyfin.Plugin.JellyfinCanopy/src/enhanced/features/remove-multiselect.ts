...
function confirmMultiRemove(targets: { name: string; surface: string }[]): Promise<boolean> {
    return new Promise((resolve) => {
        ...
        title.id = 'jc-remove-confirm-title';
        title.textContent = JC.t!('remove_confirm_title');
        ...
        targets.forEach(t => {
            ...
            from.textContent = JC.t!(REMOVE_SURFACES[t.surface].nameKey);
        ...
        cancel.type = 'button';
        cancel.textContent = JC.t!('remove_confirm_cancel');
        ...
        ok.type = 'button';
        ok.textContent = JC.t!('remove_button_text');
        ...
        const close = (result: boolean): void => {
            ...
            // release() restores focus, drops the jc-modal-open gate, and removes
        ...
        // the jc-modal-open gate that suppresses JC global shortcuts (A11Y-3).
        a11y = installModalA11y(dialog, {
            labelledBy: 'jc-remove-confirm-title',
            initialFocus: ok,
...
async function performMultiRemove(targets: { itemId: string; surface: string; card: HTMLElement }[]): Promise<number> {
    ...
    if (removed > 0) {
        ...
        exitSelectionMode();
        showNotification(JC.t!('remove_items_success'), 'success');
...
function multiSelectRemoveLabel(targets: { surface: string }[]): string {
    ...
    if (surfaces.size === 1 && REMOVE_SURFACES[targets[0].surface]) {
        return JC.t!(REMOVE_SURFACES[targets[0].surface].labelKey);
    }
    return JC.t!('remove_button_text');
}
...
JC.addMultiSelectRemoveButton = (): void => {
    ...
    if (scroller.querySelector('[data-id="jc-multiselect-remove"]')) return;
