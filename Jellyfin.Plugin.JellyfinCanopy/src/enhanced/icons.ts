// src/enhanced/icons.ts
//
// Icon name constants + emoji/Lucide/MUI icon sets, exposed as JC.icon /
// JC.IconName / JC.icons.
// (Converted from js/enhanced/icons.js — bodies semantically identical.)

import { JC } from '../globals';

console.log('🪼 Jellyfin Canopy: Icons: Module loading...');

// Poor mans enum for icon name constants
// usage: JC.IconName.PLAYBACK
//
// This helps avoid typos when requesting icons via JC.icon(name). It will
...
const IconName: Record<string, string> = Object.freeze({
    PLAYBACK: 'playback',
    SKIP: 'skip',
    SUBTITLES: 'subtitles',
    RANDOM: 'random',
...
JC.icon = function (name: string): string {
    if (!validIconNames.has(name)) console.warn(`[JC-ICONS] Unknown icon name "${name}". Use JC.IconName constants.`);
...
JC.IconName = IconName;
JC.icons = { EMOJI, LUCIDE, MUI };

console.log('🪼 Jellyfin Canopy: Icons: Module loaded successfully. JC.icon is now available.');
