...
//
// JC.showEnhancedPanel is a toggle: a second invocation while the panel is open
// removes it and returns early. That early branch removed the panel element but
// never released the modal-a11y handle, so the `jc-modal-open` body gate stayed
// ON forever — every JC keyboard shortcut stayed suppressed, focus was never
...
describe('settings panel toggle-close releases the modal-a11y handle', () => {
    ...
    it('open → toggle-close drops the jc-modal-open gate and removes the capture keydown listener', async () => {
    ...
    it('Escape dismisses the panel and releases the jc-modal-open gate', async () => {
