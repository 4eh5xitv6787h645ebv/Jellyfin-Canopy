        ...
        return escapeHtml(text)
            ...
            .replace(/^&gt;\s*\[!(WARNING|NOTE|TIP|IMPORTANT)\]\s*\r?\n((?:&gt;.*(?:\r?\n|$))+)/gm, (match, type, content) => {
                ...
                const colors: Record<string, { border: string; bg: string; icon: string }> = {
                    WARNING: { border: '#f0ad4e', bg: 'rgba(240, 173, 78, 0.1)', icon: JC.icon!(JC.IconName!.WARNING) },
                    NOTE: { border: '#00a4dc', bg: 'rgba(0, 164, 220, 0.1)', icon: JC.icon!(JC.IconName!.NOTE) },
                    TIP: { border: '#28a745', bg: 'rgba(40, 167, 69, 0.1)', icon: JC.icon!(JC.IconName!.INFO) },
                    IMPORTANT: { border: '#dc3545', bg: 'rgba(220, 53, 69, 0.1)', icon: JC.icon!(JC.IconName!.ERROR) }
                };
    ...
    notification.innerHTML = `
            <div style="padding: 20px; border-bottom: 1px solid rgba(255,255,255,0.1); flex-shrink: 0;">
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
                    <div style="width: 40px; height: 40px; background: #3e74f2bd; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px;">${JC.icon!(JC.IconName!.CLIPBOARD)}</div>
                    <div style="flex: 1;">
