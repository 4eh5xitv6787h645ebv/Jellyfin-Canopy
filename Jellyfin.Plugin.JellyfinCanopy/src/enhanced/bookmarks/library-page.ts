...
export function isPluginPagesActive(): boolean {
  ...
  return !!sb?.querySelector('a[is="emby-linkbutton"][data-itemid="Jellyfin.Plugin.JellyfinCanopy.BookmarksPage"]');
}
...
function createPageContainer(): HTMLElement {
  let page = document.getElementById('jc-bookmarks-standalone-page');
  if (!page) {
    page = document.createElement('div');
    page.id = 'jc-bookmarks-standalone-page';
    ...
    contentPrimary.className = 'content-primary jc-bookmarks-page';
...
export function showPage(): void {
  ...
  if (JC?.pluginConfig?.BookmarksUseCustomTabs) return;
  if (JC?.pluginConfig?.BookmarksUseNativeTab) return;
  ...
  const activePage = document.querySelector<HTMLElement>('.mainAnimatedPage:not(.hide):not(#jc-bookmarks-standalone-page)');
  if (activePage) {
...
function hidePage(): void {
  ...
  const page = document.getElementById('jc-bookmarks-standalone-page');
  if (page) {
  ...
  if (pageState.previousPage && !document.querySelector('.mainAnimatedPage:not(.hide):not(#jc-bookmarks-standalone-page)')) {
...
function startLocationWatcher(): void {
  ...
  pageState.locationUnsubscribe = onNavigate(() => {
...
export function handleViewShow(e: any): void {
  ...
  if (pageState.pageVisible && targetPage && targetPage.id !== 'jc-bookmarks-standalone-page') {
    hidePage();
  }
}
...
export function handleNavClick(e: Event): void {
  ...
  if (btn && !btn.classList.contains('jc-nav-bookmarks-item')) {
    hidePage();
  }
}
...
export function injectNavigation(): void {
  if (!JC?.pluginConfig?.BookmarksEnabled) return;
  if (isPluginPagesActive()) return;
  if (JC?.pluginConfig?.BookmarksUseCustomTabs) return;
  if (JC?.pluginConfig?.BookmarksUseNativeTab) return;
  ...
  const pluginPageItem = sb?.querySelector<HTMLElement>(
    'a[is="emby-linkbutton"][data-itemid="Jellyfin.Plugin.JellyfinCanopy.BookmarksPage"]'
  );
  if (pluginPageItem) {
    pluginPageItem.style.setProperty('display', 'none', 'important');
  }

  if (document.querySelector('.jc-nav-bookmarks-item')) return;

  const jellyfinCanopySection = document.querySelector('.jellyfinCanopySection');
  if (jellyfinCanopySection) {
    const navItem = document.createElement('a');
    navItem.setAttribute('is', 'emby-linkbutton');
    navItem.className = 'navMenuOption lnkMediaFolder emby-button jc-nav-bookmarks-item';
    navItem.href = '#';
    ...
    textSpan.textContent = JC.t!('bookmarks_library_title');
    ...
    const hiddenNav = jellyfinCanopySection.querySelector('.jc-nav-hidden-content-item');
    const calendarNav = jellyfinCanopySection.querySelector('.jc-nav-calendar-item');
    const insertAfter = hiddenNav || calendarNav;
    if (insertAfter && insertAfter.nextSibling) {
      jellyfinCanopySection.insertBefore(navItem, insertAfter.nextSibling);
    } else {
      jellyfinCanopySection.appendChild(navItem);
    }
...
export function setupNavigationWatcher(): void {
  ...
  onSidebarRebuild('bookmarks-nav', () => {
    if (isPluginPagesActive()) return;
    if (!document.querySelector('.jc-nav-bookmarks-item') && document.querySelector('.jellyfinCanopySection')) {
      injectNavigation();
    }
  });
}
