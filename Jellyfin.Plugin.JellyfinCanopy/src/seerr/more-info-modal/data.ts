...
try {
    ...
    const url = ApiClient.getUrl(`/JellyfinCanopy/seerr${endpoint}`);
    let response;
    const JC = window.JellyfinCanopy;
    if (JC && JC.requestManager) {
        const httpResponse = await JC.requestManager.fetchWithRetry(url, {
            method: 'GET',
            headers: {
...
/**
 * Fetch media details from Seerr API via proxy.  */
async function fetchMediaDetails(tmdbId: any, mediaType: any) {
try {
    const JC = window.JellyfinCanopy;
    if (JC && JC.seerrAPI) {
        return mediaType === 'movie'
            ? await JC.seerrAPI.fetchMovieDetails(tmdbId)
            : await JC.seerrAPI.fetchTvShowDetails(tmdbId);
    }
    ...
    const response = await ApiClient.ajax({
        type: 'GET',
        url: ApiClient.getUrl(`/JellyfinCanopy/seerr${endpoint}`),
        headers: { 'X-Jellyfin-User-Id': ApiClient.getCurrentUserId() },
        dataType: 'json'
    });
