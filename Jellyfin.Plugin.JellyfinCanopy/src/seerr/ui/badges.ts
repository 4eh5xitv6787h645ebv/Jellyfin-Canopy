...
function setStatusBadge(card: any, item: any) {
    const badge = card.querySelector('.seerr-status-badge');
...
function addMediaTypeBadge(card: any, item: any) {
    if (item.mediaType === 'movie' || item.mediaType === 'tv' || item.mediaType === 'collection') {
        ...
        if (imageContainer) {
            ...
            badge.className = 'seerr-media-badge';
            if (item.mediaType === 'movie') {
                badge.classList.add('seerr-media-badge-movie');
                badge.textContent = JC.t!('seerr_card_badge_movie');
            } else if (item.mediaType === 'tv') {
                badge.classList.add('seerr-media-badge-series');
                badge.textContent = JC.t!('seerr_card_badge_series');
            } else {
                badge.classList.add('seerr-media-badge-collection');
                badge.textContent = JC.t!('seerr_card_badge_collection');
            }
