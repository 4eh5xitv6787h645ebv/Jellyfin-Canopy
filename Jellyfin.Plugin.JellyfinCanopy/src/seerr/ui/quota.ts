...
function isQuotaError(error: any) {
    if (JC.pluginConfig?.SeerrShowQuotaInfo === false) return false;
...
function formatNextReset(resetAt: any) {
    ...
    if (deltaMs <= 0) return JC.t!('seerr_quota_reset_now');

    const minutes = Math.round(deltaMs / 60_000);
    const hours = Math.round(deltaMs / 3_600_000);
    const days = Math.round(deltaMs / 86_400_000);

    if (minutes < 60) return JC.t!('seerr_quota_reset_in_minutes', { minutes: Math.max(1, minutes) });
    if (hours < 36) return JC.t!('seerr_quota_reset_in_hours', { hours });
...
function formatQuotaLine(q: any, type: any) {
    ...
    const label = JC.t!(type === 'tv' ? 'seerr_quota_label_tv' : 'seerr_quota_label_movie');

    if (unlimited) {
        return {
            text: JC.t!('seerr_quota_unlimited', { label }),
            restricted: false,
            unlimited: true,
            resetText: ''
        };
...
function buildQuotaChip(quota: any, mediaType: any) {
    ...
    chip.className = 'seerr-quota-chip';
    if (line.restricted) chip.classList.add('seerr-quota-chip-restricted');
    else if (Number(side?.remaining) > 0 && Number(side?.remaining) <= 2) {
        chip.classList.add('seerr-quota-chip-warning');
    }
    ...
    if (line.restricted) {
        ...
        sub.className = 'seerr-quota-chip-sub';
        sub.textContent = JC.t!('seerr_quota_restricted_hint');
        chip.appendChild(sub);
    }

    if (line.resetText) {
        const reset = document.createElement('div');
        reset.className = 'seerr-quota-chip-sub';
...
async function showQuotaErrorDialog(error: any, mediaType: any) {
    const quota = await JC.seerrAPI?.fetchUserQuota?.({ skipCache: true });
    ...
    lines.push(JC.t!('seerr_quota_dialog_hint'));

    const title = JC.t!('seerr_quota_dialog_title');
...
// Exposed so seerr.js / more-info-modal.js can use the same dialog + chip.
