...
// server/constants/request.ts). All consumers reference JC.seerrStatus
// instead of scattering magic integers or display logic across the codebase.
import { JC } from '../globals';

/** Button rendering config for a display state. */
export interface SeerrButtonConfig {
    labelKey: string;
    cssClass: string;
    disabled: boolean;
    showSpinner: boolean;
 ...
 */
export interface SeerrStatusApi {
    MEDIA: Readonly<Record<'UNKNOWN' | 'PENDING' | 'PROCESSING' | 'PARTIALLY_AVAILABLE' | 'AVAILABLE' | 'BLOCKED' | 'DELETED', number>>;
    REQUEST: Readonly<Record<'PENDING' | 'APPROVED' | 'DECLINED' | 'FAILED' | 'COMPLETED', number>>;
...
declare module '../types/jc' {
    interface JEGlobal {
        /** Seerr status enums + display logic (src/seerr/seerr-status.ts). */
        seerrStatus?: SeerrStatusApi;
    }
}
...
seerrStatus.getButtonConfig = function(displayStatus) {
    switch (displayStatus) {
        case DisplayStatus.AVAILABLE:
            return { labelKey: 'seerr_btn_available',          cssClass: 'seerr-button-available',          disabled: true,  showSpinner: false, iconKey: 'available'          };
        case DisplayStatus.AVAILABLE_UPDATING:
            return { labelKey: 'seerr_btn_available',          cssClass: 'seerr-button-available-updating', disabled: true,  showSpinner: true,  iconKey: 'available'          };
        case DisplayStatus.PENDING:
            return { labelKey: 'seerr_btn_pending',            cssClass: 'seerr-button-pending',            disabled: true,  showSpinner: false, iconKey: 'pending'            };
        case DisplayStatus.PROCESSING:
            return { labelKey: 'seerr_btn_processing',         cssClass: 'seerr-button-processing',         disabled: true,  showSpinner: true,  iconKey: ''                   };
        case DisplayStatus.REQUESTED:
            return { labelKey: 'seerr_btn_requested',          cssClass: 'seerr-button-pending',            disabled: true,  showSpinner: false, iconKey: 'requested'          };
        case DisplayStatus.PARTIAL:
            return { labelKey: 'seerr_btn_partially_available',cssClass: 'seerr-button-partially-available',disabled: true,  showSpinner: false, iconKey: 'partially_available'};
        case DisplayStatus.BLOCKED:
            return { labelKey: 'seerr_btn_blocklisted',        cssClass: 'seerr-button-blocklisted',        disabled: true,  showSpinner: false, iconKey: 'cancel'             };
        default: // DELETED, NONE, unknown — requestable
            return { labelKey: 'seerr_btn_request',            cssClass: 'seerr-button-request',            disabled: false, showSpinner: false, iconKey: 'request'            };
    }
};
...
seerrStatus.getDisplayInfo = function(mediaStatus, hasActiveDownloads = false) {
    switch (mediaStatus) {
        ...
        case M.PROCESSING:
            return {
                labelKey: hasActiveDownloads ? 'seerr_season_status_processing' : 'seerr_season_status_requested',
                cssClass: 'processing'
            };
        case M.PARTIALLY_AVAILABLE:
            return { labelKey: 'seerr_season_status_partial',        cssClass: 'partially-available' };
        case M.AVAILABLE:
            return { labelKey: 'seerr_season_status_available',       cssClass: 'available' };
        case M.BLOCKED:
            return { labelKey: 'seerr_status_blocked',                cssClass: 'blocked' };
        default:
            return { labelKey: 'seerr_season_status_not_requested',   cssClass: 'not-requested' };
    }
};
...
seerrStatus.getChipConfig = function(displayStatus) {
    switch (displayStatus) {
        case DisplayStatus.AVAILABLE:
        case DisplayStatus.AVAILABLE_UPDATING:
            return { labelKey: 'seerr_btn_available',          cssClass: 'chip-available'  };
        case DisplayStatus.PARTIAL:
            return { labelKey: 'seerr_btn_partially_available',cssClass: 'chip-partial'    };
        case DisplayStatus.PROCESSING:
            return { labelKey: 'seerr_btn_processing',         cssClass: 'chip-processing' };
        case DisplayStatus.PENDING:
        case DisplayStatus.REQUESTED:
            return { labelKey: 'seerr_btn_requested',          cssClass: 'chip-requested'  };
        case DisplayStatus.BLOCKED:
            return { labelKey: 'seerr_btn_blocklisted',        cssClass: 'chip-blocklisted'};
