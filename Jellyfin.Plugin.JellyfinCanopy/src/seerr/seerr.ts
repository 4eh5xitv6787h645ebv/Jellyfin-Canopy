...
JC.initializeSeerrScript = function() {
    ...
    if (JC.pluginConfig.SeerrShowSearchResults === false) {
        console.log('🪼 Jellyfin Canopy: Seerr Search: Search results are disabled in plugin settings.');
        return;
    }
    ...
    const { checkUserStatus, search, requestMedia } = JC.seerrAPI!;
    const {
        addMainStyles, addSeasonModalStyles, updateSeerrIcon,
        renderSeerrResults, showMovieRequestModal, showSeasonSelectionModal,
     ...
     */
    function toggleSeerrOnlyMode() {
        isSeerrOnlyMode = !isSeerrOnlyMode;

        const searchPage = document.querySelector('#searchPage');
        if (!searchPage) return;

        if (isSeerrOnlyMode) {
            const allSections = searchPage.querySelectorAll('.verticalSection:not(.seerr-section)');
            hiddenSections = Array.from(allSections);
            allSections.forEach(section => section.classList.add('section-hidden'));

            const seerrSection = searchPage.querySelector('.seerr-section');
            if (seerrSection) {
                seerrOriginalPosition = document.createElement('div');
                seerrOriginalPosition.id = 'seerr-placeholder';
                seerrSection.parentNode!.insertBefore(seerrOriginalPosition, seerrSection);
                const searchResults = searchPage.querySelector('.searchResults, [class*="searchResults"], .padded-top.padded-bottom-page');
                if (searchResults) {
                    searchResults.insertBefore(seerrSection, searchResults.firstChild);
                }
            }
            ...
            JC.toast!(JC.t!('seerr_toast_filter_on'), 3000);

        } else {
            hiddenSections.forEach(section => section.classList.remove('section-hidden'));
            const seerrSection = searchPage.querySelector('.seerr-section');
            if (seerrSection && seerrOriginalPosition?.parentNode) {
                seerrOriginalPosition.parentNode.insertBefore(seerrSection, seerrOriginalPosition);
            ...
            hiddenSections = [];
            JC.toast!(JC.t!('seerr_toast_filter_off'), 3000);
        }

        const seerrSection = searchPage.querySelector('.seerr-section');
        if (seerrSection) {
            const titleElement = seerrSection.querySelector('.sectionTitle');
            if (titleElement) {
                titleElement.textContent = isSeerrOnlyMode ? JC.t!('seerr_results_title') : JC.t!('seerr_discover_title');
            }
        }
        updateSeerrIcon(isSeerrActive, seerrUserFound, isSeerrOnlyMode, toggleSeerrOnlyMode);
    }
    ...
    async function fetchAndRenderResults(query: string, options: any = {}) {
        ...
        if (results.length > 0) {
            renderSeerrResults(results, query, isSeerrOnlyMode, isSeerrActive, seerrUserFound);
    ...
    async function loadMoreSearchResults(query: string) {
        ...
        try {
            ...
            if (results.length > 0) {
                const itemsContainer = document.querySelector('.seerr-section .itemsContainer');
                if (itemsContainer) {
                    const fragment = document.createDocumentFragment();
                    results.forEach((item: any) => {
                        const card = createSeerrCard(item, isSeerrActive, seerrUserFound);
     ...
     * @param {string} query The current search query.
     */
    function setupSearchInfiniteScroll(query: string) {
        if (!JC.seamlessScroll) return;
    ...
    async function manualRefreshSeerrData(query: string | null) {
        ...
        try {
            ...
            updateSeerrResults(results, isSeerrActive, seerrUserFound);
    ...
    function initializePageObserver() {
            ...
            if (isSearchPage && currentQuery?.trim()) {
                ...
                debounceTimeout = setTimeout(() => {
                    ...
                    if (isSeerrOnlyMode) {
                        ...
                        hiddenSections = [];
                        seerrOriginalPosition = null;
                        updateSeerrIcon(isSeerrActive, seerrUserFound, false, toggleSeerrOnlyMode);
                    }
         ...
         * Idempotent — sets data-seerr-listener on the input to prevent
        ...
        function tryAttachSearchListener() {
            ...
            const iconState = `${isSeerrActive}|${seerrUserFound}|${isSeerrOnlyMode}`;
            const icon = document.getElementById('seerr-search-icon');
    ...
    function waitForUserAndInitialize() {
        ...
        const checkForUser = async () => {
            if (ApiClient.getCurrentUserId() && ApiClient.accessToken()) {
                ...
                if (isSeerrActive && JC.pluginConfig?.SeerrShowGenreDiscovery !== false) {
                    Promise.all([
    ...
    document.body.addEventListener('click', async function(event: any) {
        ...
        if (event.target.closest('.seerr-4k-popup-item')) {
            ...
            if (action === 'request4k' && tmdbId) {
                const popup = item.closest('.seerr-4k-popup');
                item.disabled = true;
                ...
                try {
                    ...
                    if (JC.pluginConfig.SeerrShowAdvanced) {
                        ...
                        JC.toast!('4K request submitted successfully!', 3000);
                        if (popup) popup.remove();
        ...
        if (mediaType === 'movie') {
            if (JC.pluginConfig.SeerrShowAdvanced) {
                ...
                try {
                    ...
                    if (JC.seerrUI?.isQuotaError?.(error)) {
                        await JC.seerrUI.showQuotaErrorDialog(error, 'movie');
                        button.innerHTML = `${JC.seerrUI.icons.request}<span>${JC.t!('seerr_btn_request')}</span>`;
                        return;
                    }
                    let errorMessage;
                    if (error.status === 404) {
                        ...
                        errorMessage = JC.t!('seerr_btn_error');
                    }
                    // Escape API error before innerHTML to prevent reflected XSS
                    button.innerHTML = `<span>${escapeHtml(errorMessage)}</span>${JC.seerrUI!.icons.error}`;
