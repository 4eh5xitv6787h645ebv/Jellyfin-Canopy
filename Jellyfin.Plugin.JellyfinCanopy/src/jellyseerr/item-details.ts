...
const logPrefix = '🪼 Jellyfin Canopy: Jellyseerr Recommendations:';
...
function waitForDetailPageReady(itemId: string, signal?: AbortSignal): Promise<any> {
    return new Promise((resolve) => {
        ...
        observerHandle = JC.helpers!.onBodyMutation!(`jellyseerr-item-details-page-detect-${++detailReadySeq}`, () => {
...
function createJellyseerrSection(results: any[], title: string): HTMLElement | null {
    ...
    const excludeLibraryItems = JC.pluginConfig?.JellyseerrExcludeLibraryItems === true;
    ...
    if (JC.hiddenContent) {
        filteredResults = JC.hiddenContent.filterJellyseerrResults(filteredResults, 'recommendations');
    }
    ...
    for (const item of filteredResults) {
        const card = JC.jellyseerrUI && JC.jellyseerrUI.createJellyseerrCard
            ? JC.jellyseerrUI.createJellyseerrCard(item, true, true)
            : null;
        if (card) {
...
async function renderSimilarAndRecommended(itemId: string) {
    ...
    try {
        ...
        const showSimilar = JC.pluginConfig?.JellyseerrShowSimilar === true;
        const showRecommended = JC.pluginConfig?.JellyseerrShowRecommended === true;
        ...
        const status = await JC.jellyseerrAPI!.checkUserStatus();
        ...
        if (showSimilar) {
            promises.push(
                type === 'movie'
                    ? JC.jellyseerrAPI!.fetchSimilarMovies(tmdbId, fetchOptions)
                    : JC.jellyseerrAPI!.fetchSimilarTvShows(tmdbId, fetchOptions)
            );
        } else {
        ...
        if (showRecommended) {
            promises.push(
                type === 'movie'
                    ? JC.jellyseerrAPI!.fetchRecommendedMovies(tmdbId, fetchOptions)
                    : JC.jellyseerrAPI!.fetchRecommendedTvShows(tmdbId, fetchOptions)
            );
        } else {
            promises.push(Promise.resolve({ results: [] }));
        }
        ...
        const excludeLibraryItems = JC.pluginConfig?.JellyseerrExcludeLibraryItems === true;
        const excludeBlocklistedItems = JC.pluginConfig?.JellyseerrExcludeBlocklistedItems === true;
        ...
        if (filteredRecommendedResults.length > 0) {
            const recommendedTitle = JC.t ? (JC.t('jellyseerr_recommended_title') || 'Recommended') : 'Recommended';
        ...
        if (filteredSimilarResults.length > 0) {
            const similarTitle = JC.t ? (JC.t('jellyseerr_similar_title') || 'Similar') : 'Similar';
...
/**
 * Waits for `JC.jellyseerrMoreInfo.checkForUnrequestedSeasons` to become
...
function waitForChecker(signal?: AbortSignal): Promise<any> {
    return pollUntil(
        () => {
            const fn = JC.jellyseerrMoreInfo && JC.jellyseerrMoreInfo.checkForUnrequestedSeasons;
...
function buildSeriesRequestMoreButton(tvDetails: any): HTMLButtonElement {
    ...
    const labelText = (JC.t && JC.t('jellyseerr_btn_request_more')) || 'Request More';
    ...
    button.addEventListener('click', (e: MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        if (JC.jellyseerrUI?.showSeasonSelectionModal) {
            JC.jellyseerrUI.showSeasonSelectionModal(
                tvDetails.id,
                'tv',
...
async function renderSeriesRequestMoreButton(itemId: string) {
    ...
    try {
        if (!JC.pluginConfig?.JellyseerrEnabled) return;
        if (JC.pluginConfig?.JellyseerrShowRequestMoreOnSeries === false) return;
        ...
        const tvDetails = await JC.jellyseerrAPI!.fetchTvShowDetails(tmdbId);
        if (signal.aborted) return;
        if (!tvDetails) return;
        ...
        if (!hasUnrequested) {
            ...
            // HTTP request to /JellyfinCanopy/jellyseerr/request, so we
...
function initialize() {
    ...
    const lifecycle = JC.core.lifecycle!.register('jellyseerr-item-details');
