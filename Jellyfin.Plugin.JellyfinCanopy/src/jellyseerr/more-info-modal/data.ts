...
try {
    ...
    const url = ApiClient.getUrl(`/JellyfinCanopy/jellyseerr${endpoint}`);
    let response;
...
try {
    const JC = window.JellyfinCanopy;
    if (JC && JC.jellyseerrAPI) {
        return mediaType === 'movie'
            ? await JC.jellyseerrAPI.fetchMovieDetails(tmdbId)
            : await JC.jellyseerrAPI.fetchTvShowDetails(tmdbId);
    }
    ...
    const response = await ApiClient.ajax({
        type: 'GET',
        url: ApiClient.getUrl(`/JellyfinCanopy/jellyseerr${endpoint}`),
        headers: { 'X-Jellyfin-User-Id': ApiClient.getCurrentUserId() },
        dataType: 'json'
    });
