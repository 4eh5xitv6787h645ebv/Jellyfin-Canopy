...
button.innerHTML = `${JC.jellyseerrUIIcons?.request || '<span class="material-icons">download</span>'}<span>${JC.t!('jellyseerr_btn_request_4k') || 'Request in 4K'}</span>`;
button.addEventListener('click', async (e: any) => {
    e.preventDefault();
    e.stopPropagation();
    if (JC.pluginConfig.JellyseerrShowAdvanced) {
        window.JellyfinCanopy?.jellyseerrUI?.showMovieRequestModal?.(data.id, data.title || data.name, data, true);
        return;
    }
    button.disabled = true;
    button.innerHTML = `<span>${JC.t!('jellyseerr_btn_requesting')}</span><span class="jellyseerr-button-spinner"></span>`;
    try {
        await JC.jellyseerrAPI!.requestMedia(data.id, 'movie', { is4k: true }, false, data);
        mountRequestedChip(data, 'movie', true);
    } catch (error: any) {
        // Quota errors get a themed dialog; restore button to idle.
        if (JC.jellyseerrUI?.isQuotaError?.(error)) {
            await JC.jellyseerrUI.showQuotaErrorDialog(error, 'movie');
            button.disabled = false;
            button.innerHTML = `${JC.jellyseerrUIIcons?.request || '<span class="material-icons">download</span>'}<span>${JC.t!('jellyseerr_btn_request_4k') || 'Request in 4K'}</span>`;
            return;
        }
        // Escape API error message before innerHTML to prevent reflected XSS
        const errorMessage = error?.responseJSON?.message || JC.t!('jellyseerr_btn_error');
        button.disabled = false;
...
if (show4kOption) {
    ...
    buttonGroup.className = 'jellyseerr-button-group jc-more-info-button-group';

    const mainButton = document.createElement('button');
    mainButton.className = 'jellyseerr-request-button jellyseerr-split-main jellyseerr-button-request';
    mainButton.innerHTML = `${JC.jellyseerrUIIcons?.request || '<span class="material-icons">download</span>'}<span>${JC.t!('jellyseerr_btn_request')}</span>`;
    ...
    mainButton.addEventListener('click', async (e: any) => {
        ...
        e.stopPropagation();
        if (JC.pluginConfig.JellyseerrShowAdvanced) {
            window.JellyfinCanopy?.jellyseerrUI?.showMovieRequestModal?.(data.id, data.title || data.name, data, false);
            return;
        }
        mainButton.disabled = true;
        mainButton.innerHTML = `<span>${JC.t!('jellyseerr_btn_requesting')}</span><span class="jellyseerr-button-spinner"></span>`;
        try {
            const response = await JC.jellyseerrAPI!.requestMedia(data.id, 'movie', {}, false, data);
            ...
            const errorMessage = error?.responseJSON?.message || JC.t!('jellyseerr_btn_error');
            mainButton.innerHTML = `<span>${escapeHtml(errorMessage)}</span>${JC.jellyseerrUIIcons?.error || ''}`;
...
const handleDocClick = (ev: any) => {
    ...
    if (moviePopupDs4k === DisplayStatus.AVAILABLE) {
        ...
        option.addEventListener('click', async (ev: any) => {
            ...
            option.disabled = true;
            option.textContent = JC.t!('jellyseerr_btn_requesting');
            try {
                const response = await JC.jellyseerrAPI!.requestMedia(data.id, 'movie', { is4k: true }, false, data);
                ...
                option.disabled = false;
                option.textContent = error?.responseJSON?.message || JC.t!('jellyseerr_btn_error');
            }
        });
    }
    ...
    requestButton.innerHTML = `${JC.jellyseerrUIIcons?.request || '<span class="material-icons">download</span>'}<span>${JC.t!('jellyseerr_btn_request')}</span>`;
    requestButton.addEventListener('click', async (e: any) => {
        ...
        requestButton.innerHTML = `<span>${JC.t!('jellyseerr_btn_requesting')}</span><span class="jellyseerr-button-spinner"></span>`;
        try {
            await JC.jellyseerrAPI!.requestMedia(data.id, 'movie', {}, false, data);
            ...
            // Escape API error before innerHTML to prevent reflected XSS
            const errorMessage = error?.responseJSON?.message || JC.t!('jellyseerr_btn_error');
            requestButton.innerHTML = `<span>${escapeHtml(errorMessage)}</span>${JC.jellyseerrUIIcons?.error || ''}`;
...
try {
    const quota = await JC.jellyseerrAPI?.fetchUserQuota?.();
    if (!actionMount.isConnected) return;
    if (actionMount.dataset.quotaRenderToken !== String(myToken)) return;

    const chip = JC.jellyseerrUI?.buildQuotaChip?.(quota, mediaType === 'tv' ? 'tv' : 'movie');
...
if (mediaType === 'movie') {
    ...
    const jellyfinMediaId = mediaInfo.jellyfinMediaId || null;
    const jellyfinMediaId4k = mediaInfo.jellyfinMediaId4k || null;
    const show4kTv = JC.jellyseerrAPI!.canRequest4k('tv');
