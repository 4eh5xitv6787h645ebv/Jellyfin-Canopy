...
if (show4kOption) {
    ...
    arrowButton.title = JC.t!('jellyseerr_btn_request_4k') || 'Request in 4K';
    ...
    mainButton.addEventListener('click', async (e: any) => {
        ...
        if (JC.jellyseerrUI?.showSeasonSelectionModal) {
            JC.jellyseerrUI.showSeasonSelectionModal(data.id, 'tv', data.title || data.name, data, false);
        }
    });
    ...
    arrowButton.addEventListener('click', (e: any) => {
        ...
        if (tvPopupDs4k === DisplayStatus.AVAILABLE) {
            option.textContent = `4K ${JC.t!('jellyseerr_btn_available') || 'Available'}`;
            ...
            option.textContent = `4K ${JC.t!('jellyseerr_btn_requested') || 'Requested'}`;
            option.disabled = true;
            ...
            option.textContent = `4K ${JC.t!('jellyseerr_btn_blocklisted') || 'Blocklisted'}`;
            option.disabled = true;
            option.classList.add('jc-4k-blocklisted');
        } else {
            option.textContent = JC.t!('jellyseerr_btn_request_4k') || 'Request in 4K';
            ...
            option.addEventListener('click', async (ev: any) => {
                ...
                close4k();
                if (JC.jellyseerrUI?.showSeasonSelectionModal) {
                    JC.jellyseerrUI.showSeasonSelectionModal(data.id, 'tv', data.title || data.name, data, true);
                }
            });
        }
...
requestButton.addEventListener('click', async (e: any) => {
    ...
    if (JC.jellyseerrUI?.showSeasonSelectionModal) {
        JC.jellyseerrUI.showSeasonSelectionModal(data.id, 'tv', data.title || data.name, data);
    }
});
...
if (show4kOption) {
    ...
    const mainButton = document.createElement('button');
    mainButton.className = 'jellyseerr-request-button jellyseerr-split-main jellyseerr-button-request';
    mainButton.innerHTML = `${JC.jellyseerrUIIcons?.request || '<span class="material-icons">download</span>'}<span>${JC.t!('jellyseerr_btn_request_more') || 'Request More'}</span>`;
    mainButton.dataset.tmdbId = data.id;
    mainButton.dataset.mediaType = 'tv';
    mainButton.addEventListener('click', async (e: any) => {
        e.preventDefault();
...
const requestButton = document.createElement('button');
requestButton.className = 'jellyseerr-request-button jellyseerr-button-request';
requestButton.innerHTML = `${JC.jellyseerrUIIcons?.request || '<span class="material-icons">download</span>'}<span>${JC.t!('jellyseerr_btn_request_more') || 'Request More'}</span>`;
requestButton.addEventListener('click', async (e: any) => {
    e.preventDefault();
    e.stopPropagation();
