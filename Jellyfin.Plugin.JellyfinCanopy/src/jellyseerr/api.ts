...
/**
 * The Seerr API surface (JC.jellyseerrAPI). Parameter/return shapes are the
...
declare module '../types/jc' {
    interface JEGlobal {
        /** Seerr API surface (src/jellyseerr/api.ts). */
...
async function get(path: string, options: any = {}): Promise<any> {
    const url = ApiClient.getUrl(`/JellyfinCanopy/jellyseerr${path}`);
...
async function post(path: string, body: unknown): Promise<any> {
    ...
    return JC.core.api!.plugin(`/jellyseerr${path}`, { method: 'POST', body, skipRetry: true });
}
...
api.canRequest4k = function(mediaType) {
    ...
    const adminEnabled = isTv
        ? !!JC.pluginConfig.JellyseerrEnable4KTvRequests
        : !!JC.pluginConfig.JellyseerrEnable4KRequests;
...
api.fetchAdvancedRequestData = async function(mediaType) {
    ...
    try {
        ...
        return { servers: [], tags: [], error: describeFetchError(error, JC.t?.('jellyseerr_err_load_server_options') || 'Failed to load server options') };
    }
};
...
api.fetchUserQuota = async function(options = {}) {
    if (window.JellyfinCanopy?.pluginConfig?.JellyseerrShowQuotaInfo === false) {
        return null;
    }
    try {
...
api.addToWatchlist = async function(tmdbId, mediaType) {
    try {
        ...
        if (!JC.pluginConfig.AddRequestedMediaToWatchlist || !JC.pluginConfig.JellyseerrEnabled) {
            console.debug(`${logPrefix} Watchlist auto-add is disabled (AddRequestedMediaToWatchlist: ${JC.pluginConfig.AddRequestedMediaToWatchlist}, JellyseerrEnabled: ${JC.pluginConfig.JellyseerrEnabled})`);
            return false;
        }
...
api.resolveJellyseerrBaseUrl = function() {
    ...
    if (JC?.pluginConfig?.JellyseerrUrlMappings) {
        ...
        const mappings = JC.pluginConfig.JellyseerrUrlMappings.toString().split('\n').map(line => line.trim()).filter(Boolean);
    ...
    if (!baseUrl && JC?.pluginConfig?.JellyseerrBaseUrl) {
        baseUrl = JC.pluginConfig.JellyseerrBaseUrl.toString().trim().replace(/\/$/, '');
    }
