 ...
 */
JC.initializeJellyseerrScript = function() {
    ...
    if (JC.pluginConfig.JellyseerrShowSearchResults === false) {
    ...
    let jellyseerrOriginalPosition: HTMLElement | null = null;
    ...
    const { checkUserStatus, search, requestMedia } = JC.jellyseerrAPI!;
    const {
    ...
    function toggleJellyseerrOnlyMode() {
        ...
        if (isJellyseerrOnlyMode) {
            ...
            JC.toast!(JC.t!('jellyseerr_toast_filter_on'), 3000);
            ...
            hiddenSections = [];
            JC.toast!(JC.t!('jellyseerr_toast_filter_off'), 3000);
        }
        ...
        if (jellyseerrSection) {
            ...
            if (titleElement) {
                titleElement.textContent = isJellyseerrOnlyMode ? JC.t!('jellyseerr_results_title') : JC.t!('jellyseerr_discover_title');
            }
        }
    ...
    async function fetchAndRenderResults(query: string, options: any = {}) {
        ...
        if (JC.hiddenContent) results = JC.hiddenContent.filterJellyseerrResults(results, 'search');
        ...
        if (results.length > 0) {
            ...
            prepareResultsWithCollections(results).then((enrichedResults: any[]) => {
                ...
                if (JC.hiddenContent) enrichedResults = JC.hiddenContent.filterJellyseerrResults(enrichedResults, 'search');
    ...
    async function prepareResultsWithCollections(rawResults: any[]) {
        ...
        try {
            results = await JC.jellyseerrAPI!.addCollections(results);
        } catch (e: any) {
    ...
    async function manualRefreshJellyseerrData(query: string | null) {
    ...
    function initializePageObserver() {
        ...
        document.addEventListener('jellyseerr-manual-refresh', function(e) {
            ...
            void manualRefreshJellyseerrData(query);
        });

        JC.helpers!.onBodyMutation!('jellyseerr-search-listener', tryAttachSearchListener);
    ...
    function waitForUserAndInitialize() {
        ...
        const checkForUser = async () => {
            if (ApiClient.getCurrentUserId() && ApiClient.accessToken()) {
                ...
                if (isJellyseerrActive && JC.pluginConfig?.JellyseerrShowGenreDiscovery !== false) {
                    Promise.all([
        ...
        if (mediaType === 'movie') {
            if (JC.pluginConfig.JellyseerrShowAdvanced) {
                ...
                try {
                    ...
                    button.innerHTML = `<span>${JC.t!('jellyseerr_btn_requested')}</span>${JC.jellyseerrUI!.icons.requested}`;
                    ...
                    if (JC.jellyseerrUI?.isQuotaError?.(error)) {
                        await JC.jellyseerrUI.showQuotaErrorDialog(error, 'movie');
                        button.innerHTML = `${JC.jellyseerrUI.icons.request}<span>${JC.t!('jellyseerr_btn_request')}</span>`;
                        return;
                    }
                    let errorMessage;
                    if (error.status === 404) {
                        ...
                        errorMessage = JC.t!('jellyseerr_btn_error');
                    }
                    // Escape API error before innerHTML to prevent reflected XSS
                    button.innerHTML = `<span>${escapeHtml(errorMessage)}</span>${JC.jellyseerrUI!.icons.error}`;
