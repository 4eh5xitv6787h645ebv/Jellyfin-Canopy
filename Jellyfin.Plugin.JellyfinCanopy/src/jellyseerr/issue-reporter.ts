// src/jellyseerr/issue-reporter.ts
import { JC } from '../globals';

/* eslint-disable @typescript-eslint/no-explicit-any -- legacy Seerr payload shapes; typed incrementally */

/** Seerr issue-reporter surface (JC.jellyseerrIssueReporter). */
export interface JellyseerrIssueReporterApi {
...
declare module '../types/jc' {
    interface JEGlobal {
        /** Seerr issue reporter (src/jellyseerr/issue-reporter.ts). */
        jellyseerrIssueReporter?: JellyseerrIssueReporterApi;
    }
}

const logPrefix = '🪼 Jellyfin Canopy: Issue Reporter:';
const issueReporter = {} as JellyseerrIssueReporterApi;
 ...
 */
const getIssueTypes = () => [
    { value: '1', label: JC.t!('jellyseerr_report_issue_type_video'), icon: JC.icon!(JC.IconName!.VIDEO) },
    { value: '2', label: JC.t!('jellyseerr_report_issue_type_audio'), icon: JC.icon!(JC.IconName!.AUDIO) },
    { value: '3', label: JC.t!('jellyseerr_report_issue_type_subtitles'), icon: JC.icon!(JC.IconName!.SUBTITLES) },
    { value: '4', label: JC.t!('jellyseerr_report_issue_type_other'), icon: JC.icon!(JC.IconName!.QUESTION) }
];
...
issueReporter.checkReportingAvailability = async function (item) {
    try {
        ...
        const statusUrl = ApiClient.getUrl('/JellyfinCanopy/jellyseerr/status');
...
issueReporter.showReportModal = function (tmdbId, itemName, mediaType, backdropUrl = null, item: any = null) {
    ...
    const formHtml = `
        ...
        <div class="jellyseerr-issue-form">
            <div class="jellyseerr-form-group">
                <label>${JC.t!('jellyseerr_report_issue_type_label')}</label>
            ...
            <div class="jellyseerr-form-group">
                <label for="issue-message">${JC.t!('jellyseerr_report_issue_message')}</label>
                <textarea
                    id="issue-message"
                    class="jellyseerr-issue-textarea"
                    placeholder="${JC.t!('jellyseerr_report_issue_message_placeholder')}"
                    rows="4"
                ></textarea>
            </div>
            <div id="jellyseerr-tv-controls-placeholder"></div>
            <div class="jellyseerr-issues-container" id="jellyseerr-issues-container">
                <div class="jellyseerr-issues-header">${JC.t!('jellyseerr_existing_issues')}</div>
                <div class="jellyseerr-issues-body" id="jellyseerr-issues-body">
                    <div class="jellyseerr-issues-loading" id="jellyseerr-issues-loading">${JC.t!('jellyseerr_loading_issues')}</div>
                </div>
            </div>
        </div>
    `;
    ...
    const { modalElement, show } = JC.jellyseerrModal!.create({
        title: JC.t!('jellyseerr_report_issue_title'),
        subtitle: itemName,
        bodyHtml: formHtml,
        backdropUrl: backdropUrl,
        buttonText: JC.t!('jellyseerr_report_issue_submit'),
        onSave: async (modalEl, button, closeModal) => {
            ...
            try {
                ...
                button.textContent = JC.t!('jellyseerr_report_issue_submitting');

                // Pass only the contents of the description box to the API
                const result = await JC.jellyseerrAPI!.reportIssue(tmdbId, mediaType, issueType, message, problemSeason, problemEpisode);

                if (result) {
                    JC.toast!(JC.t!('jellyseerr_report_issue_success'), 3000);
                ...
                if (error?.status === 403) {
                    JC.toast!(JC.t!('jellyseerr_err_no_issue_permission'), 4000);
                    ...
                    JC.toast!('Jellyseerr is not available', 4000);
                } else {
                    JC.toast!(JC.t!('jellyseerr_report_issue_error'), 4000);
                }
                button.disabled = false;
                button.textContent = JC.t!('jellyseerr_report_issue_submit');
            }
        }
    });
    ...
    void (async () => {
        ...
        const renderEmpty = (msg = JC.t!('jellyseerr_no_issues_yet')) => {
        ...
        const statusLabels: Record<string | number, string> = {
            1: JC.t!('jellyseerr_issue_open') || 'Open',
            2: JC.t!('jellyseerr_issue_resolved') || 'Resolved'
        };
        ...
        try {
            if (loadingEl) loadingEl.textContent = JC.t!('jellyseerr_loading_issues');
            const res = await JC.jellyseerrAPI!.fetchIssuesForMedia(tmdbId, mediaType, { take: 50, filter: 'all' });
            ...
            const enriched = await Promise.all(issues.map(async (issue: any) => {
                try {
                    const full = await JC.jellyseerrAPI!.fetchIssueById(issue.id);
                    return full || issue;
            ...
            renderEmpty(JC.t!('jellyseerr_load_issues_error'));
        }
    })();
    ...
    if (mediaType === 'tv') {
        void (async () => {
            try {
                ...
                const controlsHtml = `
                    <div class="jellyseerr-form-group">
                        <label for="issue-season">${JC.t!('jellyseerr_report_issue_season')}</label>
                        <select id="issue-season" class="jellyseerr-select"></select>
                    </div>
                    <div class="jellyseerr-form-group">
                        <label for="issue-episode">${JC.t!('jellyseerr_report_issue_episode')}</label>
                ...
                if (!normalized || normalized.length === 0) {
                    setOptions(seasonSelect, [{ value: 0, label: JC.t!('jellyseerr_select_all_seasons') || 'All seasons' }]);
                    seasonSelect.disabled = true;
                    setOptions(episodeSelect, [{ value: 0, label: JC.t!('jellyseerr_select_all_seasons') || 'All episodes' }]);
                    episodeSelect.disabled = true;
                    return;
                }
                ...
                if (normalized.length > 1) {
                    seasonOptions.push({ value: 0, label: JC.t!('jellyseerr_select_all_seasons') || 'All seasons' });
                }
                for (const s of normalized) {
                    seasonOptions.push({ value: s.seasonNumber, label: `${JC.t!('jellyseerr_report_issue_season') || 'Season'} ${s.seasonNumber}` });
                }
                ...
                const populateEpisodesForSeason = (seasonNum: any) => {
                    ...
                    const epOptions = [{ value: 0, label: JC.t!('jellyseerr_select_all_seasons') || 'All episodes' }];
                    if (eps.length > 0) {
                        for (const ep of eps) epOptions.push({ value: ep.episodeNumber, label: `${JC.t!('jellyseerr_report_issue_episode') || 'Episode'} ${ep.episodeNumber}${ep.title ? ' — ' + ep.title : ''}` });
                    }
...
issueReporter.createReportButton = function (container, tmdbId, itemName, mediaType, backdropUrl = null, item: any = null) {
    ...
    button.type = 'button';
    button.setAttribute('aria-label', JC.t!('jellyseerr_report_issue_button'));
    button.title = JC.t!('jellyseerr_report_issue_button');
    button.innerHTML = `
...
issueReporter.createUnavailableButton = function (container, itemName, mediaType, reason = 'unavailable') {
    ...
    let ariaLabel = JC.t!('jellyseerr_report_unavailable_button');
    let title = JC.t!('jellyseerr_report_unavailable_button');
    ...
    button.addEventListener('click', (e: MouseEvent) => {
        ...
        if (reason === 'no-tmdb') {
            ...
            JC.toast!('TMDB ID not found and Jellyseerr is not available', 4000);
        } else if (reason === 'no-permissions') {
            JC.toast!('You do not have permissions to report issues', 4000);
        } else {
            JC.toast!(JC.t!('jellyseerr_report_unavailable_toast'), 4000);
        }
    });
...
issueReporter.getTmdbIdFallback = async function (itemName, mediaType, item) {
    try {
        ...
        try {
            if (JC && JC.jellyseerrAPI && typeof JC.jellyseerrAPI.search === 'function') {
                ...
                if (imdbId) {
                    ...
                    const res = await JC.jellyseerrAPI.search(imdbId);
                ...
                const res2 = await JC.jellyseerrAPI.search(titleQuery);
...
issueReporter.applyIssueIndicator = async function (button, tmdbId, mediaType, prefetched = null) {
    if (!JC.pluginConfig?.JellyseerrShowIssueIndicator) return;
    try {
        const result = prefetched
            ? await prefetched
            : await JC.jellyseerrAPI!.fetchIssuesForMedia(tmdbId, mediaType, { take: 50, filter: 'open' });
        ...
        const issuesLabel = JC.t!('jellyseerr_existing_issues') || 'Issues';
        const openLabel = JC.t!('jellyseerr_issue_open') || 'Open';
        const reportLabel = JC.t!('jellyseerr_report_issue_button') || 'Report issue';
...
issueReporter.tryAddButton = async function () {
    ...
    if (!JC.pluginConfig?.JellyseerrEnabled || !JC.pluginConfig?.JellyseerrShowReportButton) {
    ...
    try {
        ...
        const prefetchedIssues = JC.pluginConfig?.JellyseerrShowIssueIndicator
            ? JC.jellyseerrAPI!.fetchIssuesForMedia(tmdbId, mediaType, { take: 50, filter: 'open' }).catch(() => null)
            : null;
...
JC.jellyseerrIssueReporter = issueReporter;
