...
/** Generic Seerr request modal factory (JC.jellyseerrModal). */
...
const logPrefix = '🪼 Jellyfin Canopy: Jellyseerr Modal:';
...
modal.create = function({ title, subtitle, bodyHtml, backdropPath, backdropUrl, onSave, onClose, buttonText }) {
    ...
    cancelBtn.setAttribute('aria-label', JC.t!('jellyseerr_modal_cancel'));
    cancelBtn.textContent = JC.t!('jellyseerr_modal_cancel');

    const primaryBtn = document.createElement('button');
    primaryBtn.className = 'jellyseerr-modal-button jellyseerr-modal-button-primary';
    primaryBtn.setAttribute('aria-label', buttonText || JC.t!('jellyseerr_modal_request'));
    primaryBtn.textContent = buttonText || JC.t!('jellyseerr_modal_request');
...
modal.createAdvancedOptionsHTML = function(idPrefix) {
    return `
        <div class="jellyseerr-advanced-options">
            <h3>${JC.t!('jellyseerr_advanced_options')}</h3>
            <div class="jellyseerr-form-row">
                <div class="jellyseerr-form-group">
                    <label for="${idPrefix}-server">${JC.t!('jellyseerr_server_select')}</label>
                    <select is="emby-select" id="${idPrefix}-server" class="emby-select"></select>
                </div>
                <div class="jellyseerr-form-group">
                    <label for="${idPrefix}-quality">${JC.t!('jellyseerr_quality_select')}</label>
                    <select is="emby-select" id="${idPrefix}-quality" class="emby-select"></select>
                </div>
            </div>
            <div class="jellyseerr-form-row">
                <div class="jellyseerr-form-group">
                    <label for="${idPrefix}-folder">${JC.t!('jellyseerr_folder_select')}</label>
...
modal.populateAdvancedOptions = function(modalElement, data, idPrefix) {
    ...
    if (data && data.error) {
        ...
        if (container) {
            container.innerHTML = `<h3>${JC.t!('jellyseerr_advanced_options')}</h3><div class="jellyseerr-advanced-error">${JC.escapeHtml(data.error)}</div>`;
        }
        return;
    }
