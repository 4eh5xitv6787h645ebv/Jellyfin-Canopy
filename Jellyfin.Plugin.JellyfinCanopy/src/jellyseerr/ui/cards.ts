...
function createJellyseerrCard(item: any, isJellyseerrActive: any, jellyseerrUserFound: any) {
    ...
    const base = JC.jellyseerrAPI?.resolveJellyseerrBaseUrl() || '';
    const jellyseerrUrl = base ? `${base}/${item.mediaType}/${item.id}` : null;
    const useMoreInfoModal = !!(JC.pluginConfig && JC.pluginConfig.JellyseerrUseMoreInfoModal);
        ...
        <div class="cardBox cardBox-bottompadded">
            ...
            <div class="cardText cardTextCentered cardText-first">
                <a ${titleLinkIsAttribute}
                   ...
                   title="${jellyfinHref ? titleText : (useMoreInfoModal ? titleText : (jellyseerrUrl ? (JC.t!('jellyseerr_card_view_on_jellyseerr') || 'View on Jellyseerr') : titleText))}"><bdi>${titleText}</bdi></a>
            </div>
    ...
    if (imageContainer && cardScalable) {
        ...
        const createOverview = () => {
            ...
            const contentHtml = navigatesExternally
                ? `<a is="emby-linkbutton" href="${escapeHtml(jellyseerrUrl)}" target="_blank" rel="noopener noreferrer" class="content jellyseerr-overview-link" style="text-decoration:none;color:inherit;">${escapeHtml((item.overview || JC.t!('jellyseerr_card_no_info')).slice(0, 500))}</a>`
                : `<div class="content">${escapeHtml((item.overview || JC.t!('jellyseerr_card_no_info')).slice(0, 500))}</div>`;
            overview.innerHTML = `
                ${contentHtml}
            ...
            overview.addEventListener('click', (e: any) => {
                ...
                } else if (useMoreInfoModal && JC.jellyseerrMoreInfo) {
    ...
    if (posterImage && useMoreInfoModal && JC.jellyseerrMoreInfo) {
    ...
    if (moreInfoLink) {
        moreInfoLink.addEventListener('click', (e: any) => {
            ...
            if (useMoreInfoModal && JC.jellyseerrMoreInfo) {
                e.preventDefault();
                e.stopPropagation();
    ...
    if (JC.pluginConfig.ShowElsewhereOnJellyseerr && JC.pluginConfig.TmdbEnabled && item.mediaType !== 'collection') {
    ...
    if (JC.hiddenContent && JC.hiddenContent.getSettings().enabled && JC.hiddenContent.getSettings().showHideButtons !== false && JC.hiddenContent.getSettings().showButtonJellyseerr !== false) {
