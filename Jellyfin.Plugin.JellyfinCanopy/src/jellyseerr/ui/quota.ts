...
function isQuotaError(error: any) {
    if (JC.pluginConfig?.JellyseerrShowQuotaInfo === false) return false;
...
function formatNextReset(resetAt: any) {
    ...
    if (deltaMs <= 0) return JC.t!('jellyseerr_quota_reset_now');

    const minutes = Math.round(deltaMs / 60_000);
    const hours = Math.round(deltaMs / 3_600_000);
    const days = Math.round(deltaMs / 86_400_000);

    if (minutes < 60) return JC.t!('jellyseerr_quota_reset_in_minutes', { minutes: Math.max(1, minutes) });
    if (hours < 36) return JC.t!('jellyseerr_quota_reset_in_hours', { hours });
    return JC.t!('jellyseerr_quota_reset_in_days', { days });
}
...
function formatQuotaLine(q: any, type: any) {
    ...
    const label = JC.t!(type === 'tv' ? 'jellyseerr_quota_label_tv' : 'jellyseerr_quota_label_movie');

    if (unlimited) {
        return {
            text: JC.t!('jellyseerr_quota_unlimited', { label }),
            restricted: false,
            unlimited: true,
            resetText: ''
        };
    ...
    const usagePart = days > 0
        ? JC.t!('jellyseerr_quota_usage_window', { label, used, limit, days })
        : JC.t!('jellyseerr_quota_usage', { label, used, limit });
...
function buildQuotaChip(quota: any, mediaType: any) {
    ...
    if (line.restricted) {
        ...
        sub.textContent = JC.t!('jellyseerr_quota_restricted_hint');
...
async function showQuotaErrorDialog(error: any, mediaType: any) {
    const quota = await JC.jellyseerrAPI?.fetchUserQuota?.({ skipCache: true });
    ...
    lines.push(JC.t!('jellyseerr_quota_dialog_hint'));

    const title = JC.t!('jellyseerr_quota_dialog_title');
...
async function handleRequestError(error: any, mediaType: any, requestBtn: any, resetLabel: any) {
    if (isQuotaError(error)) {
        ...
        JC.toast!(escapeHtml(describeFetchError(error, JC.t!('jellyseerr_modal_toast_request_fail'))), 4000);
    }
    if (requestBtn) {
