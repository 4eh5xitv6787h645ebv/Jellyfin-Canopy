        ...
        if (isQueued || downloadStatus.size <= 0) {
            ...
            popoverHTML += `
                <div class="jellyseerr-popover-item">
                    <div class="title">${escapeHtml(downloadStatus.title) || JC.t!('jellyseerr_popover_downloading')}</div>
...
function show4KPopup(buttonGroup: any, item: any) {
    ...
    if (popupDs4k === DisplayStatus.AVAILABLE) {
        ...
        request4KBtn.innerHTML = `<span>${JC.t!('jellyseerr_btn_blocklisted')}</span>${icons.cancel}`;
        request4KBtn.disabled = true;
        request4KBtn.classList.add('chip-blocklisted');
    } else {
        request4KBtn.innerHTML = `<span>${JC.t!('jellyseerr_btn_request_4k')}</span>`;
