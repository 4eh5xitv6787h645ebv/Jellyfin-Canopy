 ...
 */
function configureRequestButton(button: any, item: any, isJellyseerrActive: any, jellyseerrUserFound: any) {
    if (!isJellyseerrActive) {
        button.innerHTML = `<span>${JC.t!('jellyseerr_btn_offline')}</span>${icons.cloud_off}`;
        button.disabled = true;
        button.classList.add('jellyseerr-button-offline');
        return;
    }
    if (!jellyseerrUserFound) {
        button.innerHTML = `<span>${JC.t!('jellyseerr_btn_user_not_found')}</span>${icons.person_off}`;
        button.disabled = true;
...
function configureCollectionButton(button: any, item: any) {
    ...
    button.innerHTML = `${icons.request}<span>${JC.t!('jellyseerr_modal_request_collection')}</span>`;
...
function configureTvShowButton(button: any, overallStatus: any, seasonAnalysis: any, item: any) {
    ...
    switch (overallStatus) {
        case MediaStatus.PENDING: setButton(JC.t!('jellyseerr_btn_pending'), icons.pending, 'jellyseerr-button-pending'); break;
        case MediaStatus.PROCESSING: setButton(JC.t!('jellyseerr_btn_request'), icons.request, 'jellyseerr-button-request'); break;
        case MediaStatus.DELETED: setButton(JC.t!(seasonAnalysis?.availableCount > 0 ? 'jellyseerr_btn_request_more' : 'jellyseerr_btn_request'), icons.request, 'jellyseerr-button-request'); break;
        case MediaStatus.PARTIALLY_AVAILABLE:
            setButton(JC.t!('jellyseerr_btn_request_missing'), icons.request, 'jellyseerr-button-partially-available');
            ...
            break;
        case MediaStatus.AVAILABLE: setButton(JC.t!('jellyseerr_btn_available'), icons.available, 'jellyseerr-button-available', true, seasonAnalysis?.total > 1 ? JC.t!('jellyseerr_all_seasons', {count: Number(seasonAnalysis.total) || 0}) : null); break;
        case MediaStatus.BLOCKED: setButton(JC.t!('jellyseerr_btn_blocklisted'), icons.cancel, 'jellyseerr-button-blocklisted', true); break;
        default: setButton(JC.t!('jellyseerr_btn_request'), icons.request, 'jellyseerr-button-request', false, seasonAnalysis?.total > 1 ? JC.t!('jellyseerr_seasons_available', {count: Number(seasonAnalysis.total) || 0}) : null); break;
    }

    // Gate on admin toggle AND Seerr 4K capability AND this user's 4K permission.
    const show4KOption = JC.jellyseerrAPI!.canRequest4k('tv');
    ...
    if (show4KOption && !button.closest('.jellyseerr-button-group')) {
        ...
        if (tvDs4k === DisplayStatus.AVAILABLE) {
            ...
            arrowButton.title = JC.t!('jellyseerr_btn_request_4k');
        }
...
function configureMovieButton(button: any, item: any) {
    ...
    const show4KOption = JC.jellyseerrAPI!.canRequest4k('movie');
    ...
    if (show4KOption && !button.closest('.jellyseerr-button-group')) {
        ...
        if (!mainButtonDisabled) {
            ...
            mainButton.addEventListener('click', async (e: any) => {
                e.stopPropagation();
                if (JC.pluginConfig.JellyseerrShowAdvanced) {
                    ui.showMovieRequestModal(item.id, item.title || item.name, item, false);
                } else {
                    mainButton.disabled = true;
                    mainButton.innerHTML = `<span>${JC.t!('jellyseerr_btn_requesting')}</span><span class="jellyseerr-button-spinner"></span>`;
                    try {
                        await JC.jellyseerrAPI!.requestMedia(item.id, 'movie', {}, false, item);
                        if (!item.mediaInfo) item.mediaInfo = {};
                        item.mediaInfo.status = 3;
                        mainButton.innerHTML = `<span>${JC.t!('jellyseerr_btn_requested')}</span>${icons.requested}`;
                        ...
                        if (ui.isQuotaError && ui.isQuotaError(error)) {
                            ...
                            mainButton.innerHTML = `${icons.request}<span>${JC.t!('jellyseerr_btn_request')}</span>`;
                            return;
                        }
                        let errorMessage = JC.t!('jellyseerr_btn_error');
                        if (error.status === 404) {
                            errorMessage = JC.t!('jellyseerr_btn_user_not_found');
                        } else if (error.status === 403) {
                            const code = error.responseJSON?.code;
                            errorMessage = JC.t!(code ? `jellyseerr_err_${code}` : 'jellyseerr_err_no_request_permission')
                                || JC.t!('jellyseerr_err_no_request_permission');
    ...
    if (stdSpinner) {
        button.innerHTML = `${icons[stdIconKey] || ''}<span>${JC.t!(stdLabelKey)}</span><span class="jellyseerr-button-spinner"></span>`;
        button.disabled = true;
    ...
    if (!button.disabled && !button.closest('.jellyseerr-button-group')) {
        button.addEventListener('click', async (e: any) => {
            ...
            if (JC.pluginConfig.JellyseerrShowAdvanced) {
                ...
                button.disabled = true;
                button.innerHTML = `<span>${JC.t!('jellyseerr_btn_requesting')}</span><span class="jellyseerr-button-spinner"></span>`;
                try {
                    await JC.jellyseerrAPI!.requestMedia(item.id, 'movie', {}, false, item);
                    if (!item.mediaInfo) item.mediaInfo = {};
                    item.mediaInfo.status = 3;
                    button.innerHTML = `<span>${JC.t!('jellyseerr_btn_requested')}</span>${icons.requested}`;
                    ...
                    if (ui.isQuotaError && ui.isQuotaError(error)) {
                        await ui.showQuotaErrorDialog(error, 'movie');
                        button.innerHTML = `${icons.request}<span>${JC.t!('jellyseerr_btn_request')}</span>`;
                        return;
                    }
                    let errorMessage = JC.t!('jellyseerr_btn_error');
                    if (error.status === 404) {
