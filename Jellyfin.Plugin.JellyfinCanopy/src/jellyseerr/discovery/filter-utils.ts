...
function createFilterControl(moduleName: string, onFilterChange: (mode: string) => void): HTMLElement {
    ...
    const allLabel = (typeof JC?.t === 'function') ? JC.t('jellyseerr_discover_all') || 'All' : 'All';
    const moviesLabel = (typeof JC?.t === 'function') ? JC.t('jellyseerr_card_badge_movie') || 'Movies' : 'Movies';
    const seriesLabel = (typeof JC?.t === 'function') ? JC.t('jellyseerr_card_badge_series') || 'Series' : 'Series';
    ...
    buttons.forEach((btn, index) => {
        ...
        button.addEventListener('click', (e: MouseEvent) => {
            ...
            container.querySelectorAll<HTMLElement>('.jellyseerr-filter-btn').forEach((b) => {
...
function createCardsFragment(results: any[], options: any = {}): DocumentFragment {
    ...
    const filteredResults = JC.hiddenContent
        ? JC.hiddenContent.filterJellyseerrResults(results, 'discovery')
        : results;
    ...
    for (let i = 0; i < filteredResults.length; i++) {
        ...
        const card = JC.jellyseerrUI?.createJellyseerrCard?.(item, true, true);
        if (!card) continue;
...
function waitForPageReady(signal?: AbortSignal, options: any = {}): Promise<any> {
    ...
    return new Promise((resolve) => {
        ...
        observerHandle = JC.helpers!.onBodyMutation!(`jellyseerr-discovery-container-detect-${++pageReadySeq}`, () => {
