...
//
// js/plugin.js (the loader, served at /JellyfinCanopy/script) creates the
// window.JellyfinCanopy namespace, fetches config/user settings, then loads
...
// Frozen public-contract anchor: JEGlobal extends JellyfinCanopyPublicApi
// (src/facade.ts) — the single typed home for the stable window.JellyfinCanopy
...
type _FrozenPublicApi = JEGlobal extends JellyfinCanopyPublicApi ? true : never;
