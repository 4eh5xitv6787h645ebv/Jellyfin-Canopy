...
JC.initializeElsewhereScript = function() {
    ...
    const ELSEWHERE_CUSTOM_BRANDING_IMAGE_URL = JC.pluginConfig.ElsewhereCustomBrandingImageUrl || '';

    if (!TmdbEnabled) {
        console.log('🪼 Jellyfin Canopy: 🎬 Jellyfin Elsewhere: TMDB is not configured, skipping initialization');
        return;
    }
    ...
    console.log('🪼 Jellyfin Canopy: 🎬 Jellyfin Elsewhere starting...');
    ...
    function createSettingsModal(): void {
        ...
        content.innerHTML = `
            <h3 style="margin-top: 0; margin-bottom: 16px; color: #fff; font-size: 18px; font-weight: bolder;">${JC.t('elsewhere_settings_title')}</h3>
            ...
            <div style="margin-bottom: 16px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600; color: #ccc;">${JC.t('elsewhere_settings_other_countries')}</label>
                <div id="regions-autocomplete"></div>
            </div>

           <div style="margin-bottom: 16px;">
                <label style="display: block; margin-bottom: 6px; font-weight: 600; color: #ccc;">${JC.t('elsewhere_settings_providers')}</label>
        ...
        const regionsAutocomplete = createAutocompleteInput(
            JC.t('elsewhere_settings_add_countries_placeholder'),
            regionOptions,
        ...
        const servicesAutocomplete = createAutocompleteInput(
            JC.t('elsewhere_settings_add_providers_placeholder'),
            availableProviders,
            userServices.slice(),
            () => {}
        );
    ...
    function fetchStreamingData(tmdbId: string, mediaType: string, callback: (error: string | null, data?: any) => void): void {
        ...
        JC.core.api.fetch(url)
            ...
            .catch((error: unknown) => {
                ...
                // JC.core.api throws Error('HTTP <status>') for non-OK responses
    ...
    function processDefaultRegionData(data: any, tmdbId: string, mediaType: string): HTMLElement {
        ...
        if (hasServices) {
            ...
            if (IGNORE_PROVIDERS.length > 0) {
                try {
                    ...
                    console.error('🪼 Jellyfin Canopy: 🎬 Jellyfin Elsewhere: Invalid regex in IGNORE_PROVIDERS.', e);
                }
            }
        }
        ...
        searchButton.onclick = (event) => {
            ...
            searchButton.appendChild(document.createTextNode(' ' + JC.t('elsewhere_panel_search_button')));
    ...
    function createUnavailableRegionsDisplay(unavailableRegions: string[]): HTMLElement {
        ...
        title.textContent = JC.t('elsewhere_panel_not_available_in_regions', { regions: regionText });
        title.style.cssText = `
            font-weight: 600;
            font-size: 14px;
            color: rgb(255, 20, 20);
