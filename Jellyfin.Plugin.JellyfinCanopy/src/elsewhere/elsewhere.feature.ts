...
import { initializeElsewhere, installElsewhere } from './elsewhere';
...
export function activate(scope: FeatureScope): void {
    ...
    scope.track(dispose);
    cleanups.push(installElsewhere());
    cleanups.push(JC.identity.registerReset('elsewhere-feature', dispose));
    if (!scope.isCurrent()) { dispose(); return; }
    initializeElsewhere();
    if (!scope.isCurrent()) dispose();
}

export const elsewhereFeature: FeatureModule = { activate };
