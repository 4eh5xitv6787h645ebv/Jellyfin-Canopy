...
// `JC.pluginConfig.<Key>` that no SettingDescriptor projects — such a key is
...
const SNAPSHOTS_DIR = SRC_ROOT.replace(
    ...
    'Jellyfin.Plugin.JellyfinCanopy.Tests/Snapshots/',
);
...
// pluginConfig.Foo / pluginConfig?.Foo / JC.pluginConfig.Foo — the identifier is
...
describe('config-read-guard (W2-TEST-2): client pluginConfig reads are real projected descriptors', () => {
    ...
    it('every JC.pluginConfig.<Key> read is projected or a known client-synthesized key', () => {
        ...
        expect(
            dangling,
            'Client reads a JC.pluginConfig key that no SettingDescriptor projects — the '
