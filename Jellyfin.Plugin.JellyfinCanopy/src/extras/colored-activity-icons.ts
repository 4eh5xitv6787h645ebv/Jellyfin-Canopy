// src/extras/colored-activity-icons.ts
...
/** One activity-log icon rule: matched by a wildcard pattern over localized parts. */
interface IconRule {
    parts: string[];
    icon: string;
    color: string;
...
function buildActivityPattern(parts: string[]): RegExp {
...
function updateActivityIcons(
    ...
    try {
        ...
        activityLinks.forEach(anchor => {
            ...
            const avatar = anchor.querySelector<HTMLElement>('.MuiAvatar-root');
            ...
            const dataAttr = 'data-jellyfin-canopy-activity-icon';
...
function startMonitoring(context: IdentityContext, expectedGeneration: number): void {
    ...
    if (JC?.helpers?.onBodyMutation) {
        observer = JC.helpers.onBodyMutation('colored-activity-icons', callback);
    } else {
...
function initialize(): void {
    ...
    lifecycle = JC.core.lifecycle.register('colored-activity-icons');
