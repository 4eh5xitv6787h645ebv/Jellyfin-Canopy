...
function setup(opts: { admin: boolean; allUsers?: boolean }): { plugin: ReturnType<typeof vi.fn> } {
    ...
    JC.pluginConfig = { ActiveStreamsEnabled: true, ActiveStreamsAllUsers: opts.allUsers === true };
    JC.currentUser = { Policy: { IsAdministrator: opts.admin } };
    JC.t = (k: string) => k;
...
describe('active-streams session control', () => {
    ...
    it('renders Stop + Message actions for an admin on a controllable session', async () => {
        setup({ admin: true });
        await openPanel();
        const card = document.querySelector('.jc-as-card[data-session-id="sess-1"]');
        expect(card).not.toBeNull();
        expect(card!.querySelector('.jc-as-action-btn-stop')).not.toBeNull();
        expect(card!.querySelectorAll('.jc-as-action-btn').length).toBe(2); // stop + message
    });

    it('Stop needs a confirm then POSTs the stop endpoint', async () => {
        const { plugin } = setup({ admin: true });
        await openPanel();
        const stopBtn = document.querySelector<HTMLButtonElement>('.jc-as-action-btn-stop')!;

        // First click arms the confirm state — no request yet.
        stopBtn.dispatchEvent(new MouseEvent('click', { bubbles: true }));
        await flush();
        expect(stopBtn.classList.contains('jc-as-confirming')).toBe(true);
    ...
    it('opening the message form and sending POSTs the per-session message endpoint', async () => {
        ...
        await openPanel();
        const card = document.querySelector('.jc-as-card[data-session-id="sess-1"]')!;
        const msgBtn = Array.from(card.querySelectorAll<HTMLButtonElement>('.jc-as-action-btn'))
            .find((b) => !b.classList.contains('jc-as-action-btn-stop'))!;
        msgBtn.dispatchEvent(new MouseEvent('click', { bubbles: true }));
        const form = card.querySelector('.jc-as-msg-form')!;
        expect(form.classList.contains('jc-as-msg-form-open')).toBe(true);

        // A preset chip fills the textarea; Send POSTs it.
        const preset = card.querySelector<HTMLButtonElement>('.jc-as-preset')!;
        preset.dispatchEvent(new MouseEvent('click', { bubbles: true }));
        const textArea = card.querySelector<HTMLTextAreaElement>('.jc-as-broadcast-textarea')!;
        expect(textArea.value.length).toBeGreaterThan(0);
        card.querySelector<HTMLButtonElement>('.jc-as-broadcast-send')!
    ...
    it('does NOT render session actions for a non-admin viewer', async () => {
        ...
        expect(card!.querySelector('.jc-as-actions')).toBeNull();
    });
});
