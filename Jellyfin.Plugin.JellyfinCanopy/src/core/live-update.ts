...
//
// When an admin updates the JellyfinCanopy DLL while browser sessions stay
// open, those sessions keep running the OLD client bundle until a manual reload.
// This module compares the version the session loaded (JC.pluginVersion, set by
// js/plugin.js from /JellyfinCanopy/version at boot) against the server's
...
const loadedVersion = JC.pluginVersion;
...
export function notifyIfNewer(serverVersion: string | null | undefined): void {
    ...
    notified = true;
    const message = 'Jellyfin Canopy updated — refresh to load the new version';
    try {
        // Prefer the canonical core toast; fall back to the frozen JC.toast alias.
        if (JC.core.ui?.toast) {
            JC.core.ui.toast(message, 8000);
        } else if (JC.toast) {
            JC.toast(message, 8000);
        }
    } catch (err) {
