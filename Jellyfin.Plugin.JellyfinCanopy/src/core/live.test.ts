...
describe('dispatch (SDK message → JC live event routing)', () => {
    ...
    it('routes a JC-marked GeneralCommand to the marker-named event with the Arguments', () => {
        const h = vi.fn();
        on(LIVE.CONFIG_CHANGED, h);
        const args = { JellyfinCanopy: 'config-changed', Version: '1.2.3.0' };
    ...
    it('ignores GeneralCommands without the JC marker (native remote-control)', () => {
        const h = vi.fn();
        on('config-changed', h);
        on('MoveUp', h);
...
describe('hub wiring on the JC global', () => {
    it('exposes JC.core.live with the fan-out surface', () => {
        expect(JC.core.live).toBeDefined();
        expect(typeof JC.core.live?.on).toBe('function');
        expect(typeof JC.core.live?.emit).toBe('function');
        expect(typeof JC.core.live?.isConnected).toBe('function');
    });
    ...
    it('fails soft when ApiClient.subscribe is absent (setup stub has none)', () => {
        ...
        expect(JC.core.live?.isConnected()).toBe(false);
    });
});
...
describe('SDK-present wiring + teardown (fresh module)', () => {
    ...
    it('subscribes once to the native types and dispatches via the SDK callback', async () => {
        ...
        // The fresh module reassigned JC.core.live; it reports connected.
        expect(JC.core.live?.isConnected()).toBe(true);
