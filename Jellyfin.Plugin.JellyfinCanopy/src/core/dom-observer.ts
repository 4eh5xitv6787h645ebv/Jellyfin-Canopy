...
//
// Public surface: JC.core.dom { onBodyMutation, removeBodySubscriber,
// createObserver, disconnectObserver, disconnectAllObservers, waitForElement }.
// JC.helpers keeps thin aliases for unmigrated callers.
    ...
    bodyObserver = new MutationObserver((mutations) => {
        ...
        for (const [id, sub] of bodySubscribers) {
            try {
                ...
                console.error(`🪼 Jellyfin Canopy: Error in body observer subscriber "${id}":`, err);
            }
        }
    });
    bodyObserver.observe(document.body, { childList: true, subtree: true });
    console.log('🪼 Jellyfin Canopy: Shared body observer started');
}
...
function stopBodyObserverIfEmpty(): void {
    if (bodyObserver && bodySubscribers.size === 0) {
        ...
        bodyObserver = null;
        console.log('🪼 Jellyfin Canopy: Shared body observer stopped (no subscribers)');
    }
}
...
export function createObserver(
    ...
    if (existing) {
        existing.disconnect();
        console.warn(`🪼 Jellyfin Canopy: Replacing existing observer: ${id}`);
    }
    ...
    console.log(`🪼 Jellyfin Canopy: Created dedicated observer: ${id} (total: ${activeObservers.size})`);
...
export function disconnectObserver(id: string): boolean {
    ...
    if (observer) {
        observer.disconnect();
        activeObservers.delete(id);
        console.log(`🪼 Jellyfin Canopy: Disconnected observer: ${id} (remaining: ${activeObservers.size})`);
        return true;
    }
    return false;
}
...
export function disconnectAllObservers(): void {
    ...
    console.log('🪼 Jellyfin Canopy: All observers and body subscribers disconnected');
}
...
export function waitForElement(selector: string, timeout = 10000): Promise<Element | null> {
    return new Promise((resolve) => {
        ...
        timeoutId = setTimeout(() => {
            handle.unsubscribe();
            console.warn(`🪼 Jellyfin Canopy: Timeout waiting for element: ${selector}`);
            resolve(null);
        }, timeout);
    });
}
...
//   (b) re-runs on every navigation (HISTORY_UPDATE/jc:navigate via onNavigate),
...
interface InjectorEntry {
    ...
    // Set once when a buildFn returns void/null and leaves no [data-jc-key] node:
...
function isKeyedPresent(key: string, headerTray: boolean): boolean {
    const nodes = document.querySelectorAll<HTMLElement>(`[data-jc-key="${escapeKey(key)}"]`);
...
function runInjector(entry: InjectorEntry, prePaint = false): void {
    try {
        ...
        if (node instanceof HTMLElement && !node.dataset.jcKey) {
            node.dataset.jcKey = entry.key;
            return;
        }
        ...
        if (!(node instanceof HTMLElement)) {
            ...
            if (!stillPresent) {
                ...
                console.warn(
                    `🪼 Jellyfin Canopy: ensureInjected("${entry.key}") buildFn returned no ` +
                    `element and left no [data-jc-key] node — it must self-tag or return the node. ` +
                    `Suppressing re-runs for this key to avoid a DOM flood.`
                );
            }
        }
    } catch (err) {
        console.error(`🪼 Jellyfin Canopy: Error in ensureInjected("${entry.key}"):`, err);
    }
}
...
function ensureInjectorsWired(): void {
    ...
    injectorsWired = true;
    // Every URL change (HISTORY_UPDATE, jc:navigate, hashchange, popstate).
    ...
    onBodyMutation('jc-ensure-injected', () => {
 ...
 * @param key - Stable identity for the injected node (also its `data-jc-key`).
 ...
 *   it can be tagged with the key (or tag it yourself via `data-jc-key`).
...
export function ensureInjected(
    ...
    return {
        ...
        remove: () => {
            ...
            document.querySelectorAll(`[data-jc-key="${escapeKey(key)}"]`).forEach((n) => n.remove());
        }
    };
}
...
export function onSidebarRebuild(id: string, check: () => void): () => void {
    if (sidebarRebuildChecks.has(id)) {
        console.warn(`🪼 Jellyfin Canopy: Replacing sidebar rebuild check: ${id}`);
    }
    sidebarRebuildChecks.set(id, check);
    if (!sidebarRebuildHandle) {
        sidebarRebuildHandle = onBodyMutation('jc-sidebar-rebuild', () => {
            sidebarRebuildChecks.forEach((fn, checkId) => {
                try {
                    fn();
                } catch (err) {
                    console.error(`🪼 Jellyfin Canopy: Error in sidebar rebuild check "${checkId}":`, err);
                }
            });
        });
    }
...
console.log('🪼 Jellyfin Canopy: DOM observer core initialized');
