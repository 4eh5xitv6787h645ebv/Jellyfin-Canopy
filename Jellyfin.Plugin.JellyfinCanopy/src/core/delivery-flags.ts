...
// settings-config-resolution.md) and caches the installed state on JC. This
...
/** Installed state of the two delivery plugins, cached on JC by js/plugin.js at boot. */
...
/**
 * Re-apply {@link sanitizeDeliveryPluginFlags} to JC.pluginConfig using the
...
export function applyDeliveryFlagSanitization(): void {
    const installed = JC._deliveryPluginsInstalled;
    if (!installed) return;
    sanitizeDeliveryPluginFlags(JC.pluginConfig, installed);
}
