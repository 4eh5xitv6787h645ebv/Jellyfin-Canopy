...
describe('ensureInjected', () => {
    ...
    it('injects once into a live page and no-ops on re-run', () => {
        ...
        expect(page.querySelector('[data-jc-key="t-once"]')).not.toBeNull();
    ...
    it('does NOT count a marker stranded in a cached .page.hide as present (§6.8)', () => {
        ...
        expect(builds).toBe(1);
        expect(live.querySelector('[data-jc-key="t-stale"]')).not.toBeNull();
    ...
    it('re-attaches a header-tray node after the /video unmount', () => {
        ...
        expect(builds).toBe(1);
        expect(tray.querySelector('[data-jc-key="t-tray"]')).not.toBeNull();

        // Simulate entering /video: the toolbar (and our button) is destroyed.
        tray.querySelector('[data-jc-key="t-tray"]')!.remove();
        expect(tray.querySelector('[data-jc-key="t-tray"]')).toBeNull();
    ...
    it('remove() deletes the node and makes the handle inert', () => {
        ...
        expect(builds).toBe(1);
        handle.remove();
        expect(document.querySelector('[data-jc-key="t-remove"]')).toBeNull();
    ...
    it('prePaint injector re-attaches synchronously inside the mutation batch with prePaint:true', async () => {
        ...
        tray.querySelector('[data-jc-key="t-prepaint"]')!.remove();
        ...
        expect(tray.querySelector('[data-jc-key="t-prepaint"]')).not.toBeNull();
    ...
    it('non-prePaint injectors never run with prePaint:true', async () => {
        ...
        tray.querySelector('[data-jc-key="t-noprepaint"]')!.remove();
        tray.appendChild(document.createElement('div'));
        await new Promise((r) => setTimeout(r, 50));

        expect(contexts.every((c) => c === false)).toBe(true);
        expect(tray.querySelector('[data-jc-key="t-noprepaint"]')).not.toBeNull();
...
describe('runPrePaintInjectors R8 budget', () => {
    ...
    it('defers pre-paint injectors past the ~2ms budget to the rAF pass', async () => {
        ...
        tray.querySelectorAll('[data-jc-key]').forEach((n) => n.remove());
        ...
        await new Promise((r) => setTimeout(r, 50));
        expect(tray.querySelectorAll('[data-jc-key]').length).toBe(N);
...
describe('onSidebarRebuild', () => {
    ...
    it('shares a single body subscriber across all registered checks', async () => {
        const before = JC.core.dom!.getBodySubscriberCount();
        ...
        expect(JC.core.dom!.getBodySubscriberCount()).toBe(before + 1);
        ...
        expect(JC.core.dom!.getBodySubscriberCount()).toBe(before);
    });
