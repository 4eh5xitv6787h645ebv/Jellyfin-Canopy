...
describe('config hot-reload delivery-flag re-sanitize (INIT-1)', () => {
    ...
    it('re-forces a stale UsePluginPages flag to false after the live merge', async () => {
        ...
        (JC.pluginConfig as Record<string, unknown>).HiddenContentUsePluginPages = false;
        ...
        await vi.waitFor(() => {
            expect(JC.pluginConfig.RefreshMarker).toBe('done');
            expect(JC.pluginConfig.HiddenContentUsePluginPages).toBe(false);
        });
    });
    ...
    it('never exposes a stale flag as true during the private-config round-trip (LC-1)', async () => {
        ...
        JC.core.api = {
            plugin: vi.fn((path: string) => {
                ...
                if (path.startsWith('/private-config')) {
                    flagDuringWindow = JC.pluginConfig.HiddenContentUsePluginPages;
        ...
        releasePrivate({});
        await vi.waitFor(() => expect(JC.pluginConfig.HiddenContentUsePluginPages).toBe(false));
    });
});
