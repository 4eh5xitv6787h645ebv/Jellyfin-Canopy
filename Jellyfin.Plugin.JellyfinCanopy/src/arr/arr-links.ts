...
// Public surface (frozen): JC.initializeArrLinksScript (called by js/plugin.js
// Stage 6), JC._arrLinksObserver.
...
/** One match from /arr/series-slugs, with the instance URL already mapped. */
interface SeriesSlugMatch {
    instanceName?: string;
...
/** One match from /arr/movie-instances, with the instance URL already mapped. */
interface MovieInstanceMatch {
    name?: string;
    url: string | null;
    hasFile: boolean;
    sizeOnDisk: number;
...
JC.initializeArrLinksScript = async function () {
    ...
    const slugCache = new Map<string, SeriesSlugMatch[] | MovieInstanceMatch[]>();
    ...
    try {
        ...
        const sonarrInstances: ResolvedInstance[] = (JC.pluginConfig.SonarrInstances || [])
        ...
        const radarrInstances: ResolvedInstance[] = (JC.pluginConfig.RadarrInstances || [])
            .filter(i => i && i.Enabled !== false)
            .map(i => ({
                name: i.Name || 'Radarr',
                url: resolveArrLinkBase(i.Url, i.ExternalUrl, i.UrlMappings),
        ...
        if (sonarrInstances.length === 0 && JC.pluginConfig.SonarrUrl) {
            const legacyUrl = resolveArrLinkBase(JC.pluginConfig.SonarrUrl, JC.pluginConfig.SonarrExternalUrl, JC.pluginConfig.SonarrUrlMappings);
            if (legacyUrl) {
                sonarrInstances.push({ name: 'Sonarr', url: legacyUrl, rawUrl: JC.pluginConfig.SonarrUrl, urlMappings: '' });
            }
        }
        if (radarrInstances.length === 0 && JC.pluginConfig.RadarrUrl) {
            const legacyUrl = resolveArrLinkBase(JC.pluginConfig.RadarrUrl, JC.pluginConfig.RadarrExternalUrl, JC.pluginConfig.RadarrUrlMappings);
            if (legacyUrl) {
                radarrInstances.push({ name: 'Radarr', url: legacyUrl, rawUrl: JC.pluginConfig.RadarrUrl, urlMappings: '' });
            }
        }

        const bazarrUrl = resolveArrLinkBase(JC.pluginConfig.BazarrUrl, JC.pluginConfig.BazarrExternalUrl, JC.pluginConfig.BazarrUrlMappings);
        ...
        function getStatus(episodeFileCount: number, episodeCount: number): string {
            if (episodeFileCount === 0) return 'missing';
            if (episodeFileCount >= episodeCount) return 'complete';
            return 'partial';
        }

        function getExternalIds(context: Element): { tmdb: string | null; hasTmdbLink: boolean } {
            const ids: { tmdb: string | null; hasTmdbLink: boolean } = { tmdb: null, hasTmdbLink: false };
            ...
            links.forEach(link => {
                ...
                if (href.includes('themoviedb.org/movie/')) {
                    ids.tmdb = /\/movie\/(\d+)/.exec(href)?.[1] ?? null;
                    ids.hasTmdbLink = true;
                } else if (href.includes('themoviedb.org/tv/')) {
                    ids.tmdb = /\/tv\/(\d+)/.exec(href)?.[1] ?? null;
                    ids.hasTmdbLink = true;
                }
            });
            return ids;
         ...
         * @param item - Jellyfin item object with Name, OriginalTitle, and ProviderIds
         * @returns Array of { instanceName, instanceUrl, titleSlug, ... } matches
         */
        async function getSonarrSlugs(item: { ProviderIds?: Record<string, string | undefined> }): Promise<SeriesSlugMatch[]> {
            const tvdbId = String(item.ProviderIds?.Tvdb || '');
            const cacheKey = `slugs-${tvdbId}`;

            if (tvdbId && slugCache.has(cacheKey)) {
                return slugCache.get(cacheKey) as SeriesSlugMatch[];
            }
         ...
         */
        async function getRadarrInstances(tmdbId: string | null): Promise<MovieInstanceMatch[]> {
            if (!tmdbId) return [];

            const cacheKey = `radarr-${tmdbId}`;
            if (slugCache.has(cacheKey)) {
                return slugCache.get(cacheKey) as MovieInstanceMatch[];
            }
        ...
        async function addArrLinks(): Promise<void> {
            ...
            try {
                ...
                const showStatusOnSingle = JC?.pluginConfig?.ArrLinksShowStatusSingle === true;
                ...
                if (item.Type === 'Series' && item.Name && sonarrInstances.length > 0) {
                    ...
                    if (validMatches.length === 1) {
                        ...
                        const url = `${(m.instanceUrl as string).replace(/\/$/, '')}/series/${m.titleSlug}`;
                        ...
                        const items = validMatches.map((m): DropdownItem => {
                            ...
                            return {
                                name: m.instanceName,
                                url: `${(m.instanceUrl as string).replace(/\/$/, '')}/series/${m.titleSlug}`,
                ...
                if (item.Type === 'Movie' && ids.tmdb && radarrInstances.length > 0) {
                    ...
                    if (validMatches.length === 1) {
                        const m = validMatches[0];
                        const url = `${(m.url as string).replace(/\/$/, '')}/movie/${ids.tmdb}`;
                        ...
                        const items = validMatches.map((m): DropdownItem => {
                            ...
                            return {
                                name: m.name,
                                url: `${(m.url as string).replace(/\/$/, '')}/movie/${ids.tmdb}`,
                                status, badge, size, tip
                            };
                        });
                        linksFragment.appendChild(document.createTextNode(' '));
        ...
        function createLinkButton(text: string, url: string, iconClass: string, status?: string | null, badge?: string, tooltip?: string): HTMLAnchorElement {
        ...
        function createDropdown(label: string, iconClass: string, items: DropdownItem[]): HTMLSpanElement {
