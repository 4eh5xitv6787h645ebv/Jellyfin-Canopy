...
export async function openInteractiveSearch(itemId: string): Promise<void> {
    const modal = createArrModal({ title: JC.t!('arr_search_interactive'), subtitle: JC.t!('arr_search_loading'), icon: 'travel_explore' });
    ...
    if (ctx.kind === 'unknown' || !ctx.service) {
        renderCentered(modal.body, message('info', JC.t!('arr_search_not_arr_item')));
        return;
    }
    if (!ctx.supportsInteractive) {
        renderCentered(modal.body, message('info', JC.t!('arr_search_interactive_unsupported')));
        return;
    }
    if (ctx.targets.length === 0) {
        renderCentered(modal.body, message('info', JC.t!('arr_search_not_tracked')));
        return;
    }
...
function headerSubtitle(ctx: ArrContext): string {
    let sub = ctx.name || '';
    if (ctx.kind === 'season' && ctx.seasonNumber != null) sub += ` · ${JC.t!('arr_search_season')} ${ctx.seasonNumber}`;
...
class ReleaseView {
    ...
    private loadSeq = 0;
    private listEl = el('div', 'jc-arr-release-list');
    private countEl = el('div', 'jc-arr-release-count');
    ...
    private buildToolbar(): HTMLElement {
        const bar = el('div', 'jc-arr-toolbar');
    ...
    private renderList(): void {
        ...
        if (list.length === 0) {
            renderCentered(this.listEl, message('info', this.releases.length === 0 ? JC.t!('arr_search_no_releases') : JC.t!('arr_search_no_matches')));
            return;
        }
    ...
    private buildRow(release: ArrRelease): HTMLElement {
        ...
        const meta = el('div', 'jc-arr-release-meta');
        if (release.quality) meta.appendChild(el('span', 'jc-arr-badge', release.quality));
        meta.appendChild(el('span', 'jc-arr-dim', formatSize(release.size)));
        meta.appendChild(el('span', 'jc-arr-dim', formatAge(release.ageHours)));
        if (release.indexer) meta.appendChild(el('span', 'jc-arr-dim', release.indexer));
        if (release.protocol === 'torrent') {
            meta.appendChild(el('span', 'jc-arr-dim', JC.t!('arr_search_seeders', { seeders: release.seeders ?? 0, leechers: release.leechers ?? 0 })));
        } else if (release.protocol) {
            meta.appendChild(el('span', 'jc-arr-dim', release.protocol));
        }
        if (release.releaseGroup) meta.appendChild(el('span', 'jc-arr-dim', release.releaseGroup));
        ...
        meta.appendChild(el('span', 'jc-arr-cf', `CF ${Number(release.customFormatScore) || 0}`));
...
function labeled(labelText: string, control: HTMLElement): HTMLElement {
    const wrap = el('label', 'jc-arr-field');
    wrap.appendChild(el('span', 'jc-arr-field-label', labelText));
