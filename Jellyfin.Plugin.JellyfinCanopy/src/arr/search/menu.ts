...
import {
    buildNativeActionSheetItem, getActiveActionSheetScroller, fitActionSheetItem, closeOpenActionSheet,
...
export function injectSearchItems(): void {
    ...
    if (existing && existing.dataset.jcItemId === ctx.itemId) { setCaptured(null); return; }
    ...
    // Append the group at the end of the sheet, then re-fit ONCE (both axes). PERF(R4/R7): all
    // inserts (writes) happen before the single layout-reading fit pass, so we never interleave
    // offsetWidth/Height reads with DOM writes across the loop. fitActionSheetItem corrects the
    // right-edge (wide label) AND bottom-edge (extra rows) overflow the appended group would cause.
    let anchor: Element | null = scroller.lastElementChild;
    for (const item of items) {
        if (anchor) anchor.after(item); else scroller.appendChild(item);
        anchor = item;
    }
    if (items.length > 0) fitActionSheetItem(items[items.length - 1], scroller);
