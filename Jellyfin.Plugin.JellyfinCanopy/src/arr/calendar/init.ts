...
// and the public JC.calendarPage surface (split from calendar-page.js).
// The navigation-watcher fallback interval is tracked with core lifecycle.
// Public surface (frozen): JC.calendarPage + JC.initializeCalendarPage —
...
/** The frozen JC.calendarPage contract (js/plugin.js + PluginPages HTML). */
...
const pluginPagesExists = !!sidebar?.querySelector(
    'a[is="emby-linkbutton"][data-itemid="Jellyfin.Plugin.JellyfinCanopy.CalendarPage"]'
);
...
// Use event-based navigation detection (pushState/hashchange/popstate via jc:navigate)
function startLocationWatcher(): void {
    ...
    state.locationUnsubscribe = lifecycle.track(
        JC.helpers?.onNavigate
            ? JC.helpers.onNavigate(check)
            : (() => {
...
function showPage(): void {
    ...
    const activePage = document.querySelector('.mainAnimatedPage:not(.hide):not(#jc-calendar-page)');
    if (activePage) {
...
function hidePage(): void {
    ...
    const page = document.getElementById('jc-calendar-page');
    if (page) {
    ...
    if (state.previousPage && !document.querySelector('.mainAnimatedPage:not(.hide):not(#jc-calendar-page)')) {
...
function handleViewShow(e: Event): void {
    ...
    if (state.pageVisible && targetPage && targetPage.id !== 'jc-calendar-page') {
        hidePage();
    }
}
...
function handleNavClick(e: MouseEvent): void {
    ...
    if (btn && !btn.classList.contains('jc-nav-calendar-item')) {
        hidePage();
    }
}
...
function injectNavigation(): void {
    ...
    if (document.querySelector('.jc-nav-calendar-item')) {
        return;
    }
    ...
    if (jellyfinCanopySection) {
        ...
        navItem.className =
            'navMenuOption lnkMediaFolder emby-button jc-nav-calendar-item';
        navItem.href = '#';
        navItem.innerHTML = `
        <span class="navMenuOptionIcon material-icons">calendar_today</span>
        <span class="sectionName navMenuOptionText">${JC.t?.('calendar_title')}</span>
      `;
        ...
        console.log(`${logPrefix} jellyfinCanopySection not found, will wait for it`);
    }
}
...
function setupNavigationWatcher(): void {
    ...
    onSidebarRebuild('calendar-nav', () => {
        ...
        const currentConfig = JC.pluginConfig || {};
...
JC.initializeCalendarPage = initialize;
