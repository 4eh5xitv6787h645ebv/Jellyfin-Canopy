...
//
// Requests Custom Tab
// Creates <div class="jellyfincanopy requests"></div>, either inside a tab
...
(function () {
    if (!JC?.pluginConfig?.DownloadsPageEnabled) {
        return;
    }

    const useCustomTabs = !!JC?.pluginConfig?.DownloadsUseCustomTabs;
    const useNativeTab = !!JC?.pluginConfig?.DownloadsUseNativeTab;

    console.log('🪼 Jellyfin Canopy: [requests-custom-tab] DownloadsUseCustomTabs=' + useCustomTabs + ', DownloadsUseNativeTab=' + useNativeTab);
    ...
    if (useNativeTab) {
        JC.nativeTabs?.register('requests', 'Requests', function (panel) {
            const marker = document.createElement('div');
            marker.className = 'jellyfincanopy requests';
    ...
    style.textContent = [
        '.jellyfincanopy.requests {',
        '  padding: 12px 3vw;',
        '}',
        '.backgroundContainer.withBackdrop:has(~ .mainAnimatedPages #indexPage .tabContent.is-active .jellyfincanopy.requests) {',
    ...
    /** Wait for JC.downloadsPage to be ready before initializing (30s timeout). */
    function waitForDownloads(callback: (jc: ArrJE) => void): void {
        let attempts = 0;
    ...
    function findActiveContainer(): HTMLElement | null {
        const all = document.querySelectorAll<HTMLElement>('.jellyfincanopy.requests');
     ...
     * @param container - The active .jellyfincanopy.requests element.
     * @param jc - The JellyfinCanopy global object.
     */
    function renderDownloads(container: HTMLElement, jc: ArrJE): void {
        container.classList.remove('hide');
        container.style.display = '';

        const child = document.createElement('div');
        child.id = 'jc-downloads-container-tab';
        container.textContent = '';
        container.appendChild(child);

        jc.downloadsPage?.renderForCustomTab?.(child);
    ...
    function watchForContainer(jc: ArrJE): void {
        ...
        function tryMount(): void {
            ...
            if (!onHome) {
                if (wasOnHomePage) {
                    jc.downloadsPage?.stopPolling?.();
                    if (jc.downloadsPage?._state) {
                        jc.downloadsPage._state._customTabMode = false;
                    }
        ...
        jc.helpers?.createObserver?.('arr-requests-custom-tab', function () {
            if (!mountPending) {
                mountPending = true;
