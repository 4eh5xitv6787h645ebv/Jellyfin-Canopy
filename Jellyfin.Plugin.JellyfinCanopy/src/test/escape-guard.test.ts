...
//   1. wrap the expression in escapeHtml(...) (JC.escapeHtml / core/ui-kit),
...
//      a recognized producer (UPPERCASE const, icons table, JC.icon, a local
 ...
 *  - icon (JC.icon, enhanced/icons.ts): lookup into the frozen plugin icon
 *    sets (emoji / plugin-owned SVG markup — escaping would break it).
 *  - getThemeVariables (JC.themer): theme-engine CSS values (colors, blur)
...
/**
 * Translation calls: JC.t() does NOT escape params (the toast trap) — a t()
...
function checkResolvedCall(ctx: FileContext, call: ts.CallExpression): ts.Node[] | null {
    ...
    if (!infos || infos.length === 0) {
        // `const esc = JC.escapeHtml` style alias of a known-safe name.
...
function walkFile(ctx: FileContext): ts.Node[] {
    ...
    const visit = (node: ts.Node): void => {
        ...
        if (ts.isCallExpression(node)) {
            ...
            //    even when it carries no markup (the JC.t() params trap).
...
describe('escape-guard (SEC X1): HTML-template interpolations are recognizably safe', () => {
    ...
    it('every interpolation is escaped, numeric, a trusted producer, or allowlisted', () => {
        ...
        expect(
            ...
            + 'that toast() renders innerHTML and JC.t() does NOT escape params. Only genuinely '
...
describe('escape-guard classifier self-tests', () => {
    ...
    it('accepts the three safe classes: escapeHtml, numeric coercion, trusted producers', () => {
        expect(scanFixture(
            ...
            + '${JC.icon!(JC.IconName!.STAR)}${MY_SVG_ICON}${icons.request}${assetUrl(\'icons/sonarr.svg\')}`;'
        )).toEqual([]);
    });

    it('flags the toast trap: t() params carrying unescaped values, even with no markup', () => {
        const trap = scanFixture('toast(JC.t!(\'toast_subtitle\', { subtitle: subtitleName }));');
        expect(trap.map((v) => v.text)).toEqual(['subtitleName']);
        expect(scanFixture(
            'toast(JC.t!(\'toast_subtitle\', { subtitle: JC.escapeHtml(subtitleName) }));'
        )).toEqual([]);
        expect(scanFixture('toast(JC.t!(\'toast_done\'));')).toEqual([]);
    });
