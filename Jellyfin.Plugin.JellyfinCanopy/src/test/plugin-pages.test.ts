...
describe('nav-page container never self-destructs (SRV-3)', () => {
    afterEach(() => {
        ...
        vi.unstubAllGlobals();
        delete (window as unknown as Record<string, unknown>).JellyfinCanopy;
    });

    const cases = [
        { file: 'CalendarPage.html', containerId: 'jc-calendar-container', pageClass: 'jc-calendar-page' },
        { file: 'DownloadsPage.html', containerId: 'jc-downloads-container', pageClass: 'jc-downloads-page' },
        { file: 'HiddenContentPage.html', containerId: 'jc-hidden-content-container', pageClass: 'jc-hidden-content-page' },
        { file: 'BookmarksPage.html', containerId: null, pageClass: 'jc-bookmarks-page' }, // reference: sections.bookmarks
    ];
    ...
    for (const c of cases) {
        it(`${c.file}: authored container survives with no visible preferences page`, () => {
            ...
            (window as unknown as Record<string, unknown>).JellyfinCanopy = {}; // not "ready" → poll would spin, but it's stubbed
        ...
        if (c.containerId) {
            it(`${c.file}: recreates the container into its own page when missing`, () => {
                vi.stubGlobal('setInterval', () => 0 as unknown);
                vi.stubGlobal('clearInterval', () => { /* no-op */ });
                (window as unknown as Record<string, unknown>).JellyfinCanopy = {};
...
describe('DownloadsPage fallback poll (SRV-5)', () => {
    ...
    it('stops the poll once the page (container) is gone', () => {
        const refresh = vi.fn();
        runDownloadsBootstrap(refresh);
        const JC = (window as unknown as { JellyfinCanopy: DownloadsJE }).JellyfinCanopy;
        expect(JC.downloadsPage._pluginPagePollTimer).toBeTruthy();

        document.getElementById('jc-downloads-container')!.remove();
        refresh.mockClear();
        vi.advanceTimersByTime(30);

        expect(JC.downloadsPage._pluginPagePollTimer).toBeNull(); // interval cleared on nav-away
