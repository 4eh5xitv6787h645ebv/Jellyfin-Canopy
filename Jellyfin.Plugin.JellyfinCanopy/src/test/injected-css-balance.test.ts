...
//
// active-streams.ts injects a single `.jc-as-*` stylesheet via a backtick
// template literal. A selector line (`.jc-as-broadcast-timeout-row {`) was lost,
...
//   1. every `.jc-as-broadcast-*` class applied in JS has a selector in the CSS,
...
/** All `.jc-as-broadcast-*` classes applied via `className = '…'` in JS. */
function usedBroadcastClasses(): string[] {
    const set = new Set<string>();
    for (const m of SOURCE.matchAll(/className\s*=\s*'([^']*jc-as-broadcast[^']*)'/g)) {
        for (const token of m[1].split(/\s+/)) {
            if (token.startsWith('jc-as-broadcast')) set.add(token);
        }
    }
    return [...set].sort();
}
...
describe('active-streams injected CSS (W2-CSS-1 / MISC-2)', () => {
    ...
    it('every applied .jc-as-broadcast-* class has a selector in the stylesheet', () => {
        const css = extractStylesheet();
        const used = usedBroadcastClasses();
        expect(used).toContain('jc-as-broadcast-timeout-row'); // the lost one
