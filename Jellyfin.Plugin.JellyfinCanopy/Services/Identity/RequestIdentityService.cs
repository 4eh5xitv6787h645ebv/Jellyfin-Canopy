...
namespace Jellyfin.Plugin.JellyfinCanopy.Services
{
    ...
    public enum IdentityConfidence
    {
        ...
        /// <summary>Per-browser jc-spoiler-uid cookie, validated against a session on the request IP. Strong for web.</summary>
        Cookie,
    ...
    // request identity through one documented, unit-tested choke point
    ...
    //   3. jc-spoiler-uid cookie  — web browsers attach it to anonymous
    ...
    public sealed class RequestIdentityService
    {
        ...
        // src/enhanced/spoiler-guard/identity.ts). Browsers attach it to every
        ...
        // F8: negative-cache for a jc-spoiler-uid cookie that names a user
            ...
            lock (_singleUserLock)
            {
                ...
                catch (Exception ex)
                {
                    WarnRateLimited(
                        ...
                        $"JC request identity: user enumeration for the single-user shortcut threw: {ex.Message}");
                    result = null;
                }
        ...
        public RequestIdentity Resolve(Microsoft.AspNetCore.Http.HttpContext httpContext)
        {
            ...
            // Tier 3: per-browser cookie disambiguation. The jc-spoiler-uid
        ...
        // jc-spoiler-uid cookie names a user absent from the cached set: the
        ...
        private IReadOnlyCollection<Guid> ScanActiveSessionUsers(Microsoft.AspNetCore.Http.HttpContext httpContext)
        {
            ...
            catch (Exception ex)
            {
                WarnRateLimited(
                    ...
                    $"JC request identity: ISessionManager.Sessions enumeration threw: {ex.Message}");
            ...
            foreach (var s in sessions)
            {
                ...
                catch (Exception ex)
                {
                    WarnRateLimited(
                        ...
                        $"JC request identity: skipped a session row during IP match: {ex.Message}");
                    continue;
                }
            }
        ...
        private void RemoteEndpointParseFailedWarn(string endpoint)
        {
            ...
            WarnRateLimited(
                "endpoint-parse",
                $"JC request identity: unrecognized SessionInfo.RemoteEndPoint format '{safe}' — session-by-IP fallback offline. Likely a Jellyfin format change.");
        }
