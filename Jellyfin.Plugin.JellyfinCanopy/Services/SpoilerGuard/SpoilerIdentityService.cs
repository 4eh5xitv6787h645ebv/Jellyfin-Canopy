...
namespace Jellyfin.Plugin.JellyfinCanopy.Services
{
    ...
    public sealed class SpoilerIdentityService
    {
        ...
        // "jeu" (JC user) cannot appear inside Jellyfin's hex image tags, and
            ...
            lock (_mapLock)
            {
                ...
                try
                {
                    ...
                    foreach (var m in collided)
                    {
                        map.Remove(m);
                        _logger.LogWarning(
                            "Spoiler Guard identity: two users share marker {Marker}; both excluded from tag-based identity (falling back to IP matching for them).",
                            m);
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    _logger.LogWarning("Spoiler Guard identity: user enumeration failed while building the marker map: {Message}", ex.Message);
