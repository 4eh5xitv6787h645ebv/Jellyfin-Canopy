...
namespace Jellyfin.Plugin.JellyfinCanopy.Services
{
    ...
    /// stored under <see cref="JellyfinCanopy.BrandingDirectory"/> using fixed,
                ...
                if (!string.IsNullOrWhiteSpace(brandingDir))
                {
                        ...
                        && File.Exists(filePath))
                    {
                        ...
                        if (fileInfo.Length > 0)
                        {
                            ...
                            {
                                _logger.LogInformation("Jellyfin Canopy: serving custom branding via request-time middleware (IStartupFilter).");
                            }
