...
namespace Jellyfin.Plugin.JellyfinCanopy.Configuration
{
    ...
    public class PluginConfiguration : BasePluginConfiguration
    {
        public PluginConfiguration()
        {
            ...
            // initializers below (the ctor wins on a fresh config).
            SpoilerBlurEnabled = false;
            SpoilerBlurIntensity = 40;
            // Strip toggles default ON (strictest posture); admins relax the
        ...
        /// <summary>
        /// Master switch for the admin-only action-sheet Search / Interactive Search feature
        /// (movies/series/seasons/episodes → the configured Sonarr/Radarr instances). Public so the
        /// client can hide the menu items when off; the endpoints are policy-gated regardless.
        /// </summary>
        public bool ArrSearchEnabled { get; set; }

        /// <summary>
        /// Whether the mutating management actions (Monitor / Unmonitor and Add to Sonarr/Radarr) are
        /// offered on top of search. Lets an admin keep read-mostly search while disabling changes to
        /// the arr library. Gated in addition to <see cref="ArrSearchEnabled"/>.
        /// </summary>
        public bool ArrSearchManageEnabled { get; set; }
        ...
        /// </summary>
        public const string DefaultArrTagsPrefix = "JC Arr Tag: ";
        ...
        /// <summary>
        /// Admin-configured order of the pages-framework pages, a CSV of page ids
        /// (default "calendar,downloads,bookmarks,hidden-content"). The client
        /// registry (src/enhanced/pages/registry.ts) reads it, ignoring unknown ids
        ...
        // Hide the JC Reviews panel on guarded Series pages — TMDB and
        ...
        // spoiler list (UserSpoilerBlur.Series). History is checked via the
        // user's played episodes so rewatches don't re-trigger.
        public bool SpoilerAutoEnableOnFirstPlay { get; set; } = false;
        // Each successful Seerr request via JC registers a pending entry
        ...
        // Advanced per-category reveals (upstream JE feature request: granular
        // spoiler guard). When on, the user's NEXT unwatched regular episode
        // and the rest of its season get their own strip masks below instead
        // of the uniform full strip; every other unwatched episode (later
        // seasons, skipped earlier seasons, specials) keeps the full strip.
        // Category resolution failure of any kind falls back to full strip.
