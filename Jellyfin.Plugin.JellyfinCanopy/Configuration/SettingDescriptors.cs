...
namespace Jellyfin.Plugin.JellyfinCanopy.Configuration
{
    ...
    {
        /// <summary>GET /JellyfinCanopy/public-config (any caller, including pre-login).</summary>
        Public,

        /// <summary>GET /JellyfinCanopy/private-config (admin users only).</summary>
        Private,
    ...
    ///    JellyfinCanopy.Tests/Snapshots — when a golden test fails, fix the descriptor,
    ///    never the snapshot.
    /// </summary>
    internal static class SettingDescriptors
    {
        ...
        private static IReadOnlyList<SettingDescriptor> BuildRegistry()
        {
            var descriptors = new[]
            {
                ...
                Public("ThemeStudioEnabled", c => c.ThemeStudioEnabled),
                Public("ThemeStudioDefaultPreset", c => c.ThemeStudioDefaultPreset),
                Public("ThemeStudioDefaultPalette", c => c.ThemeStudioDefaultPalette),
                Public("ThemeStudioDashboardEnabled", c => c.ThemeStudioDashboardEnabled),
