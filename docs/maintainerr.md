...
2. Enter the **Internal URL** that the Jellyfin server can reach, or press
   **Detect Maintainerr** to fill it automatically when Maintainerr answers on
   a well-known address (e.g. `http://maintainerr:6246`). Detection only fills
   an empty field and sends no credentials (see
