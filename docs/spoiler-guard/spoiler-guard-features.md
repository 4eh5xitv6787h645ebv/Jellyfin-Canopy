...
Open the Seerr **More Info** modal for any title — whether you're about to request it or another user already has. You'll see an **Enable Spoiler Guard** toggle in the discovery modal:

> Spoiler Guard will engage when this title arrives in your library.

Turn it on to register a *pending* Spoiler Guard intent for the TMDB id. When the content lands in your library (via Seerr, manual import, or any other source), the plugin automatically promotes the pending entry into a real per-series or per-movie protection — no extra clicks from you.
...
The blur (or hide-mode placeholder) automatically lifts for the episode you just marked. No manual refresh needed — the plugin rides Jellyfin 12's live-update push channel (the `UserDataChanged` websocket message the server broadcasts when your watched-state changes) and rewrites the affected image URLs on the page within a couple of frames of the server confirming the change.
...
Your admin decides which spoiler surfaces get stripped, but you can relax any of them for yourself. Open the JE settings panel (gear icon → **Jellyfin Elevate**) and expand the **Spoiler Guard** section. Under **"Show me this even with Spoiler Guard on"** there's a checkbox per category:
