...
Users can also manually opt in via the **Enable Spoiler Guard** toggle in the Seerr More Info modal (always available regardless of this toggle), which is useful when another user has already requested the title.
...
The in-place refresh is also what runs automatically when a user marks an episode watched / unwatched — that path (driven by Jellyfin 12's `UserDataChanged` live-update push) is *always* soft regardless of this setting (a reload mid-playback is too jarring).
