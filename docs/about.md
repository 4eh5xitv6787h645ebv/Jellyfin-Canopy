...
The project is developed **100% with AI** — agentic coding tools drive the design, implementation, testing, and review — directed and curated by a human maintainer. Nothing lands on trust alone: every change has to pass the blocking gate suite (type-checking, unit tests, coverage, builds, contracts, and release validation) before it ships. ESLint findings and its warning-cap signal are always reported for review but are advisory; a broken lint invocation still blocks.
