...
| `home-libraries-v12` | Existing home section-zero cards in their original source order | scrolling row or responsive desktop/phone grid |
...
Dynamic accents are implemented by `dynamic-color.ts` and the identity-owned runtime. Discovery performs no computed-style or geometry walk: one shared body-mutation subscriber schedules one post-paint animation frame and selects the first exact same-origin `/Items/{id}/Images/Primary` or `/Items/{id}/Images/Backdrop` candidate requested by the profile. The response must be an image and is streamed with a hard 2 MiB limit before decode. `createImageBitmap` downsamples directly to 32 × 32 where available; Firefox/WebKit/Safari capability gaps use the same bounded blob through a temporary local `HTMLImageElement`, then revoke its object URL immediately after the 32 × 32 canvas read. Deterministic finite buckets select a useful chromatic color, and only typed primary/on-primary variables enter the generation-owned dynamic style. The query-free in-memory LRU holds at most 16 derived colors. One analysis can be in flight, navigation/configuration/identity teardown aborts it, and a transient failure gets at most three total attempts with 1 s then 5 s backoff despite any number of DOM mutations. Candidate paths, query tags, item IDs, pixels, object URLs, and derived cache entries are never persisted, logged, exported, or sent off server.
...
On supported surfaces, root capability attributes record the resolved breakpoint, pointer/hover state, light/dark mode, contrast, transparency, reduced motion, forced colors, profile, preset, palette, and route. Effects publish `data-jc-theme-effects-level`, `-effects-material`, `-image-treatment`, `-motion-profile`, `-page-transition`, `-stagger`, `-dynamic-source`, and `-dynamic-accent`. Calendar state publishes `data-jc-theme-schedule`, `-schedule-kind`, and `-schedule-time-zone`; manual selection is explicit rather than absent. Presentation enums are published separately as `data-jc-theme-density`, `-navigation`, `-home-hero`, `-home-libraries`, `-details`, `-seasons`, `-card-actions`, `-poster-ratio`, `-cast-shape`, `-progress-position`, `-watched-indicator`, and `-unwatched-indicator`. Player enums use the same validated-token boundary and publish `data-jc-theme-player-osd-density`, `-control-material`, `-pause-screen-material`, `-subtitle-backdrop`, and `-trickplay-shape`. Media-query and layout-class listeners update those attributes and the same two theme style layers live. Auto navigation resolves to bottom on phones and header on desktop/wide; auto seasons resolve to list on phones and grid on desktop. Home libraries preserve source order while switching between a horizontal scrolling row and an opt-in responsive grid. Reduced-motion and reduced-transparency preferences can only remove effects; coarse/no-hover input converts hover-only card actions to always-visible; high contrast strengthens focus. Logical properties, safe-area variables, wrapping metadata and titles, and bounded dialog/card sizes cover RTL, long labels, zoom, missing artwork, and mixed media ratios without adding a runtime-owned content or focus-order surface.
...
`scripts/theme-studio-quality.contract.json` is the release-gate inventory for Theme Studio. Run `npm run check:theme-studio-quality` after changing a preset, supported surface, research source, test owner, screenshot, accessibility rule, Playwright configuration, or CI workflow. The checker fails closed unless all four supported modern viewports, the three unsupported stock/no-op markers, all nine primary presets, both desktop and phone Linux baselines, all 157 classified forum threads, the complete reviewed GitHub/GitLab root inventory, the accessibility scanner contract, every cross-cutting evidence owner, and the exact 28-test Firefox/WebKit inventory remain present. This keeps the definition of done machine-readable instead of inferring it from a green but narrower test selection.

The #449 responsive home-library grid adds exactly 3,661 raw bytes across the generated adapter, source maps, and localized editor labels. This audit also canonicalizes esbuild's virtual output directory and rebases source-map paths to the shipped `dist` layout, removing checkout-path bytes that varied between local and CI temporary directories. The resulting platform-independent total/published ceilings are the exact 7,120,696/7,203,373-byte production measurements; a one-byte-lower negative test fails both boundaries. Boot, cold-Home, feature-closure, request, and gzip ceilings remain unchanged. The refreshed desktop/phone PNGs reduce the measured documentation/repository visual payloads from 27,880,765/35,686,680 bytes to 27,879,254/35,685,169 bytes. Both aggregate ceilings are now those exact measurements, removing the repository policy's former 3,231-byte slack as well as retaining the 1,511-byte image reduction.
...
| Firefox and WebKit interoperability | The blocking `theme_studio_browsers` matrix reruns the exact committed 28-test Theme Studio inventory in both engines with zero retries and zero skipped tests |
...
Chromium remains the sole pixel-baseline owner because browser rasterization is not expected to be byte-identical. Firefox and WebKit run the same semantic, geometry, accessibility, lifecycle, policy, migration, and stock/no-op assertions with snapshot comparison disabled; this is not a reduced test selection. Run either engine locally with `npm run e2e:local -- --shards 2 --cpus-per-server 2 --browser firefox --theme-studio-only`, replacing `firefox` with `webkit` as needed. The required-inventory reporter rejects a missing, duplicate, unexpected, failed, interrupted, timed-out, flaky, or skipped test.
