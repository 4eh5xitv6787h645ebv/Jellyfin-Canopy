...
**In the tree (bounded):** `Services/ScriptInjectionStartupFilter.cs` preserves Jellyfin's `Accept-Encoding` negotiation and uses the source validators internally to revalidate cached identity/gzip/Brotli representations. Each injected representation gets a strong body-derived ETag; GET and HEAD expose the same transformed metadata, and `If-Match`/`If-None-Match` are evaluated against that ETag. The source `Last-Modified` is deliberately not exposed because script generation can change independently of `index.html` — reusing that timestamp could validate an older injected shell — so ETag is the authoritative transformed validator. A warm request performs one bodyless host revalidation plus bounded metadata work. Cold fills use a canonicalized encoding key, are serialized process-wide, and stop buffering or decoding at 2 MiB before serving the untouched host response; canceled or incomplete bodies are never admitted. The singleton cache is capped at 12 representations, 2 MiB per representation, and 8 MiB total.
