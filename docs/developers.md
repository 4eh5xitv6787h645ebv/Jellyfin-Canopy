...
Jellyfin Canopy supports **only Jellyfin 12's modern React/MUI web layout**. Jellyfin may still expose a classic layout choice, but Canopy treats it as an unsupported host state: the Jellyfin base UI remains usable and Canopy's client feature graph does not initialize.

- The **React/MUI modern layout** is the required non-TV browser host. Selection is stored in the unprefixed `localStorage` key `layout`.
- TV mode is separate and exempt from browser-layout steering; it is not classified as the unsupported classic browser layout.

!!! warning "Gate before the feature graph"
    `js/plugin.js` reads Jellyfin's saved layout at authenticated startup. A stored desktop/mobile classic value stops initialization before the runtime manifest or feature modules are loaded. `src/core/layout.ts` then owns only modern-layout readiness and stamps `jc-modern-layout` after a visible MUI toolbar exists.

The route tree is chosen **once at module init** (`WEB src/RootAppRouter.tsx`), so a layout change requires a full reload.

**Present ≠ ready.** The fixed MUI AppBar itself can have no useful `offsetParent`; detect readiness from a visible `.MuiAppBar-root .MuiToolbar-root` child with client rects.
...
| Item-detail action row | `.page:not(.hide) .mainDetailButtons` — classic view-manager markup hosted inside the modern shell |
| Home sections | `#indexPage .homeSectionsContainer` — modern wrapper around the native home controller |
| Player OSD | `.videoOsdBottom .buttons.focuscontainer-x` |

`getHeaderRightContainer()` resolves only the real MUI action tray. That tray is destroyed on entering `/video` and remounts after exit, so header injection must be idempotent and retryable. `getSidebarContainer()` resolves `.MuiDrawer-paper` on modern mobile only; modern desktop has no drawer.

**Header-tray single-row containment (the resolver owns it).** Every Canopy header button is inserted into the real MUI action tray returned by `getHeaderRightContainer()`. The resolver marks that Box with `jc-header-tray`; shared CSS keeps it a single horizontally scrollable row with non-shrinking children and `flex:1 1 0`, so overflow stays reachable and the separate profile Box remains on-screen. The resolver never synthesizes a fallback container and returns `null` until the real action Box is ready.
...
This lives in `js/plugin.js` (`resolveLayoutEnforcement` / `applyLayoutEnforcement`), run from the pre-auth early public-config fetch so the login screen is covered too. Only `ForceExperimental` steers a client; `None`, retired values, and unknown values are inert. A modern/default browser is never reloaded, and a stored `tv` layout is exempt. Two loop guards protect it:
...
Detection recognizes classic desktop/mobile values only at this boot boundary. Persisted retired or unknown enforcement values normalize to `None`; there is no classic-layout steering or Canopy feature path.
...
| Classic-layout DOM fallbacks (`.headerRight`, `.mainDrawer-scrollContainer`) | **DELETE** — Canopy client features do not initialize on that unsupported host layout |
...
### Modern-layout injection doctrine

The modern MUI AppBar, action tray, mobile drawer, and hosted native views are the only Canopy injection targets. Source paths below use the `src/…` shorthand for `Jellyfin.Plugin.JellyfinCanopy/src/…`.

!!! note "Source is authoritative"
    Live DOM probes can explain host markup, but this worktree's modern-only source and tests define Canopy's support contract.

#### Modern readiness — `detectModernLayout()` owns the cached state

`detectModernLayout()` probes a visible `.MuiAppBar-root .MuiToolbar-root`. The first successful result is cached and stamps `jc-modern-layout`; an unresolved header is not cached, so navigation retries can observe the later MUI mount. Unsupported classic-layout detection belongs to the loader and runs before this module can be imported.
...
- Do not infer readiness from Jellyfin's generic `layout-desktop` class; it is not a MUI-toolbar readiness signal.

#### Modern CSS readiness — the `jc-modern-layout` stamp

`stampLayoutClass()` adds `jc-modern-layout` only after the MUI toolbar resolves. It is a no-op while the header is not ready and idempotent after resolution, so post-resolution navigation checks are cheap.

The stamp lets readiness-sensitive CSS avoid applying before the modern shell has mounted.

#### The modern header injection container — `getHeaderRightContainer()`
...
- If the real MUI action Box is not ready, the resolver returns `null`. It never synthesizes a competing toolbar container; callers retry after the host mounts or remounts the tray.

!!! warning "Gotcha — the profile avatar is a SEPARATE element OUTSIDE Canopy's modern container"
    The user-menu / profile avatar button (`[aria-controls="app-user-menu"]`) lives in the user-menu Box that is the tray's **next** sibling. Do not assume the avatar is part of the tray or try to order within it.

A one-time CSS fix (`id jc-mui-header-button-fix`) normalizes Canopy's native-compatible `.headerButton.paper-icon-button-light` markup inside `.MuiToolbar-root` to MUI's 48px button / 24px icon convention.
...
- The **layout bootstrap** in `src/core/layout.ts` stamps `jc-modern-layout` after MUI toolbar readiness, then retries on navigation only until the first successful resolution.
...
#### Modern header and drawer surfaces

- **The pages tray** reconciles after `jc-modern-layout` readiness and injects one compact ordered button per available page.
- **The drawer** resolves `.MuiDrawer-paper` on modern mobile. Modern desktop has no drawer, so page discovery uses the AppBar tray and preferences menu.
...
- Enhanced native-tabs creates a wrapper `<div id="jc-native-tabs-group">` with `order:-1` inside the resolved MUI action tray, so the group leads within that tray. It owns an aria-hidden separator and inserts links before it.
...
!!! note "Modern discoverability"
    When the host tab slider is not laid out in the modern shell, `ensureDiscoverable` publishes the equivalent link through the MUI action tray. This is the supported path and must remain keyboard accessible.
...
The browser editor preserves the same separation. The normal preferences route without a different `userId` remains self-service. Admin-target mode is selected only from the currently visible user's preferences view, including the modern host's path and hash route forms:
...
| **Canopy modules are updated** | The loaded schema-v2 client-manifest `buildId` is compared with the active server's current content-derived module-graph id, so a same-version module replacement is detected. The classic `plugin.js` loader belongs to the document host and is deliberately outside this remote-server identity. The client follows the configured Smart / Home-only / Ask / Disabled policy. |
...
- **Editing is protected too.** Dashboard/configuration/metadata/profile routes and Jellyfin 12's `userprofile`, `mypreferencesmenu`, `mypreferencescontrols`, `mypreferencesdisplay`, `mypreferenceshome`, `mypreferencesplayback`, and `mypreferencessubtitles` routes defer reload. Open dialogs/action sheets, Canopy custom overlays, focused editors, and pending settings/mutation transactions are protected by the same document-lifetime safety owner.
- **Scope is Canopy web code.** Desktop/mobile browsers and Jellyfin app WebViews execute the coordinator. Fully native clients that do not load Canopy have no plugin-owned page runtime to refresh. An opted-in Platform native client may use the existing inert config-changed marker to refetch item-detail state, but that does not give it a Canopy page or Smart Refresh runtime.
...
- **The first upgrade from a pre-Smart-Refresh bundle can only prompt.** JavaScript already running in a page cannot acquire the new safety coordinator retroactively. The legacy `_je` version heartbeat and config-change push therefore return a dotted-numeric compatibility successor, which makes those old pages show their existing “refresh to load the new version” prompt even after a same-version replacement. A visible old page checks at most every 15 minutes. One user reload moves it onto the automatic protocol; a backgrounded/timer-suspended old page cannot be contacted until it runs again.

Hidden/background documents do not poll: a hidden `visibilitychange` or Cordova `pause` aborts an in-flight check and cancels pending poll/decision timers. Visible `visibilitychange`, `focus`, `pageshow`, network `online`, and Cordova `resume` all trigger an immediate no-cache state check. Foreground decisions wait for the complete coalesced check drain, so an old Smart policy cannot reload before a newly-disabled policy arrives. Cordova pause state belongs to the document rather than an account epoch, protecting Android WebViews whose `document.visibilityState` remains visible across both Activity pause and an identity transition. Visible documents use the admin-configured 5–3600 second cadence. A policy value of zero inactivity still has a one-second client-side interaction-settle floor, allowing Jellyfin-owned fire-and-forget actions (Favorite/Played) to dispatch before the document can reload.

The anonymous, no-store `GET /JellyfinCanopy/client-refresh-bootstrap.js` runs before `plugin.js` and gives the document an initial server/config/force baseline. For a remotely selected server, the classic loader captures the equivalent authenticated state before it reads any owner configuration. The authenticated `GET /JellyfinCanopy/client-refresh-state` response contains the stable Jellyfin `ServerId`, Canopy build id, Jellyfin process generation, configuration revision, force revision, and current policy; it also refreshes the live-session registry heartbeat. Policy and revision come from one provider-lock snapshot, including `ShowNotices`, so a remote server—not the document host—owns whether refresh UI is visible. `ShowNotices` is additive within schema 1: an absent field from an older server means `true`, while a present non-boolean value rejects the payload. Bootstrap adoption requires `ServerId`, and a foreign bootstrap is discarded when the active API client switches servers. For rolling-upgrade compatibility only, an authenticated schema-1 response may omit `ServerId`; that response is bound to its already-authenticated transport context, while a present malformed or mismatched id is rejected. The classic loader and runtime both accept the remaining payload only when every enum, boolean, bounded integer, non-negative revision, and SHA-256 identity is exact, and revisions may not move backwards within one Jellyfin process. A pre-Smart-Refresh remote server can return no usable bootstrap without breaking Canopy startup: the loader continues, marks the baseline unavailable, and the first later valid state conservatively treats the newly observed Jellyfin generation plus positive configuration and force revisions as edges instead of silently absorbing them. Normal policy filtering still applies. Baseline and pending intent survive logout, while an explicit switch to another server starts a new baseline.

Every irreversible reload first reserves a timestamp under a three-per-minute window. Session storage is preferred and the same bounded history is mirrored to local storage as a durable fallback when session storage is unavailable or over quota. If neither backend can prove and persist the bound, automatic reload fails closed and, when notices are enabled, the page shows the manual-reload notice. Dismissal is keyed to the complete refresh-state watermark, so a retry or unchanged Canopy mismatch cannot recreate the same message; it never clears pending intent or stops safety evaluation. Notice suppression is likewise presentation-only: Smart, Home-only, and forced decisions continue silently, while Notify remains non-automatic and waits for a manual reload.

Canopy overlays must use `installModalA11y` or `holdRefreshSafetyForElement`; removing an owned overlay must release that hold. The core API layer holds all POST/PUT/PATCH/DELETE work from queueing through final settlement, and the settings persistence queue adds a transaction hold spanning its write, retries, rebase, and evidence reads. A panel may close without making the underlying save safe to interrupt. If a generic settings, shortcuts, or Elsewhere save has an ambiguous outcome, its local intent remains dirty and the transaction hold deliberately remains fail-closed until a later acknowledged save, identity/page teardown, or an explicit manual reload discards it. Hidden Content uses the same fail-closed rule when its bounded retries end with current-identity dirty data.

`POST /JellyfinCanopy/client-refresh` is protected by `Policies.RequiresElevation`.
...
- A successful authenticated Platform v1 item-detail resolve also records its boundary-validated **device id and user id**. Registration precedes the resolver's final configuration-generation check: a concurrent save therefore either sees the registered device and sends its marker or makes the resolve retry/fail instead of publishing stale state. A raced failed resolve may leave a TTL-bounded provisional entry because removing it could erase a newer resolve; that entry is safe because the carrier is inert and delivery still requires the same user to have a live session on the device. This is an explicit native participation signal, not a new event stream or server-pushed state.
- `LiveNotifierService` sends the config-changed command **per registered device** via `SendMessageToUserDeviceSessions`. Native clients that never reach the Platform resolver's publication boundary are never registered and never receive the carrier; a raced failed resolve may leave only the safe provisional entry described above. Participating native clients use the marker only to refetch authoritative item-detail state.
...
**In the tree (bounded):** `Services/ScriptInjectionStartupFilter.cs` preserves Jellyfin's `Accept-Encoding` negotiation and uses the source validators internally to revalidate cached identity/gzip/Brotli representations. Each injected representation gets a strong body-derived ETag; warm GET and HEAD expose the same transformed metadata, and `If-Match`/`If-None-Match` are evaluated against that ETag. A cold HEAD never downloads the shell just to manufacture metadata; until GET has populated the cache (and whenever HEAD discovers that the source changed), it stays bodyless and omits the source ETag, timestamp, length, and range metadata so an intermediary cannot merge those into an injected GET. The source `Last-Modified` is deliberately not exposed because script generation can change independently of `index.html` — reusing that timestamp could validate an older injected shell — so ETag is the authoritative transformed validator. A warm request performs one bodyless host revalidation plus bounded metadata work. Four fixed admission stripes single-flight both cold fills and source invalidations; keys mirror ASP.NET Core's first-candidate/wildcard rules but retain only the selected identity/gzip/Brotli representation, preventing arbitrary headers or quality values from churning the cache. A source-file byte range cannot describe the rewritten shell, so `Range` and `If-Range` are ignored for index requests and the complete injected `200` representation is returned; this also keeps range conditionals on the transformed ETag. HEAD remains bodyless. Buffering, decoding, and re-encoding stop at 2 MiB; an oversized or untransformable source switches to the already-produced host bytes in the same downstream pass, with source preconditions parsed and combined exactly like ASP.NET Static Files before buffered bytes are committed. Canceled or incomplete bodies are never admitted or committed as fallback. The singleton cache is capped at 12 representations, 2 MiB per representation, and 8 MiB total.
...
- **qBittorrent item telemetry** (`Services/Qbittorrent/QbittorrentTelemetryService.cs`): authorization resolves one exact Jellyfin item before upstream work. One process-wide single-flight snapshot retains at most 2,000 normalized rows for 30 seconds after a bounded login + read-only torrent-list + best-effort logout lifecycle (12-second primary deadline plus at most two seconds of session cleanup); the response body is capped at 2 MiB and discarded after parsing. A TTL timer drops normalized raw paths even without a later caller, configuration revisions cannot publish stale snapshots, and failures clear prior rows then back off for two seconds. The transport owns only exact legacy `SID` and qBittorrent 5.2 `QBT_SID_<port>` cookies; the pooled handler owns no cookies, credentials, redirects, or proxy. Item matching is server-only, mapping at most 32 configured roots and rejecting equal-strength ambiguity. Browser DTOs contain seven closed redacted fields and no hash/name/path/tracker URL/literal-IP topology/credential material. This is operator-local S9 work with at most one upstream operation in flight, so S5 fleet jitter is inapplicable.
...
│   ├── LiveNotifierService.cs # Pushes live updates (config-changed etc.) to JC web sessions
│   │                          # and opted-in Platform native sessions (via ILiveSessionRegistry;
│   │                          # see the Live updates section)
│   ├── LiveSessionRegistry.cs # Registry of eligible Canopy web/native sessions — scopes live pushes
...
- `npm run typecheck:src` / `./verify.sh lint` / `npm run test:client` — strict type check, advisory ESLint reporting, and vitest unit tests for the `src/` tree. `npm run test:client:coverage` runs that complete suite once with the `src/core` ratchet; `npm run test:server:coverage` does the same for xUnit and Cobertura. Do not precede either coverage command with its plain test command: the coverage run is the test evidence. Client and server coverage record a reviewed clean-run envelope in `scripts/coverage-baselines.json` and select its observed high-water—not a modal or repeated lower result—as `coveredLines`; that high-water never decreases at fixed scope. A narrow downward instrumentation tolerance permits negligible tool variance. Its floor may move downward only to the exact minimum supported by linked clean-run evidence on identical source, tests, and total-line scope. Unexplained coverage loss, an unevidenced or unreviewed tolerance increase, scope drift, and an unrecorded gain still fail with measured-versus-required diagnostics. A legitimate scope change updates that artifact explicitly in the reviewed change. Use raw `npm run lint` only when you specifically need ESLint's unmodified exit code.
