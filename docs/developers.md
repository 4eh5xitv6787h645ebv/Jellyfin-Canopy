...
- **qBittorrent item telemetry** (`Services/Qbittorrent/QbittorrentTelemetryService.cs`): authorization resolves one exact Jellyfin item before upstream work. One process-wide single-flight snapshot retains at most 2,000 normalized rows for 30 seconds after a successful login + read-only torrent-list pair; the response body is capped at 2 MiB and discarded after parsing. Failures back off for two seconds. Item matching is server-only, mapping at most 32 configured roots and rejecting equal-strength ambiguity. Browser DTOs contain seven closed redacted fields and no hash/name/path/tracker URL/topology/credential material. This is operator-local S9 work with at most one upstream operation in flight, so S5 fleet jitter is inapplicable.
