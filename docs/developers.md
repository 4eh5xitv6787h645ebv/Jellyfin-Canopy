...
`scripts/theme-studio-quality.contract.json` is the release-gate inventory for Theme Studio. Run `npm run check:theme-studio-quality` after changing a preset, supported surface, test owner, screenshot, accessibility rule, Playwright configuration, or CI workflow. The checker fails closed unless all four supported modern viewports, the three unsupported stock/no-op markers, all nine primary presets, both desktop and phone Linux baselines, the accessibility scanner contract, every cross-cutting evidence owner, and the exact 28-test Firefox/WebKit inventory remain present. This keeps the definition of done machine-readable instead of inferring it from a green but narrower test selection.
