...
- A successful authenticated Platform v1 item-detail resolve also records its boundary-validated **device id and user id**. This is an explicit native participation signal: it makes that device eligible for the existing inert config-changed marker, not for a new event stream or server-pushed state.
- `LiveNotifierService` sends the config-changed command **per registered device** via `SendMessageToUserDeviceSessions`. Native clients that never successfully resolve the Platform surface are never registered and never receive the carrier; participating native clients use its marker only to refetch authoritative item-detail state.
