...
Jellyfin Elevate keeps open browser sessions in step with the server without manual refreshes. The client subscribes once to Jellyfin 12's websocket (the SDK socket — see [Jellyfin 12 Platform](../v12-platform.md#2-script-injection-events-s2)) and fans the server's pushes out to the plugin's features.
