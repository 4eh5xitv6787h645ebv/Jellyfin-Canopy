...
Pass them raw text, interpolate their result raw. If you add a producer like these, it must escape its input up front the same way (and be added to the guard's `PRE_ESCAPING_PRODUCERS` list).
...
- **`isCssColor`** (`src/enhanced/hidden-content-page/admin.ts`) — the same idea for CSS color values; the candidate validator for theme variables (see the guard's allowlist notes).
