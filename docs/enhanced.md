...
Each user can optionally restrict these presentations to an ordered allowlist of up to 16 canonical language tags. Regional tags such as `pt-BR` and `pt-PT` remain separate choices. Filtering happens after metadata is canonicalized and before the three-presentation poster limit, so the first configured languages that are actually present win. **Original** may be placed first only when the item supplies an authoritative original-language tag and that language is present in its audio metadata; Canopy never infers it from a title, filename, country, or interface language. Leave the list empty for the existing unfiltered behavior, choose **Inherit administrator default** to follow future server-wide changes, or use **Reset** to return an explicit custom policy to the compatibility default. A change acknowledged for your own account is applied live; an administrator editing somebody else changes only that user's saved policy.
