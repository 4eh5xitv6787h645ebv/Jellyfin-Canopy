...
Show which audio languages a title has without guessing a nationality. Language Tags display up to **3** deterministic presentations and sit bottom-left by default. On Series and Season posters, a small **✓** means every accessible episode has that language, **◐** means only some do, and **?** means an incomplete media probe prevents a full/partial claim. An empty container displays **0**; a fully inspected non-empty container with no recognized audio-language metadata displays **—**; and a traversal that exceeds the bounded request budget displays **?** without exposing a guessed episode count. These summaries use only episodes the current Jellyfin user can access.
...
Poster and detail-page language displays use the same canonical resolver. Full canonical tags remain in accessible labels, while malformed and undetermined values are omitted rather than producing a broken or misleading flag. Caller-specific Series/Season coverage is never retained in browser storage; older browser language-tag cache entries are discarded once, then leaf-item metadata refills them. Flag icons are served from the plugin's **local asset cache** (mirrored from the flag-icons / flagcdn sets), so no third-party request is made while local asset caching is enabled.
