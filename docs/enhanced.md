...
**Positioning:** the **Settings** tab has a draggable subtitle **position grid** — click or drag on it to place the subtitles, and use the **reset** button to return to the defaults of vertical **85%** and horizontal **50%**. The safe horizontal edge margin keeps even wrapped cues on screen. The chosen point anchors the bottom-center of custom subtitles, so a second line grows upward instead of shifting the caption lower. The live sample mirrors the selected colors, compact background, relative size, and font. Position is a per-user setting and takes effect only when Jellyfin's own subtitle style is set to **Custom**.
...
Show which audio languages a title has without guessing a nationality. Language Tags display up to **3** deterministic presentations and sit bottom-left by default. On Series, Season, and Collection posters, a small **✓** means every accessible episode or collection member has that language, **◐** means only some do, and **?** means an incomplete media probe prevents a full/partial claim. An empty container displays **0**; a fully inspected non-empty container with no recognized audio-language metadata displays **—**; and a traversal that exceeds the bounded request budget displays **?** without exposing a guessed count. These summaries use only episodes or collection members the current Jellyfin user can access. Collection membership is evaluated independently from Series/Season hierarchy; only direct, real Movie members are eligible, so folders, nested collections, Series, and other mixed member types do not distort the denominator. Canopy stops its per-response collection union and member-query work after 20,000 direct relationship edges. Jellyfin currently returns each individual collection's relationship list as one materialized host result, so this limit bounds Canopy's work after that host call rather than the host's allocation for that one list; an oversized result is withheld as **?**. Library events refresh the shared Movie evidence and collection dependency map on the background cache worker, so a response already in flight can retain the preceding snapshot until that bounded flush publishes.

Each user can optionally restrict these presentations to an ordered allowlist of up to 16 canonical language tags. The settings control offers only a bounded inventory derived from audio metadata in that user's currently accessible library; it does not use the current page, expose item identifiers, or accept free-form values. Regional tags such as `pt-BR` and `pt-PT` remain separate choices. Filtering happens after metadata is canonicalized and before the three-presentation poster limit—even across mixed full, partial, and unknown coverage—so the first configured languages that are actually present win. **Original** may be placed first only when the leaf item, first Episode, or accessible collection member supplies an authoritative original-language tag and that language is present in its audio metadata; Canopy never infers it from a title, filename, country, or interface language. Leave a custom list empty for the existing unfiltered behavior, choose **Inherit administrator default** to follow future server-wide changes, or use **Reset** to return to inheritance. A change acknowledged for your own account is applied live to open poster and details presentations; an administrator editing somebody else changes only that user's saved policy and loads that target's own library inventory.

![Season poster with full English and partial Japanese language coverage](images/language-coverage.png)
...
Poster and detail-page language displays use the same canonical resolver and the same full/partial/unknown ordering. Collection detail rows use member-based accessible labels and the same three-presentation limit as their poster. Full canonical tags remain in accessible labels, while malformed and undetermined values are omitted rather than producing a broken or misleading flag. Caller-specific Series/Season and Collection coverage is response-only and is never retained in browser storage; when Spoiler Guard strips tags for the viewer, the server omits the entire Collection projection, including its counts. An older server that does not provide Collection coverage likewise shows no guessed member language. Older browser language-tag cache entries are discarded once, then leaf-item metadata refills them. Flag icons are served from the plugin's **local asset cache** (mirrored from the flag-icons / flagcdn sets), so no third-party request is made while local asset caching is enabled.
