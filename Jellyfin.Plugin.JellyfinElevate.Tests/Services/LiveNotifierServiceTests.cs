...
/// <c>JellyfinElevate = config-changed</c>, and native web clients must ignore
...
public class LiveNotifierServiceTests
{
    ...
    public void BuildConfigChangedCommand_StampsMarkerValueAndVersion()
    {
        ...
        Assert.Equal("config-changed", command.Arguments["JellyfinElevate"]);
    ...
    /// purely to smuggle its <c>JellyfinElevate = config-changed</c> marker; it must NOT
