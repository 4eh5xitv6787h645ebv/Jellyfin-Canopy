...
using Jellyfin.Plugin.JellyfinElevate.Logging;
using Jellyfin.Plugin.JellyfinElevate.Tests.TestDoubles;
using Microsoft.Extensions.Logging;
using Microsoft.Extensions.Logging.Abstractions;
using Xunit;

namespace Jellyfin.Plugin.JellyfinElevate.Tests.Logging;

/// <summary>
/// Covers the dedicated JellyfinElevate_*.log sink that replaced the custom
...
public sealed class FileLoggerTests : IDisposable
{
    ...
    private readonly JellyfinElevateFileLoggerProvider _provider;

    public FileLoggerTests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "je-filelog-tests-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_tempDir);
        _provider = new JellyfinElevateFileLoggerProvider(new StubAppPaths(_tempDir));
    }
    ...
    public void CurrentLogFilePath_KeepsDailyNamePattern()
    {
        ...
        Assert.Matches(@"^JellyfinElevate_\d{4}-\d{2}-\d{2}\.log$", name);
...
#pragma warning restore CA2017
    ...
    public void RotateLogs_UsesFilenameDate_NotWriteTime()
    {
        ...
        var oldByName = Path.Combine(_tempDir, "JellyfinElevate_2000-01-01.log");
        var recentByName = Path.Combine(_tempDir, $"JellyfinElevate_{DateTime.Now:yyyy-MM-dd}.log");
        ...
        using var provider = new JellyfinElevateFileLoggerProvider(new StubAppPaths(_tempDir));
    ...
    [Theory]
    [InlineData("JellyfinElevate_2020-01-01.log", 2020, 1, 1)]
    [InlineData("JellyfinElevate_2026-12-31.log", 2026, 12, 31)]
    public void ResolveLogFileDate_PrefersFilenameDate(string fileName, int year, int month, int day)
    {
        ...
        var resolved = JellyfinElevateFileLoggerProvider.ResolveLogFileDate(fileName, () => DateTime.Now);
    ...
    public void ResolveLogFileDate_FallsBackToLastWriteTime_WhenNameHasNoDate()
    {
        ...
        Assert.Equal(
            fallback,
            JellyfinElevateFileLoggerProvider.ResolveLogFileDate("JellyfinElevate_not-a-date.log", () => fallback));
    }
}
