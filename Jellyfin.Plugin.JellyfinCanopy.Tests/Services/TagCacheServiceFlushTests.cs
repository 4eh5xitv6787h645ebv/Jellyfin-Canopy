...
namespace Jellyfin.Plugin.JellyfinCanopy.Tests.Services
{
    ...
    public sealed class TagCacheServiceFlushTests
    {
        ...
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "jc-tagcache-rebuild-" + Guid.NewGuid().ToString("N"));
        ...
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "jc-tagcache-guardrace-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(tempDir);
            try
            {
                var movieId = Guid.NewGuid();
