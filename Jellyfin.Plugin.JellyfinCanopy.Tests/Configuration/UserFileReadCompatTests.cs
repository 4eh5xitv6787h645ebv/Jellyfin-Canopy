...
namespace Jellyfin.Plugin.JellyfinCanopy.Tests.Configuration
{
    ...
    public class UserFileReadCompatTests : IDisposable
    {
        ...
        {
            _baseDir = Path.Combine(Path.GetTempPath(), "jc-userfile-read-" + Guid.NewGuid().ToString("N"));
        ...
        private string UserDir => Path.Combine(_baseDir, "configurations", "Jellyfin.Plugin.JellyfinCanopy", UserId);
        ...
        {
            var configDir = Path.Combine(_baseDir, "configurations", "Jellyfin.Plugin.JellyfinCanopy");
