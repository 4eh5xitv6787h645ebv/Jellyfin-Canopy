...
namespace Jellyfin.Plugin.JellyfinCanopy.Tests.Configuration
{
    ...
    /// server→page; nothing checked that each <c>JC.pluginConfig.X</c> the TypeScript reads
    ...
    /// <see cref="SettingExposure.Neither"/>: <c>JC.pluginConfig</c> is public-config merged
    ...
    public class ClientConfigKeyLivenessTests
    {
        ...
        public void EveryClientPluginConfigReadIsBackedByAProjectedDescriptor()
        {
            ...
            Assert.True(
                orphans.Count == 0,
                "JC.pluginConfig reads with no projected (Public/Private/Both) descriptor — the key is "
        ...
        {
            // src/types/jc.ts's PluginConfig is a deliberate placeholder (an index signature
            ...
            Assert.True(hasIndexSignature,
                "src/types/jc.ts PluginConfig lost its `[key: string]: unknown` index signature");
            ...
            Assert.True(phantom.Count == 0,
                $"src/types/jc.ts PluginConfig declares keys with no projected descriptor: {string.Join(", ", phantom)}");
        }
        ...
        {
            var text = File.ReadAllText(Path.Combine(SrcRoot(), "types", "jc.ts"));
            var start = text.IndexOf("export interface PluginConfig {", StringComparison.Ordinal);
            Assert.True(start >= 0, "PluginConfig interface not found in src/types/jc.ts");
        ...
        private static string SrcRoot([CallerFilePath] string sourceFile = "")
            => Path.GetFullPath(Path.Combine(
                ...
                "..", "..", "Jellyfin.Plugin.JellyfinCanopy", "src"));
    }
}
