// Unit tests for src/jellyseerr/more-info-modal/render.ts — XSS escaping of
...
// JE.escapeHtml (the test setup stub is a no-op) which render.ts captures
// at import.
import '../../core/ui-kit';
...
describe('buildModalContent escaping', () => {
    beforeAll(() => {
        ...
        (JE as { t?: (key: string) => string }).t = (key: string) => key;
    });
    ...
    it('renders hostile posterPath/backdropPath/title inert (no element injection)', () => {
        ...
        const statValues = host.querySelectorAll('.je-more-info-stat-value');
