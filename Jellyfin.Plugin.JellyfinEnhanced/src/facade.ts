...
// window.JellyfinEnhanced — the members that external consumers depend on and
// that therefore must never be removed or renamed:
//
//   - user scripts (third-party snippets people paste into their server) read
//     JE.core.*, JE.t, JE.toast, JE.pluginConfig, JE.currentSettings, etc.
//   - Configuration/config-page.js drives JE.customPlugins.refresh() and mirrors
//     JE.toast semantics.
//   - automation / E2E waits on JE.initialized.
...
//
// This is documentation-as-types, not a narrowing: JEGlobal (src/types/je.ts)
...
import type { JECore, PluginConfig, UserSettings } from './types/je';

/**
 * The frozen public API of window.JellyfinEnhanced. Signatures here MUST match
 * the corresponding JEGlobal members exactly (JEGlobal extends this interface).
 */
export interface JellyfinEnhancedPublicApi {
