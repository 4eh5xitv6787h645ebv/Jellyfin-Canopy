...
// `.je-tag-host` div that is a *sibling* of `.cardImageContainer` inside
...
import type { TagSpec } from '../types/je';
...
function minimalSpec(): TagSpec {
    return {
        ...
        containerClass: 'je-q',
        taggedAttr: 'jeQTagged',
    };
}

/**
 * Build a faithful card and return the `.je-tag-host` render target — the
...
describe('tag-renderer-base ignore selectors (MISC-1)', () => {
    ...
    it('ignores the je-tag-host render target on #searchPage when DisableTagsOnSearchPage is on', () => {
        JE.pluginConfig = { DisableTagsOnSearchPage: true };
    ...
    it('does NOT ignore search-page cards when DisableTagsOnSearchPage is off', () => {
        JE.pluginConfig = { DisableTagsOnSearchPage: false };
        const host = buildCardHost('searchPage');
        const ctx = register('test-quality-search-off', minimalSpec());
        expect(ctx.shouldIgnore(host)).toBe(false);
    });

    it('ignores the je-tag-host on #itemDetailPage .infoWrapper cards', () => {
