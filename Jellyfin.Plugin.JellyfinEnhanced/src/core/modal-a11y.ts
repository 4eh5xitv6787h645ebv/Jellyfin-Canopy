...
//
// Shared accessible-modal helper for JE's custom overlays. Each overlay used to
// hand-roll (or omit) focus management, and the global shortcut listener
// (enhanced/events.ts) fired *through* any open JE modal (INT-1). This is the
...
// open-modal counter + the `je-modal-open` body class — the signal the global
...
/** True while any JE modal installed via installModalA11y is open. */
 ...
 * trap, Escape, focus restore, and suppression of JE global shortcuts while
...
export function installModalA11y(root: HTMLElement, opts: ModalA11yOptions = {}): ModalA11yHandle {
    ...
    openModalCount++;
    document.body.classList.add('je-modal-open'); // read by the global key listener (INT-1)
    ...
    return {
        release(): void {
            ...
            if (openModalCount === 0) document.body.classList.remove('je-modal-open');
