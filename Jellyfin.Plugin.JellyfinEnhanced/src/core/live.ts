...
// receives — UserDataChanged, LibraryChanged — plus JE's own out-of-band channel
// (a GeneralCommand whose Arguments carry a JE marker; native ignores it), then
// fans every message out to feature handlers registered through JE.core.live.on.
//
// It fails soft when ApiClient.subscribe is unavailable (older hosts / no SDK
// socket bridge): JE still works and features keep polling / manual refresh.
//
// Public surface: JE.core.live { on, off, emit, isConnected, getHandlerCount }.

import { JE } from '../globals';
import { register } from './lifecycle';
import type { LiveApi, LiveHandler, LiveMessage } from '../types/je';

JE.core = JE.core || {};

const logPrefix = '🪼 Jellyfin Enhanced: Live:';
...
// GeneralCommand carries JE's own channel (marked in its Arguments dictionary).
...
// Arguments dictionary. Its VALUE is the JE live event name to fan out — so a
// single closed-enum message type multiplexes any number of JE channels.
const JE_MARKER = 'JellyfinEnhanced';

/** JE live event names — the `type` passed to on() / emit(). */
export const LIVE = {
...
/**
 * Subscribe to a JE live event type.
...
/**
 * Route a raw SDK message to the matching JE live event. Native remote-control
 * GeneralCommands (no JE marker) are ignored here — the host client still
 * handles them; JE only claims its own marked messages.
 */
