...
const REPO_ROOT = TEST_FILE_PATH.replace(
    /Jellyfin\.Plugin\.JellyfinEnhanced\/src\/test\/[^/]+$/,
    '',
);
const TMP_DIR = REPO_ROOT + 'tmp';

// content 'je-checksum-fixture' → this uppercase MD5 (verified out-of-band).
const FIXTURE_CONTENT = 'je-checksum-fixture';
const FIXTURE_MD5 = '8A60C02525CF844A77FEB385AE0A357E';
const FIXTURE_ZIP = 'je-checksum-fixture.zip';
...
describe('verifyChecksums (MB-8)', () => {
    ...
    it('skips (does not error) an entry whose asset is absent locally (frozen history)', () => {
        const { errors } = verifyChecksums(manifest('F'.repeat(32), 'je-absent-fixture.zip'), TMP_DIR);
        expect(errors).toEqual([]);
    });
});
...
describe('measurePackage cobertura line counting (MB-9)', () => {
    it('counts each class-level line once, not the duplicated method lines', () => {
        const xml = [
            '<coverage><packages>',
            '<package name="Jellyfin.Plugin.JellyfinEnhanced">',
