...
// ui-kit installs the real JE.escapeHtml (the setup stub is a no-op) which the
...
describe('requests page error state', () => {
    ...
    beforeEach(async () => {
        ...
        JE.core = { api: { plugin } };
        JE.pluginConfig = { JellyseerrEnabled: true, ShowDownloadsInRequests: true };
        JE.t = (k: string) => k;
        JE.toast = toast;
        // Import AFTER JE.core.api is set — data.ts captures `JE.core.api` at eval.
