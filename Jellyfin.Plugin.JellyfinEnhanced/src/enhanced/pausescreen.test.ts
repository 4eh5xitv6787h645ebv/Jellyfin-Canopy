...
function je(): PauseApi {
    return window.JellyfinEnhanced as unknown as PauseApi;
}

function initPauseScreen(): void {
    (window.JellyfinEnhanced as unknown as PauseApi).initializePauseScreen();
}
...
describe('pause-screen singleton + teardown', () => {
    beforeEach(() => {
        ...
        (window.JellyfinEnhanced as unknown as { t: (k: string) => string }).t = (k: string) => k;
