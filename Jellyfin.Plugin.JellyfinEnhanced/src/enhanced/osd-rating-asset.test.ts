...
describe('osd-rating tomato icons', () => {
    afterEach(() => { document.getElementById('je-osd-rating-style')?.remove(); });

    it('injects data-URI tomato glyphs, never a relative assets/img url', () => {
        ensureStyles();
        const style = document.getElementById('je-osd-rating-style');
        ...
        expect(css).toContain('.je-tomato.fresh');
        expect(css).toContain('.je-tomato.rotten');
    });
});
