...
export function showOffsetAdjustmentModal(group: any): void {
  ...
  if (syncedBookmarks.length === 0) {
    toast(JE.t!('bookmark_no_synced'), 2000);
    return;
  }
    ...
    try {
      ...
      if (updatedCount > 0) {
        const message = offset === 0
          ? JE.t!('bookmark_offset_cleared').replace('{count}', String(updatedCount))
          : JE.t!('bookmark_offset_applied').replace('{count}', String(updatedCount)).replace('{offset}', `${offset > 0 ? '+' : ''}${offset}s`);
        toast(message, 3000);
        closeDialog();
...
export function showDuplicatesSyncModal(bookmarks: Record<string, any>): void {
  ...
  if (duplicates.length === 0) {
    toast(JE.t!('bookmark_no_duplicates'), 3000);
    return;
  }
      ...
      <div class="je-bm-library-modal-content">
        <div class="je-bookmarks-modal-header" style="display: flex; gap: 16px; align-items: flex-start; margin-bottom: 24px;">
          ...
          <div style="flex: 1;">
            ...
            <p style="margin: 0; font-size: 13px; color: #aaa;">${JE.t!('bookmark_duplicate_subtitle').replace('{count}', String(duplicates.length))}</p>
          </div>
        </div>
  ...
  modal.querySelectorAll<HTMLButtonElement>('button.je-btn:not([data-sync-from])').forEach(btn => {
    ...
    btn.addEventListener('click', () => { void (async () => {
      ...
      if (!confirm(JE.t!('bookmark_merge_confirm').replace('{count}', String(oldBookmarks.length)))) {
        return;
      }
      ...
      try {
        ...
        toast(JE.t!('bookmark_merge_success').replace('{count}', String(synced.length)), 3000);
        ...
        toast(JE.t!('bookmark_merge_failed'), 3000);
        btn.disabled = false;
